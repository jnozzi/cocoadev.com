Hello there, I'm having trouble being able to copy an General/NSView, here is the code i've tried:
 .h
...
Basically I want to use the General/NSView in interface builder as a template and then copy that to all the 'tabs' i want, I want to use General/NSViews not General/TabViewItems. The problem is they end up pointing to the same object, so I get the two views I want, and they work in two tabs, just they show the same object. I've tried [contentTemplate copy] to which it doesn't respond...  I'm out of ideas, anyone want to shed some light on what I'm doing wrong, I've never done something like this.

Thanks!

----

The easiest way to do this kind of template thing is to put your template General/NSView in a separate nib and just load the nib every time you need a new view. The new General/NSNib class can facilitate this sort of thing. The other way to "copy" a view is to pass it through an archiver, like so:
