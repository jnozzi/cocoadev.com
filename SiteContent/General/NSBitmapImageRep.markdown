...
* Does <code>[[[NSGraphicsContext]] saveGraphicsState]; [[[NSGraphicsContext]] setCurrentContext:newContext];</code>
...
* <code>[[[NSGraphicsContext]] restoreGraphicsState];</code>
...
This way is performant ( and ONLY AVAILABLE in 10.6 and above! ): <code>[[[[NSBitmapImageRep]] alloc] initWithCGImage:[image [[CGImageForProposedRect]]:context:hints:]]</code>.  It will not allocate a new image buffer unless the image was PDF backed or something like that that is incompatible with a bitmap.  
...
Eh. I think I'll use <code>[[[[NSBitmapImageRep]] alloc] initWithData:[image [[TIFFRepresentation]]]]</code>.
