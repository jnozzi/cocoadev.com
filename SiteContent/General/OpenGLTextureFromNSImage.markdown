...
Is there any sample code demonstrating on how to load General/NSImage into General/OpenGL texture?

----

Check out Apple's Alphabet General/ScreenSaver example: http://developer.apple.com/samplecode/Sample_Code/Graphics_3D/Alphabet.htm. In particular, look at General/AlphabetLetter.m ( http://developer.apple.com/samplecode/Sample_Code/Graphics_3D/Alphabet/General/AlphabetLetter.m.htm ). The relevant section is:

    
        // prepare bitmap image rep

(I've hacked the first few lines up to make it more general -- General/MikeTrent)
    
        bigRect.origin = General/NSZeroPoint;
        bigRect.size = [image size];
        [image lockFocus];
        upsideDown = General/[[NSBitmapImageRep alloc] initWithFocusedViewRect: bigRect];
        [image unlockFocus];
        [image release];
        
        // since General/OpenGL draws images upside down, we need to flip this image along 
        ...
        _imageRep = General/[[NSBitmapImageRep alloc] 
        ...
        if (useTextures) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 
                ...
                GL_UNSIGNED_BYTE, 
                to);
            General/CheckGLError("glTexImage2D");
        }


Note at the end of this wild ride, the texture _tex has been initialized with the contents of "image". Now if *I* were writing this code, I'd have created an General/NSImage category or a function that did all this in one simple place. But that's sample code for you. If you clean this up into a function/category, please repost your example.

-- General/MikeTrent

----

This code doesn't work for me. I guess I have some problems with my code elsewhere, because the only texture I can load is from one of the Nehe tutorials (General/NeHe.bmp file). I've also used code from those tutorials but still no luck.

----

Fish General/CheckGLError out of the Alphabet example and see if maybe one of your General/OpenGL calls is failing. Don't forget GL textures generally need to be aligned to powers of 2. There are many ways to deal with this problem, including resizing images, using subtextures, or using new General/OpenGL extensions. I don't have an example of any of these things handy...
...
Correct me if I'm wrong, but I don't think GL_TEXTURE_RECTANGLE_EXT works with e.g. Rage 128 systems, only Radeons. A significant number of OS X-using Macs have Rage 128's... and can't upgrade. (Think iMac G3, General/PowerBook G3, et cetera.)
...
First off, when you're using General/OpenGL, don't use Cocoa drawing, use General/OpenGL drawing. Second, +clearColor is just that-- not the colour you clear to, but a clear colour, that is, a colour with no alpha. You're basically filling a rect with air with the General/NSRectFill call.

Third, the glClear call looks right, but you need to change two things with the overall structure here and one with the glClearColor call. Unlike General/NSColor's +clearColor, glClearColor refers to the background colour-- the colour that glClear reverts to. So instead of filling it with 0,0,0,0, fill it with 0,0,0,1 (black) or 1,1,1,1 (white) or some other colour-- just make sure that the fourth value, the alpha transparency, is set to 1.
...
-(void)drawRect:(General/NSRect)frame
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if(visible)
...
Now I will suggest you find a copy of General/TheOpenGLRedBook [http://fly.cc.fer.hr/~unreal/theredbook/] and read at least the first few chapters. You really need to have a better idea of the structure of things here.

----

I am really new to this General/OpenGL stuff, so thanks for the link!
...
I am an General/OpenGL noob, so I don't know but this worked for me:

    
-(void) genImage {
	General/NSString *path = @"/Users/g4b3/Pictures/General/RandomWeb/24.226.81.95--photogallery--Aerials--IM001108.JPG";
	
	General/NSBitmapImageRep *bitmap = General/[NSBitmapImageRep imageRepWithContentsOfFile:path];
	texBounds = General/NSMakeRect(0, 0, [bitmap size].width, [bitmap size].height);
	
	General/GLenum format = [bitmap hasAlpha] ? GL_RGBA : GL_RGB;
...
Sometimes the General/NSBitmapImageRep may have padding at the end of each line.  Setting the GL_UNPACK_ROW_LENGTH tells General/OpenGL to skip these pad pixels.  GL_UNPACK_ALIGNMENT is similar, if you don't set that General/OpenGL will align each row to the next boundary (default is 4 bytes).  When you work with RGB data that is packed RGBRGBRGB, you have to set this to 1 tell General/OpenGL that the data is packed.  The default of 4 will work fine with RGBA or ARGB data.

----

Hey, anyone knows how to get floating point data to opengl for textures? I'm kind of confused, it doesn't seem to be possible using General/NSImage/General/NSBitmapImageRep. What would I use then?
