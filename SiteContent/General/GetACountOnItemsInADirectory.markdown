Is there a faster way to get the count of items in a directory than <code>[[[[[NSFileManager]] defaultManager] directoryContentsAtPath:aPath] count];</code> ? Because I'm not interested in the items but only in the amount of items in that folder.
