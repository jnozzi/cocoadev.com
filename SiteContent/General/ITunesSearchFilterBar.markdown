...
I'm trying to create the search filter bar that exists in both iTunes and Mail.  I guess it can be done with an General/NSView with a background gradient, along with some custom General/NSButtons, but I was wondering if anyone knew of an easier/different way to accomplish this.  Even a tutorial or example somewhere online would be great (I searched around google and cocoadev for a while, finding nothing).  Thanks! http://goo.gl/General/OeSCu
...
Well, you don't have to make custom buttons if you're using Tiger or newer. In Tiger, General/NSButton.h adds to the General/NSBezelStyle enum two new values: General/NSRoundRectBezelStyle and General/NSRecessedBezelStyle. I forget which is which, but neither are available from General/InterfaceBuilder and one of them draws those buttons.

Other than that, yeah, you're probably going to have to build it yourself. Apple provides no classes to do it for you (General/NSMatrix might help though, but I doubt it.) -General/JonathanGrynspan

----

Alright, so I've started to get this going... I've got an General/NSView with a custom background image (just a screenshot of the iTunes gradient), and I've got some buttons set up on top of it.  However, when the button isn't selected, the striped background of the window shown through, and not the General/NSView's background.  I like how the button doesn't show any border when it isn't selected, and it actually doesn't change if the mouse hovers over (if I wanted to really replicate the original iTunes/Mail button, this would happen, but I haven't gotten that far yet, and I don't think I really mind it if I never impliment that).  Any idea why this is happening?  Thanks.

http://www.burgundylogan.com/media/General/FilterBarButtonBackground.jpg

Here is how I currently set up any given button...
    
General/NSButton *newButton = General/[[NSButton alloc] init];
...
Try General/newButton cell] setBackgroundColor: [[[NSColor clearColor]];
...
General/NSImage *backgroundImage = General/[NSImage imageNamed:@"General/FilterBarBackground"];
[backgroundImage compositeToPoint:General/NSMakePoint(0,0) operation:General/NSCompositeCopy];
...
Have you seen the General/AMRollOverButton available from harmless.de? http://www.harmless.de/cocoa.html
...
The General/AMRollOverButton has been available for several years - well before Tiger. Perhaps you've misunderstood the documentation?

----

No, I got that General/AMRollOverButton is a possibility.  I was talking about the General/NSRecessedBezelStyle, which is an General/NSBezel than can be applied to General/NSButtons.  This is what I'm using in the above pictures.  However, General/NSRecessedBezelStyle is not available before 10.4, and just shows up as text, with no button background, but the button's actions still work.  This is why General/AMRollOverButton is probably a good idea, since it would work on earlier versions.
...
----
I think you want the source-over compositing operation, not copy. --General/BenStiglitz

----
Logan, did you find a solution ? I have the same issue with General/NSReccessedStyle button's. Tried everything: setting cell background image, cell background color, subclassing -drawInteriorWithFrame, -drawWithFrame, � no sucess.
I can't believe this has not been solved by someone.

----

Did you try subclassing General/NSButton and overriding -isOpaque to always return NO? -General/JonathanGrynspan

----

Have you tried setting the color to nil?  I did this with General/NSBrowserCell which was overdrawing on my nice gradient (thanks Shipley) and it worked fine.
    
- (General/NSColor *)highlightColorInView:(General/NSView *)controlView
{
	return nil;//General/[NSColor clearColor];
}

Course that's the wrong class but it should work for you too.

*It's also possible the black is being caused by the button thinking that it     -isOpaque -- General/JediKnil*
...
I started a new project.  downloaded the school planner app, opened up its mainMenu.nib, and dragged the General/FilterBar view from its window over to my window.  Then I closed the old project.  So, logan, it's the same name at the top level so you can just drag my stuff over there if you want.
...
filterbar (an General/NSView), 
it's button General/SDFilterBarButton (an General/NSButton) 
and its cell General/SDFilterBarButtonCell (an General/NSButtonCell)
...
I just got some email help from Baron Stephane and I was able to get the background coloring to work to a certain degree.  The General/FilterBar now looks like this:

http://www.burgundylogan.com/media/FilterBar1.1Coloring.jpg

I'm happy enough with it to use it in the program, but there are a few quirks.  For example, the original iTunes bar and General/NSRecessedBezel have a different image for hover and click, whereas this General/FilterBar doesn't.  Also, when you click any button, there's a quick blink between the hover and the click image.  I'm not sure if these things are fixable.  Anyways, you can give it a try here.  Thanks!
...
Just wanted to add that I recently created my own implementation, called General/AMButtonBar:
...
I'll add one more alternative to the mix.  Mine's called General/SRScopeBar, uses multiple groups, labels, separators, automatic resizing, collapsing, an overflow menu, an accessory view, etc and it comes with an IB3 plugin and is bindings compatible.  Check it out at http://www.flytocode.com
