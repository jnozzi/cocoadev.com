...
I want the user to enter an expression in a field and then, this expression can be calculated and the result be returned. http://goo.gl/General/OeSCu

For example, you enter "4 + 5" into a General/NSTextField, and the stringValue is converted into a real expression that returns 9.
...
The following example just needs two General/NSTextFields and a controller object that responds to one action (    inputAction:). This invokes the command-line tool *bc*, at /usr/bin/bc. There is related material at General/BCWrapperSource, and more at General/HandlingLargeNumbers.

**General/ControllerObject.h**

    
@interface General/ControllerObject : General/NSObject {

    General/IBOutlet General/NSTextField *inputField, *outputField;
    General/NSTask *bcTask;
    General/NSFileHandle *inputHandle, *outputHandle;

}
- (General/IBAction)inputAction:(id)sender;
- (void)bcTaskOutput:(id)note;
@end


**General/ControllerObject.m**

    
@implementation General/ControllerObject 
...
- (void)bcTaskOutput:(id)note {
    ...
    if (object == outputHandle) {
        General/NSData *data = [outputHandle availableData];
        General/NSString *output = General/[NSString stringWithCString:[data bytes] length:[data length]];
...
- (General/IBAction)inputAction:(id)sender {
    General/NSString *input = [sender stringValue];
    General/NSString *inputString = General/[NSString stringWithFormat:@"%@\n", input];
    General/NSData *inputData = General/[NSData dataWithBytes:[inputString cString] 
                                        length:[inputString length]];
    if (inputData) {
        if (bcTask) {[bcTask terminate]; [bcTask release]; bcTask = nil;}
        bcTask = General/[[NSTask alloc] init];
        General/NSPipe *inputPipe = General/[NSPipe pipe];
        inputHandle = [inputPipe fileHandleForWriting];
        General/NSPipe *outputPipe = General/[NSPipe pipe];
...
There is an example in General/BookBuildingCocoaApplications that deals with this kind of thing.
...
This is what bugs me in Cocoa, to do something that simple you gotta write a bunch of code... If maybe the guys at Apple were a little bit smarter they should really try implementing the method Evaluate, which I have seen and used a lot in General/JavaScript, General/ActionScript, Lingo and some few others.
It could be something like:

 [aNSTextField evaluateValue:General/[NSTextField string] as:General/NSMathematicalExpression];
...
Or use an interpreted language like General/JavaScript, General/ActionScript, Lingo, Perl, Ruby, Python, Tcl, shell script, etc.

----

Actually you do have your "evaluate" method in Cocoa, it comes in the form of General/NSAppleScript.

----

That is some sexy code up there, thanks to whomever passed it along.  It is also a nice introduction to General/NSTask and General/NSPipe.
...
be:
General/NSString *input = [inputField stringValue];

? Sender would be the button that generated the action method- taking the stringValue would return, well I'm not sure..

----

If there was a button,     [sender stringValue] would return the title of the button ("OK" or whatever). If this is the General/NSTextField's action (triggered by hitting enter in the text field) sender will be the text field.
...
There used to be an General/NSInputMnager sample that did this.
There used to be anNSFormatter example that did this.
*

And the code above is nice, but all it does is this
    
- (General/IBAction)takeExpressionStringFrom:(id)sender
{
   char     answer[PATH_MAX];
   FILE  *fd = popen("bc -i -q", "r+");
   fputs(General/sender stringValue] cString], fd);
   fputs("\n", fd);
...
I'm writing a custom General/NSFormatter.  I use isPartialStringValid for key parsing.  However, I need to know when the user has committed (i.e., pressed enter, etc) to do final validation (for example, I accept fractions, and need to know the "2/" is invalid vs "2/4").   I would only return an object from getObjectValue after final validation is completed.

----

I would think that this is part of the purpose of     -General/[NSFormatter getObjectValue:forString:errorDescription:].  If the string cannot be transformed into an object (because it's invalid), return     NO and set the error description string.