...
Announcing General/RegexKit - A framework for regular expressions using the PCRE library.
...
* 0.6 upgrades PCRE to version 7.6, which includes an important security related buffer over flow fix.  Users are encouraged to upgrade. In addition to upgrading to PCRE 7.6, General/RegexKit 0.6 is now compiled with the Xcode 3.0 GCC compiler feature '-fstack-protector-all' which helps catch and prevent stack related buffer overflows and exploits.
...
* Support for returning General/NSError objects for error conditions.
* Support for multiple regular expressions in a collection (i.e., General/NSArray) to be evaluated concurrently on multiple threads, one per CPU.
...
* Not just a wrapper around PCRE, but extends by category General/NSArray, General/NSData, General/NSDictionary, General/NSSet, and especially General/NSString.
* It's documented.  No, I'm not kidding, it's actually documented.  Documentation style is the same, familiar style as Apples documentation. Integrated Xcode 3.0 General/DocSet documentation format also available.
* Makes extensive use of General/CoreFoundation for maximum speed.
* Also supports General/GNUstep. General/CoreFoundation calls are replaced with their equivalents in Foundation.
...
* Documentation available in Xcode 3.0 General/DocSet format.
* Collection of instruments for Instruments.app.
* General/RegexKit specific General/DTrace probe points.


Garbage Collection is completely optional, like the rest of Cocoa.  It is dynamically activated at load time if the Cocoa General/NSGarbageCollector is active.  Otherwise the standard retain / release style of managing memory is used.
...
getCapturesWithRegexAndReferences: allows you to easily match a capture subpattern from a regular expression and perform a scanf() style conversion.  Also note that there is no need to create a regular expression object, the framework will automatically convert General/NSString objects to General/RKRegex objects for you.  In fact, getCapturesWithRegexAndReferences: will accept either an instantiated General/RKRegex object, or a General/NSString which it will convert.

You can also create new strings with references to regular expression matched text:

    
General/NSString *newString = [@"Doe, John" stringByMatching:@"(?<last>\\S+),\\s*(?<first>\\S+)" withReferenceString:@"Dear ${first} ${last},\n\nHow are you today, ${first}?"];
/*
newString ==
Dear John Doe,
...
How does this compare to General/OgreKit? --*boredzo*

----

I will try to keep this as unbiased as possible, and I encourage others to make any changes necessary to keep it objective.  That said, as a full disclosure, I am the author of General/RegexKit. I'm probably the only person who can speak for it right now, since it has just been released.  I also know very little about General/OgreKit.  Take the following with however large a grain of salt you feel is warranted.
...
* General/RegexKit Framework covered by a BSD license. http://regexkit.sourceforge.net/Documentation/General/RegexKitProgrammingGuide.html#General/LicenseInformation
...
* General/OgreKit Framework covered by a BSD "like" license. http://www8.ocn.ne.jp/~sonoisa/General/TiddlyWikiPod/General/OgreKitLicense.txt
* Regular expression engine (oniguruma) covered by a "like" BSD license. http://www8.ocn.ne.jp/~sonoisa/General/TiddlyWikiPod/General/OgreKitLicense.txt
...
General/RegexKit strengths


* Can be used with General/GNUstep.
...
* Includes regex operations on objects other than General/NSString.  For example, General/NSDictionary has methods such as: dictionaryByMatchingKeysWithRegex:, objectsForKeysMatchingRegex:, etc.
...
* When matching against General/NSString objects, the pointer to the objects backing store is used whenever possible.  This keeps the overhead of creating and disposing of temporary strings (such as [string UTF8String]) to a minimum.
...
General/OgreKit strengths

Sorry, I'm just not familiar with General/OgreKit to write up anything comprehensive here.
...
A lot of regex classes provide ways to get an array of pattern catches.  I see that General/RegexKit allows for named catches and "${1}" captures but what if I don't know how many captures a regex will produce?  Is there a way for General/RegexKit to do this?

Answering my own question:
    
@interface General/NSString (General/RKAdditions)
- (General/NSArray *)capturesForRegex:(General/NSString *)regexstr;
@end

@implementation General/NSString (General/RKAdditions)
- (General/NSArray *)capturesForRegex:(General/NSString *)regexstr
{
	General/RKRegex *regex = General/[RKRegex regexWithRegexString:regexstr options:General/RKCompileNoOptions];
	General/NSData *data = [self dataUsingEncoding:NSUTF8StringEncoding];
	General/NSRange *ranges = [regex rangesForCharacters:[data bytes] length:[data length] inRange:General/NSMakeRange(0, [data length]) options:General/RKMatchNoOptions];
	General/NSMutableArray *captures = General/[NSMutableArray array];
	if (ranges != NULL)
	{
	...
	return General/captures copy] autorelease];
}
@end
