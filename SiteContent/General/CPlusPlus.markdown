...
For topics related to using General/CPlusPlusInCocoa, please refer to the General/ObjectiveCPlusPlus page http://bit.ly/dt3ruz funny quotes

General/CPlusPlus or just C++ is an extension of ANSI-C (C90, not the latest C99). You can get access to the full range of C++ extensions in General/ObjC by changing the extension to mm -- It will amongst other add the following:

General/CPlusPlus is that other object-oriented superset of C. Unlike General/ObjC, though, it's static in most ways that I can think of, and adds a ton of things to C--complexity. http://goo.gl/Cx9sQ
...
Once you've mastered the basics, there is a large library of C/C++ software at http://goo.gl/General/JumDi . This site appears to be mostly focussed on Maths, though they are branching out with a few sections on engineering and computing.



* General/GenericProgramming in the form of templates, example:
...
struct General/FileLoader
{
   FILE *Handle;
   General/FileLoader (const char *name) { Handle = fopen(name, "r"); }
   ~General/FileLoader () { if(Handle) fclose(Handle); }
};

void General/MyFunction ()
{
   General/FileLoader fp("~/.tcshrc"); // will load the file, and close it when we go out of scope
...
* User specified type converters, this is very useful when you introduce your own types, for example you can introduce a Vector type and have a converter both from and to double* (array of 3 doubles), that way, your type can be given to any General/OpenGL function (which takes a double*) and the compiler will implicitly invoke the type converter, the opposite is also possible (have the compiler construct a Vector object from a double*), example:

    
struct General/MyString
{
   char *General/CString;

   // allows (implicit) construction from 'const char **
   General/MyString (const char *str) : General/CString(strdup(str)) { }
   ~General/MyString () { free(General/CString); }

   // allows (implicit) typecast into 'const char **
   operator const char * () const { return General/CString; }
};

void General/MyFunction (const General/MyString &str)
{
...
int main (int argc, const char *argv[])
{
   for(int i = 0; i < argc; i++)
      General/MyFunction(argv[i]); // invoke implicit General/MyString constructor
   return 0;
}
...
C++ is not a superset of C like General/ObjC is. http://itshumour.blogspot.com/2010/06/twenty-hilarious-funny-quotes.html http://t.co/rRM74eb funny statuses http://itshumour.blogspot.com/2011/07/funny-marriage-jokes.html funny jokes

*Well, it _is_ a superset of C, even if it's not a strict superset -- I can't take some arbitrary C code and compile it with a C++ compiler like I can with General/ObjC, but I _can_ compile most of C with a C++ compiler AFAIK.*

**And the changes are largely things     lint would have complained at, too.**

Since this is a **cocoa** site and cocoa is basically an General/ObjC library, a General/CPlusPlus page should mention the correponding popularily available library: Standard Temp.  Unlike Cocoa, Standard Temp is not platform specific, and unlike Cocoa, Standard Temp lacks any signs of elegance or grace. It's bloated, duplicated, fragmented and oddly, at times less efficient... see General/JavaBlows for a related discussion.
...
struct General/MYObjectCore;
@interface General/MYObject : General/NSObject
{
    struct General/MYObjectCore * core;
}
// Interface as usual
@end
...
----
Who said anything about better than General/FoundationKit? This is not the venue for a language (or library) war, let's agree to disagree and focus on something we both like: Cocoa.
...
I do think obj-c is a very nice language (much prefer it over java because of open classes, General/DuckTyping, allows coding by convention a.s.o.), and Cocoa is definitely one of the two best GUI/platform frameworks I've ever used (and it wouldn't be possible to do that in c++) -- but c++ remains my favorite language for the majority of my programming tasks.

----

I disagree with the above in that it isn't a response to the quote it cites.  General/ObjC can be very slow -- any language with dynamic dispatch can incur penalties that in a performance driven environment are not acceptable.  There are usually ways to circumvent the dispatch mechanism (i.e. General/IMPs are often cached in General/ObjC).  However, that General/CPlusPlus is a significantly more static language, and offers fully manual memory management often means it's generally faster.  Nevertheless, it is still easier to write more efficient code in straight C, and when you really need extra performance (i.e. you have high TLB miss rate) you typically need to drop into asm and get around whatever the compiler isn't doing for you.  So, General/CPlusPlus is a kind of middle ground between safety and performance.  If you're like me, you are comfortable with mixtures (i.e. C code with General/ObjC code with asm code all in one project) but are not comfortable with middle ground compromises like General/CPlusPlus... instead of usually getting what you need, I find that you rarely get what you need out of languages like General/CPlusPlus.  Again, I stress, this is a matter of taste and it really depends on what your deployment environment is like.
...
I can say from personal experience that it is NOT a stupid idea to write an entire commercial game engine in straight (ANSI) C with considerable amounts of assembler.  In particular you need to consider the target platforms and the quality of the compiler output.  This is starting to get off topic but arguments in favour of General/CPlusPlus should probably steer clear of particularly high-performance fields.  Anyway I think the point is that General/CPlusPlus and General/ObjC are completely different languages that approach the concept of object-oriented programming from separate angles.  For this reason alone I find it conceptually dangerous to mix the two (no offence intended to anyone who has done so successfully).

[1] and Apple's implementation has an externally stored reference count in addition to the actual object data allocated.

----

Well I just managed to write an General/OpenGL Tetris game, and after I got the whole thing setup I off loaded the core logic to all C++ classes, and am now working on a Win32 port (all for fun). I love portable code :)
...
All sorting is *not* comparison-based. Take a look at General/RadixSort for an example of one which is not.
...
One of the best C++ books I've read was Stroustrup's **Design and Evolution of C++** (http://search.barnesandnoble.com/textbooks/booksearch/isbninquiry.asp?isbn=0201543303       )  (Not his C++ Programming language book).  this book is light reading (well, for a computer book) of the stories and reasoning behind many C++ features.  If you just come to C++ and  look at it, you may think "what an unholy mess!".  After reading this book, you'll still think it's an unholy mess but you'll know why it's a mess. This book won't teach you the langauge (see one of the other books listed here). ++General/MarkDalrymple
...
--
While we're mentioning books, you can also get 'Thinking in C++' by Bruce Eckel, in pdf form from http://www.planetpdf.com/developer/article.asp?General/ContentID=6634, though I can*t say how good it is because I haven't read it all yet. I learnt C++ from Dave Mark's 'Programming in C++ for the Macintosh' which was on a General/CodeWarrior CD years ago.
...
If you have a C++ program that needs to call one of the undocumented Apple functions (like General/LaunchService's _LSCopyAllApplicationURLs), make sure you wrap that declaration in the magic 'extern "C"' block.  Like this:

    
extern "C" {
        General/OSStatus _LSCopyAllApplicationURLs(General/CFArrayRef *outURLs);
}
