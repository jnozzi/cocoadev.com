I'm interested in knowing how [[NSArray]] works behind the scenes. Is it a linked list? If so, wouldn't <code>objectAtIndex:</code> have to iterate through the list n times, where n is the index, meaning it's not as efficient and fast as a normal array?
...
''Oops, my bad, I was mistaken. However, unlike Foundation/CF containers, STL's restrictions are so tight as to virtually guarantee one particular implementation, even if it's not specified. For example, <code>set</code> and <code>map</code> both require a comparison operator but not a hash operator, guaranteeing that it will use a tree of some kind, and not a hash table. On the other hand, I assume that <code>hash_set</code> and <code>hash_map</code> guarantee constant-time access (I couldn't find the behavior guarantees for these), making it a requirement to use a hash table. Foundation's collections are looser on the guarantees which makes it less clear what kind of implementation it uses.''

'''<code>hash_set</code> and <code>hash_map</code> are not STL yet, I believe.'''
