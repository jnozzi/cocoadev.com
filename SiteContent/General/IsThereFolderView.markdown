I couldn't find any documentation on General/FolderView. Is it safe to assume there is no official documentation about it? Can anyone offer any suggestions on how it should be implemented? 
Also, i was wondering if it was necessary go gather all the data before I try to lay out something using General/NSOutlineView. Anyone have any experience in this area?
----

General/FolderView? What is it? If you want to show a filesystem hierarchy in a General/NSOutlineView look at /Developer/Examples/General/AppKit/General/OutlineView/
...
subclass General/NSControl.  we'll call it  General/NSFolderView
subclass actionCell, call this Guy General/NSFolderCell
...
override the cell creation methods and Subclass General/NSObject to create a Data Source object  (read and read and reread the table View class spec)
...
that way, your local datasource is the only object that needs to know how to get to the database, and it only gives values (probably as General/NSNumbers or General/NSStrings) away.

----

You may want to check out General/UKDistributedView, I've never tried it, but I think it does what you are looking for: http://www.zathras.de/programming/cocoa_stuff.php