Just started using General/XCode version 3.0. The first clean build I attempted on a project I got a couple of warnings about General/NSString constants.

**warning: input conversion stopped due to an input byte that does not belong to the input codeset UTF-8**

The culprit was a format string that has a degrees symbol **Angle (%i�)**. What is the proper way to create an General/NSString constant that has UTF16 characters? A quick way to kill this particular warning was to just sub the UTF16 character with a **%C** format entry.

    
General/NSString *angleString = General/[NSString stringWithFormat:@"Angle (%i%C)", angle, 0xb0];
...
The warning says that an input byte does not belong to the "input codeset UTF-8". The UTF-16 character 0xb0 resolves to the two byte sequence {0xc2, 0xb0} in UTF-8 encoding. One thing I didn't mention is that the degree symbol pasted into the source code is being printed/drawn correctly in my Cocoa app. Since I have General/XCode set to save files in the default UTF-8 encoding the symbol character is being written to disc as {0xc2, 0xb0}. This warning was not being generated in General/XCode versions before 3.0. 

----
Regardless of all that, it is not permitted to use non-ASCII characters in General/NSString constants, so the rest is somewhat academic.