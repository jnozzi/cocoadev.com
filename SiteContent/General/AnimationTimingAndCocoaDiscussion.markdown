*My parts in this discussion are those of a person who was just beginning to learn to code; view this discussion as a learning-experience-in-progress from several years ago. -  General/JoeOsborn*

Most of this page references a former revision of an article which suggested many poor programming practices.  Mike and others were very right- General/NSTimer is perfectly suitable for games. I cannot recommend any approach to animation that requires while() loops.  The article has since been fixed.-- General/JoeOsborn

----

OK. My complaint with this code example is the use of an unmetered "while (1)". When you watch this program in top(1) you will see it consuming 80-100% of the CPU in order to do a mere 30fps animation. In general, this means the program is taking time away from other processes that could be using it (though not starving them, thanks to the kernel). In specific, on my General/TiBook 667 this means the fan is about to kick from "off" to "hyper overdrive" and my battery life is about to drop through the floor.

This is the hallmark of a poorly written Mac OS X program, most commonly seen in poor Carbon ports.

The good thing about General/NSTimer is your code will block on a kernel trap waiting for the right time to execute. When that time arrives, your program wakes up, does the work, and goes back to sleep again. Personally I disbelieve the arguments against General/NSTimer. I've used them to get reasonably good performance for hundreds of iterations per second. But, General/NSTimer isn't the only fish in the sea.
...
Rather than using General/NSTimer, you could also use performSelector:withObject:afterDelay: to string together animation methods. Adjust the "after delay" method each time to account for the time spent in your prepareForNextFrame.
...
Finally, why mess around with General/CoreServices on a Cocoa web site? You can get this value from the General/NSDate API:
...
But again, I have a hard time believing General/NSTimer is all that bad -- it's certainly a **lot** better than melting people's laptops into slag. Code responsibly.

-- ("Mr Grumpy") General/MikeTrent
...
ps- An honest question since I can't understand the phrasing: What does this mean? "Because of the various input sources a typical run loop manages, the effective resolution of the time interval for an General/NSTimer is limited to on the order of 50-100 milliseconds." http://developer.apple.com/techpubs/macosx/Cocoa/TasksAndConcepts/ProgrammingTopics/Timers/Concepts/timerobjects.html  'resolution' is the word i'm struggling with.

----

It refers to the possible frequencies you can achieve. What it means, I guess, is that if you have an General/NSTimer set to fire every 25 milliseconds, it's not gonna fire as often as you'd like it to. Only about half that much at best, in fact.

Myself, I've been using General/MacOSX's GLUT implementation, which for interest's sake, is implemented on top of Cocoa, and other than the bug I discovered in glutTimerFunc() (anybody interested? :) ), I've found it performs admirably. And yes, it's using General/NSTimer in there. So I guess this is a YMMV issue; at the moment, my GLUT app is a lightweight, so I guess it might be referring more to apps like General/OmniWeb. With the amount of stuff its run loops have to handle, I'd not be surprised if their timers couldn't be very fine-grained. -- General/RobRix

----

I reimplemented all my animation using General/NSDate.  It's better, and the user of General/OSAnimationTimer only needs to call two methods ever(one to initialize, and one to sleepThreadUntilNextFrameDate).  The class is still useful, if only to refactor some common timing code out of a lot of different classes.  The examples above are changed too.  As usual, the Cocoa way is better. ^_^  It is a bit slower(the numbers I've been giving as framerates to the -init methods have to be doubled...), but it's much more pleasant to use.

I'll check out General/CocoaGL and GLUT and whatnot sometime...
...
But, while this kind of technique avoids using General/NSTimer, it also avoids going through the Cocoa run loop. Your program will be spending all of its time drawing and none of its time processing key strokes, listening for power off notifications, receiving apple events, noticing the user has resized the monitor, certain sound operations and so on. You're going to need to solve this problem.

This is the main reason why I advocate using General/NSTimer. It works with the General/NSRunLoop so you can write a normal Cocoa app (not Carbon, not GLUT) with some kind of animation. The other techniques above that work with General/NSRunLoop also integrate better into a Cocoa app.
...
My game doesn't animate 24/7, it only animates when a character moves- though I have noticed the tendency for keystrokes to line up... But it's not a bug, it's a feature! Type in a bunch of arrow keys and sit back as you get to where you're going.... eheh... Yeah, that's less than ideal.  But to achieve 32 frames per second, I'd need a timer that fires with an General/NSTimeInterval of .03125, and I've never been successful in pulling off a timer like that(Plus it does look pretty ugly to have     myAnimationMethod:(NSTimer*)extraneousArgument methods).  I tried the multithreading bit a couple of days ago, that didn't work out at all.  I don't know from multithreading. (I'm enjoying this discussion, though, this is what I live for. ^_^)

--General/JoeOsborn

----

I did some General/OpenGL work using General/NSTimer for a class at my university. I found that General/NSTimer seemed to perform a lot better outside of the main thread. The General/NSRunLoop associated with that thread didn't seem to have nearly as much to deal with, and ran a lot faster. Has anyone tried sampling the resolution/sec that an General/NSTimer in a thread that ISN'T controlling the General/AppKit has? I've seen 3d apps that do this correctly (that tweakwaves demo is a good example) so there HAS to be a reasonable solution to this problem out there.
...
It's important to note when you do this, that there can be many problems with this approach, especially if you plan to do anything involved with General/OpenGL or General/AppKit. General/OpenGL ignores threading, so just access it from one thread and you'll be fine (you might want to put a lock around acquiring the conext just to be safe). Earlier Kritter put up an efficient locking design that would be useful here.
...
"...while this kind of technique avoids using General/NSTimer, it also avoids going through the Cocoa run loop. Your program will be spending all of its time drawing and none of its time processing key strokes, listening for power off notifications, receiving apple events, noticing the user has resized the monitor, certain sound operations and so on. You're going to need to solve this problem.

This is the main reason why I advocate using General/NSTimer. It works with the General/NSRunLoop? so you can write a normal Cocoa app (not Carbon, not GLUT) with some kind of animation. The other techniques [detailed in General/AnimationTimingAndCocoaDiscussion] that work with General/NSRunLoop? also integrate better into a Cocoa app.

Of course, you could always poll your run loop 30-60 times a second within your animation loop, but now we're back to that whole "poor application design" problem. Or you could use Carbon or GLUT event processing functions, but neither of these are really Cocoa.

You could run your animation loop on a second thread, while the main thread handles all these mundane tasks. But even then, the second thread will want its own run loop. And now you have to worry about concurrency issues."--General/MikeTrent(full quote in General/AnimationTimingAndCocoaDiscussion)

(General/NSjoie note: the above code is capable of using General/NSRunloop; a project I'm working on(the code from which I modified for these examples) uses General/NSRunloop for input with the normal -keyDown: mechanism.)

----

It looks like Apple is making some efforts in this area.... See General/NSAnimation and General/NSViewAnimation, both added in 10.4 .... 
...
In a separate thread:
 ...
 while(1) // Don't get angry!
 {
     ...
     if(now - _lastTime < _minInterval)
     {
         ...
         _lastTime = now;
         NSAutoreleasePool * arp = General/NSAutoreleasePool alloc] init];
