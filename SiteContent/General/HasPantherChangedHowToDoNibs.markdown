...
The panel is controlled by a class called Controller, which is instantiated in General/MainMenu.nib. This class, once it gets the data, does some parsing, error checking, etc. Then it needs to either send this data to the document and its network of subclasses, or get a data field from the document. The thing is, I'm not sure how to do this. Controller is instantiated in General/MainMenu.nib (this is document-based architecture), and General/MyDocument has its own nib. I don't see how I can get an outlet between Controller and General/MyDocument, since if I instantiated a General/MyDocument object in General/MainMenu.nib, wouldn't it be a different General/MyDocument that what the user is seeing? Similarly, wouldn't instantiating a Controller in the General/MyDocument.nib make a different Controller object than the one the data was given to?
...
There are a couple of good pages here on the subject - just search for General/ResponderChain or General/MakingNibsTalkToEachOther

----
I read the two links, but I still have a bit of a gap in my understanding. I set it up so that the panel can trigger an action in General/MyDocument by creating an action in the General/FirstResponder in the panel's nib and General/MyDocument.nib. The thing is, I still don't understand how to send data gathered by the panel (an General/NSTextField) and worked with in the Controller class to General/MyDocument. General/FirstResponder can't do outlets, so how do the panel and General/MyDocument share data? Or am I missing something really obvious?
...
(You may not be able to put a method that is not an General/IBAction into your responder chain. I've never tested this - so I don't really know.)
...
It seems to me that if you can access a method in General/MyDocument from your panel controller, if that method has an argument, you can
pass the data you need in that argument. But if you can only invoke an General/IBAction (which has an argument (id) sender, then
...
There is an example of this in General/BookBuildingCocoaApplications.
...
I think that is where your misunderstanding of the chain is. Quoting from General/MakingNibsTalkToEachOther:
*Another method would be to use the General/FirstResponder. To do this, first create the action you want to be called in the General/NSDocument subclass in General/MyDocument.nib. Then double click the General/FirstResponder in Properties.nib. Add the action with the same name to the General/FirstResponder. When calling the General/FirstResponder's action, it will go down the General/ResponderChain and call the first object that replies to that method. In other words, the currently active document. Note: be careful using common method names for this - you don't want another object to respond to the method before it gets to the document.*
...
*It should also be brought up, that if the panel is in the General/MyDocument.nib file, a new panel will be created for every document. Is this the desired functionality? If not, then it will need to be in a separate nib file.*

So if you just add a method to the panel controller that accesses the data in the panel's text field and put it in the responder chain in General/FirstResponder, you should be able to use it in the document class, with the caveat mentioned above.

----

Ahhh, so I'm thinking of it the wrong way. I can link the main menu option to the General/FirstResponder object in General/MainMenu.nib, then have General/MyDocument carry the action method to open the Panel and create Controller in General/MyDocument when needed. I think I got it. Thanks a lot! Couldn't have done it without you.
...
(Congratulations on decyphering General/NSResponder!, BTW. That's the right way to go. Very appropriate.)