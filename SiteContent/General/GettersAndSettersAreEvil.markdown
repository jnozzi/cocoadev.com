...
In a sister-article to General/ExtendsIsEvil, Allen Holub [http://www.holub.com/] explains why getter and setter methods are evil, too:

http://www.javaworld.com/javaworld/jw-09-2003/jw-0905-toolbox.html
http://www.javaworld.com/javaworld/jw-01-2004/jw-0102-toolbox.html (a follow-up, with design guidelines)

Something to consider while playing around with General/KeyValueCoding, and General/JavaBeans.

I hope this will be an equally interesting discussion as General/ExtendsIsEvil. The introduction to the article also further explains some of the controvercies in General/ExtendsIsEvil.

--General/TheoHultberg/Iconara

*Also discussed at: http://c2.com/cgi/wiki?General/AccessorsAreEvil *

And also mentioned in the interview with General/BjarneStroustrup, although his conclusion may differ from that of the Java world: http://www.artima.com/intv/goldilocks2.html
...
General/ChrisEccles :  Hear, Hear !  There is no point in subclassing if all you want is a repository for data.  Long live the General/NSMutableDictionary.
...
Now, I understand the argument being made, and it's certainly got merit in certain circumstances, but a blanket statement like "Is Evil" is like saying there's only General/OneTrueWay to indent code. As far as I'm concerned, anybody who makes such blanket statements is an ignoramus, regardless of his or her credentials. And now, you can call *me* an ignoramus, for making a blanket statement about ignorami.

--General/ShamylZakariya

*General/ShamylZakariya: You may be a misunderstanding of Allen Holub point. He is not suggesting you access instance variables directly in place of accessor methods - direct access is even eviler (if there is such a word). Instead he is suggesting that no object take apart another object and change its values, but it should ask the other object to do it. -- General/RyanBates*
...
I also find it difficult to live without accessors when doing General/UnitTesting because I need to check if the object behaved correctly with its data. Anyone have a solution to this? Perhaps I'm doing General/UnitTesting incorrectly.
...
Back to accessor methods. I see the importance of data abstraction, but if a method simply returns data, does that mean it is an accessor method? For example, if one is creating an e-mail client which contains a Message class, is it wrong to have a "- (General/NSString *)message" method in the Message? We don't care how it stores it's message, it may retrieve it from a file , generate it, or even store it as an General/NSString instance variable - but all we want is the message. Is this wrong?
...
Overall these are two very interesting articles and I am looking forward to the discussion. -- General/RyanBates
...
*I also find it difficult to live without accessors when doing General/UnitTesting*
...
It should be noted that Allen Holub is a very seasoned developer, and what he writes is not to be taken lightly. It is based on many years of experience on object oriented design. You cannot say that it is nonsense. You may want to go to his webpage [http://www.holub.com/] and look up the article linked to next to the article on General/ExtendsIsEvil. 
...
- (void)setDeliveryDate:(General/NSDate *)newDate
{
	[deliveryDate release];
	deliveryDate = [newDate copy];
}

- (General/NSDate *)deliveryDate
{
	return deliveryDate;
}
...
Why is General/CocoaDev turning into Slashdot? Offsite links and persuant discussions seem out-of-character with this site. � General/BrentGulanowski

*Oh, come on, General/GettersAndSettersAreEvil is an important (albeit an advanced) discussion. It just so happens that it refers to a previously written article. How many are there anyway? Two? (this one and General/ExtendsIsEvil). --Theo*

**Also, you will find off-site links on every single class page. See General/NSWindow, General/NSObject, General/NSResponder, etc.)**