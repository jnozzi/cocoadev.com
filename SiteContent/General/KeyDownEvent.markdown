...
I'd like to be able to capture all incoming key events - as in, my process becomes the only one to receive them. I worked out the following bit of code to do the first part of this task - it should run the %%BEGINCODESTYLE%%keyPressed()%%ENDCODESTYLE%% function whenever a key is pressed.
...
Of course, it doesn't work for me - whenever a key is hit I hear a beep, but %%BEGINCODESTYLE%%keyPressed()%%ENDCODESTYLE%% is never accessed. A quick check shows that %%BEGINCODESTYLE%%[[InstallEventHandler]]()%%ENDCODESTYLE%% is successful, and none of the variables are invalid.
...
According to [[CarbonEvents]].h, [[GetEventMonitorTarget]]() receives all events of the specified class and type, as long as your program isn't frontmost. I'm not sure whether mine is or not, since it is a background application (%%BEGINCODESTYLE%%[[LSUIElement]]%%ENDCODESTYLE%% is set to %%BEGINCODESTYLE%%1%%ENDCODESTYLE%% in the Info.plist), and the only window is an overlay window.
