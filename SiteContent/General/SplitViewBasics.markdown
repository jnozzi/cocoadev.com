...
'''Note:''' if you have multiple [[NSSplitView]]'s with the same delegate, you must call <code>adjustSubviews</code> on the split views that you don't adjust! Here's the modified code from above:
<code>if (sender == mainSplitView) // your split view
{
