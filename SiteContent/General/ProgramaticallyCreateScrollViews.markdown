...
I've discovered a few things. First, it only happens when you tell an General/NSScrollView to use a documentView which has a -headerView method. In this case it's an General/NSOutlineView, but the exact same thing happens when I put in a custom General/NSView subclass with a headerView method.
...
Last, this does not happen with an General/NSOutlineView I create in IB. I don't know why, I've tried just about everything to get the same results as the IB outline view, including subclassing it and setting a breakpoint so I could look at its private variables in the debugger, but I came away with nothing.

Here's some code, if it helps:

    
	General/NSView *contentView = General/self window] contentView];

	outlineView = [[[[SelectionOutlineView alloc] init];
	[outlineView setFocusRingType:General/NSFocusRingTypeNone];
	[outlineView setDataSource:General/[NSApp delegate]];
	
	General/NSScrollView *scrollView = General/[[[NSScrollView alloc] initWithFrame:outlineRect] autorelease];
	[scrollView setAutoresizingMask:General/NSViewHeightSizable];
	[scrollView setDocumentView:outlineView];
	[scrollView setHasVerticalScroller:YES];
	[scrollView setHasHorizontalScroller:NO];
	
	General/NSTableColumn *titleColumn = General/[[[NSTableColumn alloc] initWithIdentifier:General/GRFilterOutlineViewIdentifier] autorelease];
	General/titleColumn headerCell] setStringValue:@"Header"];
...
----
Sadly, calling -tile on either the General/NSScrollView or the General/NSBrowser does not fix the problem. I guess this would be the place where the scrollview is messing up-- when it lays out the components it's expecting to draw an extra scroller or part of the header view or something, even though it shouldn't. I suppose I could try subclassing General/NSScrollView and overriding -tile, but, well, I shouldn't have to.
----
...
----
When I'm in those situations, I just have a separate nib for each subcomponent, or even just a separate General/CustomView in the same nib. Then you can still get your easier layout and separation of stuff, and programmatic insertion of things, but without having to construct everything manually.
----
...
IB General/OutlineView:
     General/ScrollView Frame: {{0, 0}, {480, 360}}
     General/DocumentView Frame: {{0, 0}, {463, 342}}
     General/ClipView Frame: {{1, 17}, {463, 342}}
         General/ClipView documentRect: {{0, 0}, {463, 342}}
         General/ClipView documentVisibleRect: {{0, 0}, {463, 342}}
     General/HeaderView Frame: {{0, 0}, {463, 17}}
     General/CornerView Frame: {{464, 0}, {16, 17}}
Coded General/OutlineView:
     General/ScrollView Frame: {{0, 0}, {480, 360}}
     General/DocumentView Frame: {{0, 0}, {465, 343}}
     General/ClipView Frame: {{0, 17}, {465, 343}}
         General/ClipView documentRect: {{0, 0}, {465, 343}}
         General/ClipView documentVisibleRect: {{0, 0}, {465, 343}}
     General/HeaderView Frame: {{0, 0}, {465, 17}}
     General/CornerView Frame: {{465, 0}, {16, 17}}
...
I also tried enumerating though all the subviews. One thing I noticed was that apparently the IB scrollview had an extra subview with an invalid frame, presumably for the empty horizontal scroller. However, when I instantiated a new General/NSScroller with an equivalent frame and set it as the horizontal scroller, it still didn't fix things. So, I just don't know what's wrong.

----
I think I may have just encountered a related problem due to the headerView.  I created a subclass of General/NSView as a container for 3 General/WebViews, which are created programmatically.  I could verify in the debugger that the General/WebViews were subviews of my view, but after the view was added to the view hierarchy of the window, one of the views was moved outside my view as a sibling.  I finally tracked the problem down to the fact that my view subclass had an instance variable named "headerView".  Evidently, General/NSScrollView has special, undocumented handling for a subview that has a headerView.  In my case, the first drawing of my class and subclasses was inconsistent after being added to the scroll view.
...
This may help explain the source of the problem, but the real question is how to deal with it.  I just renamed my instance variable.  In your case, I would suggest experimenting with building up your subviews differently.  In fact, I would try init'ing the outline view based on the content size of the scroll view:
	...
	General/NSSize contentSize = General/[NSScrollView frameSizeForContentSize:outlineRect.size hasHorizontalScroller:NO hasVerticalScroller:YES borderType:???];

	outlineView = General/[[SelectionOutlineView alloc] initWithFrame:General/NSMakeRect(0, 0, contentSize.width, contentSize.height)];
	[outlineView setFocusRingType:General/NSFocusRingTypeNone];
	[outlineView setDataSource:General/[NSApp delegate]];
