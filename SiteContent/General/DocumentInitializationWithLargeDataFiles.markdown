...
I'm a little concerned by the line <code>rawData[index+y] = (UInt32)(abs(logf(A.realp[y]''A.realp[y]+...</code>, since <code>index</code> doesn't appear to be initialized (or, indeed, declared). Did you mean <code>x''pixelsY</code> here perhaps? That'd cause the problem for sure.
...
''Well, not exactly. An instance of [[NSDocument]] ''can'' be reinitialized (in the English sense, not the Cocoa sense) by reloading the data from a <code>read</code> method (like <code>readFromFile:ofType:</code>). --[[JediKnil]]'''
...
I see something strange in the lines
        ...
        <code>sourceFile = [[fileName copy] autorelease];</code>

       <code> documentType = [[docType copy] autorelease];</code>
