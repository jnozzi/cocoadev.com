...
You have the right idea. Delegates aren't magic or anything. You just have an ivar like <code>id delegate</code>, then make sure it responds to the delegate messages before you send them.
...
<code>[delegate tableView:self didClickTableColumn:column]</code>
...
----
To be more precise: an informal protocol is declared exactly the same way that you declare a category. It is thus like saying to the compiler: I am adding to the class <code>[[NSObject]]</code> the following methods.
Categories can be for anything other than <code>[[NSObject]]</code>, but an informal protocol only makes sense as a category of <code>[[NSObject]]</code>, as you usually don't put any limitation in what a delegate can be.
