I guess my real question is: Is General/CFDictionary a wrapper for General/NSDictionary or is it the other way around?
...
----
General/NSDictionary/General/CFDictionaryRef are **toll-free bridged** like most of the common, primitive data structures. The implementation of objectEnumerator is most likely trivial as all that needs to happen is the creation of an General/NSEnumerator that understands how to enumerate a given container.

In fact     General/NSDictionary *myDictionary = (General/NSDictionary *)someCFDictionary; is quite legal.

All in all, Objective-C classes are just simple structures anyways.

----

But how would you iterate a General/CFDictionary? The General/CFDictionaryRef is an opaque structure, so the General/NSDictionary (assuming it is using Core Foundation General/APIs) would need to request an array of all keys/values.
----
Hmm, General/CFDictionaryGetKeysAndValues jumps to mind.

So does this...

http://developer.apple.com/documentation/General/CoreFoundation/Reference/General/CFDictionaryRef/index.html

----

Yes, that was the function I was referring to -- but if General/NSEnumerator uses that function then it pays a rather high abstraction penalty, since it is practically making a full copy of the General/NSDictionary.
...
----
A copy, yes, but not a deep copy. Just the contained object pointers. A quick disassembly would reveal what the compiler is really generating. Off the top of my head, I don't think General/NSEnumerator is **publicly** toll-free bridged, but that is not to say the mechanism doesn't exist. I would have to check when I find the time. I don't remember anyone claiming that Core Foundation was a speed deamon. Most "reference count" API's are not.
----
Well, I found the time, just because you got me curious. I spent some time in the debugger (the new disassembly mode rocks; hit Cmd-F14 twice in the debugger) and it indeed (eventually) calls General/CFDictionaryGetKeysAndValues. I should point out that it is - General/[NSDictionary objectEnumerator] that calls this indirectly, there are a number of concrete classes that have intimate knowledge of the Core Foundation types. I will also note that the time spent in General/CFDictionaryGetKeysAndValues is shall we say, breif. Maybe 70 lines of assembler at most with no function calls.
...
I was asking the question because General/NSEnumerator is seriously limited (e.g. doesn't support copying, only forward iteration, no assignment during the iteration etc.), so I wanted to write a replacement, but it would seem rather inefficient, though now it is clear that the native General/NSEnumerator suffer from the same inefficiency.
...
General/NSArray* values = ...;
General/NSDictionary* dict = ...;
General/NSEnumerator* outer = [values objectEnumerator];
while(id value = [outer nextObject])
{
   General/NSEnumerator* inner = [dict objectEnumerator];
   while(id dict_value = [inner nextObject])
   {
      if([value isEqual:dict_value])
      {
         General/NSLog("Dictionary has value: %@\n", value);
         break;
      }
   }
}
...
General/NSDictionary* dict = ...;
General/NSEnumerator* outer = [values objectEnumerator];
General/NSArray *allValues = [dict allValues];
while(id value = [outer nextObject]) if ([allValues containsObject:value]) General/NSLog(@"contains value: %@", value);

----
You could if General/NSDictionary had a     containsObject method, which it does not.     allKeysForObject is probably the closest equivalent, but not very efficient. You could just grab     allValues up front and perform a     containsObject on that. There are a number of methods that can be used to optimize this sequence, but as this exercise is showing, a dictionary or map data structure is not really a good candidate for enumeration. If you find that your code is doing this often, you may want to consider changing your design in this regard.

I will also point out that if you use Core Foundation directly for this sequence, you bypass the autorelease pool issue.
----
Or place the dictionary values in a General/NSSet for O(max(n, m)) performance (assuming set construction is O(n) and set membership test is O(1), unfortunately Apple does not reveal the General/TimeComplexity for container methods, but I think General/CFSet says that membership testing is at worst O(lg N) but expected O(1), which is a bit strange, cause with hashing it could at worst be O(n), unlike e.g. a balanced binary tree, which would guarantee the O(lg N), but would also be expected O(lg N)) -- but as you hint, the exercise was not about the actual task, but merely showing that General/NSEnumerator can easily lead to code performing much worse than the actual source hint, since it is unknown for the user that the enumerator copies the entire dictionary (and unintuitive).
----
Here is a Core Foundation equivilant of your example that may suite your requirements and avoids the autorelease pool.
    
static __inline__ General/NSString* General/GetFirstDictionaryValueAlsoContainedInArray(General/CFDictionaryRef theDict, General/CFArrayRef theArray)
{
	General/NSString* result = NULL;
	General/CFIndex count = General/CFArrayGetCount(theArray);
	if (count) {
		General/CFIndex index;
		General/NSString *value;
		for (index = 0; index != count; ++index) {
			value = (General/NSString*) General/CFArrayGetValueAtIndex(theArray, index);
			if (General/CFDictionaryContainsValue(theDict, value)) {
				General/NSLog(@"Dictionary has value: %@\n", value);
				result = value;
				break;
			}
	...
	return (result) ? General/result retain] autorelease] : nil;
}
...
----
And as my last statement alluded to, maybe you should rethink trying to force General/NSDictionary/General/CFDictionaryRef into this mold and pick an alternate container. You would need intimate knowledge and access to the General/CFDictionary internals.
----
*That is no good -- I need iteration of a dictionary... and what I ask for above is really not that unreasonable or goes against any common software design philosophy -- I realize that it is not do-able with Apple's General/APIs, but please do not tell me that I have to change my design, I do know how to make a proper design!*
----
...
You have been shown how to enumerate a General/NSDictionary and a General/CFDictionaryRef both here and in Apple's documentation.
...
**std::map** supports bidirectional iterators. As far as bidirectional General/NSEnumerator(s) are concerned, file a bug report with Apple.

I will note that std::map is currently implemented as a Red-black tree. The **std::hash_map** extension template is of course a implemented as a hash table, which (in most implementations) only support forward iteration. I guess I don't have to tell you that General/NSDictionary/General/CFDictionaryRef are implemented as hash tables.

Furthermore, given the Core Foundation calls that bypass the autorelease pool issues; I would wager a bet that the single heap allocation, and copying 1000 values out of an General/CFDictionaryRef and iterating through them as an array is going to be far faster than the 1000(+) Objective-C method calls made through an theoretical bidirectional General/NSEnumerator proxy to do achieve same thing. If you are just interested in the values, you are allocating less than 4K for the Core Foundation method, less than 8K with the keys.

----

ehm... yeah... that's great if we only have 1,000 values in the dictionary, if we only have one active iteration at a time and if we always need to iterate the full dictionary. That is quite a lot of assumptions.  I wanted to write a replacement for General/NSEnumerator because it lacked some features, like it does not implement General/NSCopying etc., the actual use case may differ a lot, so comparing the time spent copying 1,000 values with sending 1,000+ General/ObjC methods makes no sense to me... but then again, I am just one of those academic buzz-word craving wannabes who make theoretic assumptions about scalability...
...
The std::hash_map implementation that I had seen that allows bidirectional iteration maintains a second container internally and marries the two up. So you have a hash table implementation that is maintaining two separate data structures. It would seem that Apple has taken a more traditional approach by basically implying that you shouldn't be using an associative container, specifically a hash-table, for linear enumeration and if you really want to do this, you will pay a price, but the General/NSDictionary implementation will not. I handle this in one of my projects by maintaining a separate container on top of the General/NSDictionary myself, quite similar to the std::hash_map I was referring to. It takes care of the fast-track keyed look-ups that the hash-table was designed for and also allows for fast, low-overhead indexing/enumeration via an appropriate container.
...
With regard to bidirectional iteration, this is more of a general need with General/NSEnumerator (i.e. when used for other containers), but I wanted it for General/NSDictionary as well, for completeness (and I have on occasion had a problem where it would have been useful).
...
Whatever, We all know about proper algorithm design, it's not like these things were born yesterday. The std::hash I was referring to was using a linked list, to be specific. I am sure Apple (actually it's a General/NeXT implementation I believe) has good reason, besides the obvious, for implementing and having kept the General/NSDictionary/General/CFDictionaryRef (General/NSMutableDictionary/General/CFMutableDictionaryRef implied) the way they have. If you **need** to fix it for the purposes of linear enumeration, you know what to do. As another thought, a theoretical General/NSMap based on red-black would be trivial to implement, and as I am sure you know, you can return a subclass of General/NSEnumerator from your own (or overridden) - (General/NSEnumerator *)objectEnumerator.

----
The General/NSDictionary/General/NSEnumerator does not have the interface you are looking for, but you're forgetting the alternate interface. Cast your General/NSDictionary to a General/CFDictionary, and use a CF style function -- General/CFDictionaryApplyFunction comes to mind. 
...
This is way off in General/MailingListMode, the discussion (from what I've read) isn't even about any concrete engineering issues.  The original question doesn't have anything to do with Cocoa either.
