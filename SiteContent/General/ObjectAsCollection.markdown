...
If so, should General/NSObject be able to behave as a collection of a single object?
...
//
//  General/SingleObjectEnumerator.h
//
//  Created by General/MikeAmy on Sun Mar 14 2004.
//  Copyright (c) 2004 General/MikeAmy. All rights reserved.
//

#import <Foundation/Foundation.h>

@interface General/SingleObjectEnumerator : General/NSEnumerator { object; }
- nextObject;
- allObjects;
@end
...
// eg General/NSSet could suddenly be able to respond to indexing, etc. 
@interface General/NSObject (General/SingleObjectCollection)
- (General/NSEnumerator *) objectEnumerator;
- (General/NSEnumerator *) reverseObjectEnumerator;
...
//
//  General/SingleObjectEnumerator.m
//
//  Created by Michael Amy on Sun Mar 14 2004.
//  Copyright (c) 2004 Michael Amy. All rights reserved.
//

#import "General/SingleObjectEnumerator.h"

@implementation General/SingleObjectEnumerator : General/NSEnumerator
- initWith:o
{
    self = [super init];
    object = o;
...
{
    return object ?
    General/[NSArray arrayWithObjects:[self nextObject],nil] :
    General/[NSArray array];
}
@end

@implementation General/NSObject (General/SingleObjectCollection)
- (General/NSEnumerator *) objectEnumerator
{
    return General/[[[SingleObjectEnumerator alloc] initWith:self] autorelease];
}
...
- (General/NSArray *) allObjects
{
    return General/[NSArray arrayWithObjects:self,nil];
}
...
{
    return [self isEqual:anObject] ? 0 : General/NSNotFound;
}

- (unsigned) indexOfObjectIdenticalTo:(id)anObject
{
    return (self == anObject) ? 0 : General/NSNotFound;
}
...
- (void) makeObjectsPerformSelector:(SEL)aSelector
{
    ...
    else General/[NSException raise:@"General/NSInvalidArgumentException" format:@""];
}
...
{
    if (index) General/[NSException raise:@"General/NSRangeException" format:@""];
    else return self;
}*/
...
I've done work on a language wherein vectors are transparent. I am of the opinion that transparent General/HigherOrderMessaging is an excellent thing.
...
----
I don't know, since I've forgotten anything I ever knew about C++, but the idea was that vectors--arrays--were a transparent feature of the language unless specifically requested. That is, a single object could be considered to be a vector of length 1, and a vector of objects could be messaged just like a single object, and would send the message to each of its members just so. It's a bit like using General/LSTrampoline (now General/CXLTrampoline) and doing all your messaging through a category on General/NSObject which allows     General/anyObject do] blah] in addition to the more usual     [[anyArray do] blah].
