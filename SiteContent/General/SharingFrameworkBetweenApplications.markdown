...
Know what I'm talking about? Could someone help? Thanks. --General/KevinWojniak
...
Symlinks are very un-Mac. If the user moves the applications relative to eachother, the symlinks will break, it's not how applications should work on a Mac. A better way to do this could be to use     General/[NSWorkspace absolutePathForAppBundleWithIdentifier:] to get the path to the application which contains the framework, and then load it dynamically with     General/[NSBundle bundleAtPath:] (see General/DynamicallyLoadFrameworks).
.
