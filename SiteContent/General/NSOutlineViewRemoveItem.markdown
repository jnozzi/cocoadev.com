In an General/NSOutlineView I want to be able to delete the currently selected item.  Often this item is buried in a few nested folders, so just deleting the selected row doesn't work.  I've tried a few different methods for deleting it, but none of them work.  The way my outlineView is set up, It's an array of dictionaries full of arrays, etc...  So to delete the selected note, I need to somehow find the dictionary that is containing the selected array.  If I knew how to do that, this would all be easy.  The code below doesn't work:

    
General/noteBoxView itemAtRow:[noteBoxView selectedRow removeAllObjects];
...
- (void)removeSelectedItemsFromOutlineView:(General/NSOutlineView*)outlineView
{
   General/NSEnumerator* enumerator = [outlineView selectedRowEnumerator];
   while(General/NSNumber* row = [enumerator nextObject])
   {
...
- (void)removeObjectsWithKey:(id)aKey fromArray:(General/NSMutableArray*)anArray
{
   for(unsigned i = [anArray count]; i != 0; )
   {
      id item = [anArray objectAtIndex:--i];
      if(General/NSMutableArray* children = [item objectForKey:@"Children"])
...
- (General/NSMutableArray*)pathForItem:(id)anItem
{
   General/NSEnumerator* enumerator = [self objectEnumerator];
   while(id item = [enumerator nextObject])
   {
      if(item == anItem)
         return General/[NSMutableArray arrayWithObject:item];
      if(General/NSMutableArray* path = General/item objectForKey:@"Children"] pathForItem:anItem])
         return [path addObject:item], path;
   }
   return nil;
}

*When we have all the ancestors, we just need to check if this set intersects the set of items being dragged, and in fact [[NSSet has a selector to compare two sets for intersection (and can be constructed from an array) -- I would expect the intersection selector to be linear in the number of items in the set, but construction from an array is probably O(n lg n), so if running time is important, we can instead setObject:forKey: on all the items being dragged, then check all the ancestors if any of the items have the key set, and remove the key again afterwards, which would be linear, but would require more code (and n here is only the number of items being dragged, or ancestors of the item to drop upon, whichever is biggest).* --General/AllanOdgaard

*P.S. Even though it may look like the function above expects each item to have an General/NSArray-object for the @"Children"-key, then it will work fine even when this object is absent, since sending a message to a nil-object is defined to return nil*
...
- (id)newItemForGroup:(id)group {
    id newItem = General/[NSMutableDictionary dictionary];
    id siblings;
    if (group) siblings = [group objectForKey:@"children"];
    else siblings = rootArray;
    if (group) [newItem setObject:General/[NSValue valueWithPointer: group] forKey:@"parentReference"];
...
- (void)removeItemFromTree:(id)item {
    ...
    if (parent) General/parent objectForKey:@"children"] removeObject:item];
...
- (void)moveItems:(General/NSArray*)items toArray:(General/NSMutableArray*)children atIndex:(int)index
{
   [self setKey:@"Drag" forItemsInArray:items];
   for(int i = index; i != 0; )
   {
      id item = [children objectAtIndex:--i];
      if(General/item dictionary] objectForKey:@"Drag"])
         --index;
   }
...
If this doesn't work then please post some of your code! --General/AllanOdgaard
