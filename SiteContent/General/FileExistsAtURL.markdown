...
Then you need to connect to the url and see if it comes back 404. See http://developer.apple.com/documentation/Cocoa/Reference/Foundation/ObjC_classic/Classes/General/NSHTTPURLResponse.html#//apple_ref/doc/uid/20001694/CJBECBFB
...
@implementation NSURL (General/ValidityChecking)

- (BOOL)httpIsValid
        BOOL isValid = NO;
        General/NSMutableURLRequest *request = General/[NSMutableURLRequest requestWithURL:self];
        [request setHTTPMethod:@"HEAD"];
        General/NSHTTPURLResponse *response = nil;
        General/[NSURLConnection sendSynchronousRequest:request
...
Unfortunately, this turns out to be very slow on Panther.  Cocoa sits around waiting for data that will never come. And since the General/NSHTTPURLProtocol class is private, there's no obvious way to extend it to be more sensible...

So I'd look at General/CURLHandle or something.  You need to do the equivalent of the above.
...
I hope someone can help to get this working as I need to do HEAD requests myself and would prefer to not use General/CURLHandle. --General/MikeZornek
