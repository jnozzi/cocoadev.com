...
I'll give a better example, without generic names. I've got a Class called General/GameObject. This creates new static objects in the world, handles loading meshes, etc, and placing it in the world. It is subclassed by General/MovableGameObject which handles (as it sounds) moving objects around the scene. General/MovableGameObject is subclassed by General/PhysicalGameObject, which is objects that are handled by the physics engine. It uses some of the methods in General/MovableGameObject, but it overrides some as well.

Now, I want to have Light and Camera objects. These objects will handle any specifics for lights, cameras, etc. I can make them subclasses of General/MovableGameObject, and I can move them around the scene. But, what if I want the camera to be affected by the physics engine? 

I could subclass Camera and make General/PhysicalCamera, but I'm duplicating the physics related code. I could subclass General/PhysicalGameObject and make General/PhysicalCamera, but now I'm duplicating the camera related code. 

I'd like to have one Camera class, and be able to dynamically chose when I make an object if it subclasses General/PhysicalGameObject or General/MovableGameObject. Is this possible?
...
If you are looking for something more than your problem statements explicitly states, consider the a Design Pattern: http://developer.apple.com/documentation/Cocoa/Conceptual/General/CocoaFundamentals/General/CocoaDesignPatterns/chapter_5_section_2.html
http://developer.apple.com/documentation/Cocoa/Conceptual/General/CocoaFundamentals/General/CocoaDesignPatterns/chapter_5_section_3.html
...
Start here: http://developer.apple.com/referencelibrary/General/GettingStarted/GS_Cocoa/index.html
or http://developer.apple.com/documentation/Cocoa/Conceptual/General/ObjectiveC/index.html
...
Look at using Composition, rather than Inheritance. Perhaps a General/MoveableGameObject could have a Camera, not be a Camera. This works nicely with Delegation.
http://www.c2.com/cgi/wiki?General/CompositionInsteadOfInheritance
...
@interface General/AbstractA <A> { } // has common methods

@interface SubA1 : General/AbstractA { } // implements importantMethod in one way

@interface SubA2 : General/AbstractA { } // implements importantMethod in another way
