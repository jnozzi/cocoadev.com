I am working on a textview that links to General/SQLite. The behavior I would like is as follows:

- sqlTextview is either empty (new) or has past statements that are uneditable
- User types in SQL statement
- New text is appended to General/NSString * queryString
- If last character = ";" then perform query
- queryString is appended to history text (General/NSMutableArray * history)
...
It sounds like you are after some form of terminal emulation. I too am interested in this type of functionality out of an General/NSTextView. I wonder if Apple will ever make the text view in Terminal app part of Cocoa. You really should try to find a way to emulate a terminal in an General/NSTextView first before trying to reinvent the wheel. Please keep us up to date on your progress.
