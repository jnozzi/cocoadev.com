...
Zombies will take effect for all Objective-C objects that are deallocated through normal means, including most Cocoa classes as well as user-created classes. On 10.4 and earlier, a rather important exception to this is most/all General/TollFreeBridged classes, as they are deallocated using General/CoreFoundation which General/NSZombieEnabled will not trap, in which case General/CFZombie must be used. On 10.5, General/NSZombieEnabled will work for General/TollFreeBridged classes as well.

See the General/CFZombie page for information on how to debug errors involving General/TollFreeBridged classes such as NSCFString. 
...
*In the "Variables to be set in the environment:" section, make a variable called "General/NSZombieEnabled" and set its value to "YES".


General/NSZombieEnabled should not be left in place permanently, as by default no objects will ever be truly deallocated, making your application use tremendous amounts of memory. If you find yourself needing to use it often, you can create a second executable in your project which uses General/NSZombieEnabled so that you can easily switch between them. Do this by right-clicking the original executable, selecting "Duplicate", and then setting up General/NSZombie in the new executable.

Or, you can not use Xcode and instead run your application from the terminal, with the command:
    env General/NSZombieEnabled=YES Whatever.app/Contents/MacOS/Whatever
