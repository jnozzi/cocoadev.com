...
Actually, Apple says that after some investigation this is a known issue -- <code>isEqual:</code> should not break because <code>[obj isEqual:obj]</code> should return <code>YES</code>, even when there are cyclic references (since a pointer comparison will be done before any ''deep'' comparison), and comparing two self referencing dictionaries with <code>isEqual:</code> does in fact work.
