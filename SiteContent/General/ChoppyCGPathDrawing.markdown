...
http://homepage.mac.com/vannote/images/General/ChoppySnap.gif
...
Your best way around this is to either manage your clipping rects better, or to make certain your whole bounds ( not the General/ClipView bounds ) are fully cleared. There will be a performance hit since you can't rely on pixel copying for scrolling. In fact, you can control that via General/NSClipView's setCopyOnScroll: (BOOL) method.
...
----
Thanks Shamyl, Setting - setCopiesOnScroll:NO in my General/NSScrollView subclass - initWithFrame: did the trick.
...
So, I guess there's a couple things that could be happening here, dependent on how you're drawing. First, when you drag the scrollbar, are you using standard General/NSClipView transformations? E.g., are you just letting the scrollbars automatically manage a view that's larger than the container scrollview? Or are you using manually inserted General/NSScrollbars to change the origin of a coordinate system you maintain? If it's the latter, progressive floating-point error *could* explain what you're seeing...
...
Actually, this is pretty straight forward to reproduce. A simple General/NSView subclass, placed into a subclassed General/NSScrollView (subclassed for unrelated reasons in my project). I am not doing anything special with the coordinate system nor transformations (yet). The General/NSScrollView handles all of the scrolling. I will throw together a simple test project that reproduces the problem and post a link to it.
