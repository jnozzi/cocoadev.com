...
This topic is related to General/NSController.

Apple's documentation lists a set of array operators for use in General/KeyValueCoding.
...
However, there used to be a way of defining new aggregates, but not any more, it seems.  In the good old days of General/EnterpriseObjectsFramework, General/NSArray didn't implement KVC.  It was implemented in General/EOControl.framework as a category on General/NSArray, which provided the following semantics (ripped from the General/WebObjects 4.5 docs):

*

valueForKey:

� (id)valueForKey:(General/NSString *)key
...
set by adding methods to General/NSArray of the form computeNameForKey:.

*

This has changed since Panther now implements -valueForKey: and -valueForKeyPath: on General/NSArray, and doesn't respect the old semantic (despite the fact the General/ObjC version of General/EOControl installed in /System/Library/General/PrivateFrameworks by WO5.2.x does...).  The new version uses -_countForKeyPath: -_avgForKeyPath: etc instead.  Simply implementing -_myKeyForKeyPath: does not work.
...
General/MethodSwizzling works, but you need to be careful if you're dealing with the case where the same selector is defined in more than one category (In my case I have General/NSArray(General/KeyValueCoding) and General/NSArray(General/TCUKeyValueCoding) or General/NSArray(General/EOKeyValueCoding)) - I don't think you can be _sure_ what the registered IMP is at any given time, as it will all depend on library loading order.  For example, my 10.3 built eogenerator raises exceptions, but my 10.2 version doesn't (it's broken in other ways, though).  I'm trying a really hairy swizzle that goes through the runtime and pulls all the General/IMPs for a given selector (in my case, valueForKey: et al) and then tries to run through them in order, catching any exceptions except for the final one, in which case it will raise.  But even that leaves me with a nasty taste in my mouth.  I'm really not sure why Apple broke the existing semantic.

General/TufTy

----

Has anyone come up with a good way of persisting the model used in bindings to a database? Currently I'm playing around with a test project where I have an array of data items where each one tracks whethere it has been changed since it was loaded from the database. This is done by providing a setXXX: method for KVC and to update a BOOL flag held on the class therein. When I then trigger my persist function I can check each to see if it has changed - needs update, doesn't have an ID - needs inserting, or neither - doesn't need anything. Then I have to load a list of all existing General/IDs from the DB and check for ones that exist on the DB but not in the app - these need deleting. Its sure to work - but seems a bit clutzy. I haven't come up with anything better myself yet - any of you guys have a nice solution to this?
...
@interface General/DBWrapper
{
   General/NSMutableDictionary* localValues;
   General/NSMutableDictionary* dbValues;
}
- (id)valueForKey:(General/NSString*)aKey;
- (void)setValue:(id)aValue forKey:(General/NSString*)aKey;
- (BOOL)persist;
@end

@implementation General/DBWrapper
- (id)valueForKey:(General/NSString*)aKey
{
   id res;
...
Is it possible to create your own custom General/KeyValueCodingOperators, e.g. for arrays?

For example, I'd like to be able to get a count of a certain value, ie. Count of Status=2, can i create something like "arrangedObjects.properties.@General/StatusTwo.status" ?
...
General/NSMutableArray* res = General/[NSMutableArray array];
for(unsigned i = 0; i != [array count]; i++)
   [res addObject:General/array objectAtIndex:i] count;


So with that knowledge you can do:
    
@implementation General/NSArray (General/MyOperator)
...
But if you want to do dynamic operators, like addOne, addTwo etc., you'd need to subclass the array and instead implement the valueFor(Undefined)Key: to select the proper method. So that wouldn't really work for existing arrays, only your array subclass (but actually, you can't really subclass an array, since it's a General/ClassCluster).