**General/ParseError because of old-style compiler prohibitions on declaring variables after executable code starts**

I have an error message :

"parse error"       before   
 " General/NSString * title = [song substringToIndex:range.location];"
...
int main (int argc, const char * argv[]) {
    ...
    General/NSString * song = @"let forever be, the chemicla broters";
    General/NSRange range = [song rangeOfString: @","];
    printf("comma location is: %i\n", range.location);
    
    General/NSString * title = [song substringToIndex:range.location];
    printf("title is : %s\n", [title UTF8String]);
    
    General/NSString * artist = [song substringFromIndex: range.location + range.length];
    printf("artist is : %s\n", [artist UTF8String]);
...
This is because you are declaring General/NSString after calling functions. You aren't allowed to do this in C or Obj-C. Put the title and artist variables up at top, and set them later.
...
ONE more snag I've come accross, there is an exercise where you create a delegate to popup an alert on whether or not to terminate an application.  The book shows (General/NSTerminationReply)applicationShouldTerminate: when instead it needs to be (General/NSApplicationTerminationReply) for Mac OS X 10.1.  Maybe apple changed this enum from 10.1 to 10.2, I don't know because I don't have access to a 10.2 install (yet -- gonna upgrade in about a month or so...)

I have to recommend this book to anyone starting out with Cocoa programming.  It starts off with excellent examples of using General/NSStrings in various ways and then moves on to more complex topics.  This book even mentions this web site on page 341.  Now that's cool!
----
...
In a small tool I've written/translated from Java today, the gcc compiler gives me a parse error whenever my own class General/LCVertex is
...
In the file that references General/LCVertex, did you #import "General/LCVertex.h"?
...
The compiler absolutely has to know about every type you use before you use it. This means that you have to either #import your header in the .h as well (not recommended unless you're subclassing it), or you have to tell the compiler that General/LCVertex is a class by typing     @class General/LCVertex; somewhere above the part where you use it.
...
- (General/IBAction)startRip:(id)sender
{
    cdda2wav = General/[[NSTask alloc] init];
    
    [cdda2wav setCurrentDirectoryPath:General/NSHomeDirectory()];
    
    [cdda2wav setLaunchPath:@"/sw/bin/cdda2wav"];

    [cdda2wav setArguments:General/[NSArray arrayWithObjects:@"-paranoia","-B","-O","aiff","cddb=1","dev=General/IODVDServices",nil]];
...
You need to add the @ before each General/NSString you're creating.
...
{
cut..
    cdda2wav = General/[[NSTask alloc] init]; <-- HERE
cut..
}
...
http://developer.apple.com/documentation/Cocoa/Conceptual/General/CodingGuidelines/Articles/General/NamingBasics.html
...
- (void)someFunction
{
	General/NSObject *myObject = General/[[NSObject alloc] init];
...
When I put in code (directly cut & pasted from the documentation mind you!) it will not work - it simply gives a parse error. The code being pasted into my General/MyDocument.m file:

    
- (void)webView:(General/WebView *)sender didReceiveTitle:(General/NSString *)title forFrame:(General/WebFrame *)frame
{
    ...
    if (frame == [sender mainFrame]){
        ...
        General/sender window] setTitle:title];
...
Did you #import <General/WebKit/General/WebKit.h> at the top of your file?
...
----
This page (and others like it) is exactly why all the experienced developers have stopped contributing to or maintaining this wiki. Too many people ignore the community rules and etiquette, use the wiki in General/MailingListMode, and create so many pages that amount to noise (through which you must search ... and search ... and search) when they wind down.

----
You know, I really don't think that General/MailingListMode is the menace that it's made out to be. The General/PortlandPatternRepository (THE original Wiki) has a ton of conversational-style pages and it gets by just fine. -- General/MikeAsh