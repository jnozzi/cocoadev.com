...
I am trying to write a wrapper for a command line tool that requires user input after launch. I have set up the pipes to and from the General/NSTask object.
...
- (General/IBAction)run:(id)sender
{
	//clean up the view
	[outputText setString:@"Process Started\n"];
	
	//set our attributes
	attributes = General/[[NSArray alloc] initWithObjects:@"/usr/bin/Unison",[commandText stringValue],[commandText2 stringValue],@"-dumbtty",nil];
	
	//sort out the task
	unisonTask =General/[[NSTask alloc] init];
	
	//set outputs
	[unisonTask setStandardOutput: General/[NSPipe pipe]];
	[unisonTask setStandardError: [unisonTask standardOutput]];
	
	//set inputs
	[unisonTask setStandardInput:General/[NSPipe pipe]];
	
	[unisonTask setLaunchPath:[attributes objectAtIndex:0]];
	[unisonTask setArguments:[attributes subarrayWithRange: General/NSMakeRange(1,([attributes count] - 1))]];
	...
	General/[[NSNotificationCenter defaultCenter] addObserver:self 
											 selector:@selector(getData:) 
											   ...
											   object:General/unisonTask standardOutput] fileHandleForReading;
	
	General/[unisonTask standardOutput] fileHandleForReading] readInBackgroundAndNotify];
...
{
	General/NSLog(@"sending command: %@",[respondText stringValue]);
	General/[unisonTask standardInput] fileHandleForWriting] writeData:[[respondText stringValue] dataUsingEncoding:NSUTF8StringEncoding;
	
	General/[unisonTask standardInput] fileHandleForWriting] closeFile];
...
- (void) getData: ([[NSNotification *)aNotification
{
    ...
    if ([data length])
    {
        ...
        General/outputText textStorage] appendAttributedString:[[[[[NSAttributedString alloc] initWithString:General/[[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease]]autorelease]];
    } 
...
- (void) stopProcess
{
    ...
    General/[[NSNotificationCenter defaultCenter] removeObserver:self name:General/NSFileHandleReadCompletionNotification object: General/unisonTask standardOutput] fileHandleForReading;
    
    // Make sure the task has actually stopped!
    [unisonTask terminate];
	
	while ((data = General/[unisonTask standardOutput] fileHandleForReading] availableData]) && [data length])
	{
		[[outputText textStorage] appendAttributedString:[[[[[NSAttributedString alloc] initWithString:General/[[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease]]autorelease]];
	}
