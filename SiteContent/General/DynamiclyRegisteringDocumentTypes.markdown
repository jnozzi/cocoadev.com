...
There is the new 10.3 LS function General/LSRegisterURL() (see http://developer.apple.com/documentation/Carbon/Reference/General/LaunchServicesReference/General/LSRReference/function_group_5.html), but that reads from the Info.plist. So maybe you could do something *really* nasty like dynamically write to your Info plist (which may very well fail--there is no guarantee that an app can write to its bundle), otherwise, look for something in Launch Services -- it's the sort of thing that LS would handle I think.

----

I would consider subclassing General/NSDocumentController

makeDocumentWithContentsOfFile:ofType:

- (id)makeDocumentWithContentsOfFile:(General/NSString *)fileName ofType:(General/NSString *)docType
Creates and returns an General/NSDocument object for document type docType from the contents of the file fileName, which must be a fully qualified path. The returned object is not retained. Returns nil if the General/NSDocument subclass for docType couldn�t be determined or if the object couldn�t be created. This method invokes General/NSDocument�s initWithContentsOfFile:ofType: and is invoked by openDocumentWithContentsOfFile:display:.

See Also: � makeUntitledDocumentOfType:, � openDocument:

makeDocumentWithContentsOfURL:ofType:

- (id)makeDocumentWithContentsOfURL:(NSURL *)aURL ofType:(General/NSString *)docType
Creates and returns an General/NSDocument object for document type docType from the contents of aURL. The returned object is not retained. Returns nil if the General/NSDocument subclass for docType couldn�t be determined or if the object couldn�t be created. This method invokes General/NSDocument�s initWithContentsOfURL:ofType: and is invoked by openDocumentWithContentsOfURL:display:.

See Also: � makeUntitledDocumentOfType:, � openDocument:

The General/NSDocumentController class keeps track of the first instance of General/NSDocumentController (or a custom subclass) that is created, and returns that instance from its sharedDocumentController method. To get your application to use your custom subclass of General/NSDocumentController, you must ensure your subclass is the first instance of General/NSDocumentController created when the application starts up. There are two ways to do this.
...
----
I wasn't able to find any hook to "dynamically" register document type information. General/NSDocumentController does, however, appear to be the holder of such information as well as the creator of new documents. Subclassing it is the same approach I use to enhance and otherwise fine-tune the creation of new documents.

-jason
----
Untested: in your General/NSDocument subclass implement the class methods +(General/NSArray*)readableTypes and +(General/NSArray*)writableTypes to look up a General/NSMutableArray that holds the list of file types... does this work?
----
...
----
Yup, I just spent most of the evening with the Launch Services documentation and a lot of other docs I don't remember seeing before. The funny thing is, the method (hack) "modify your own Info.plist and tell the finder about it" is nearly blessed by Apple. The fact that General/LSRegisterURL and General/LSRegisterFSRef exist pretty much says it all.

I put together a method that reads in the Info.plist with the General/NSPropertyListSerialization/General/NSData classes, modifies the resulting General/NSMutableDictionary, reverses the process to write it back out, and then notifies Launch Services that my Info.plist has changed with the afore mentioned General/LSRegisterURL. Haven't tested it yet but it looks like it should work. I am sure I am going to run into trouble with the permissions issue sooner rather than later though.
----
What you mean is that you wrote **infoPlist = General/[[[NSMutableDictionary alloc] init] addEntriesFromDictionary:General/[[NSBundle mainBundle] infoDictionary]] and [infoPlist writeToFile:General/[NSString stringWithFormat:@"%@/Contents/Info.plist",General/[[NSBundle mainBundle] bundlePath]] atomically:YES]** right? you know... using the builtin functionality?
----
No, I mean using the General/NSPropertyListSerialization methods. I tend to code very defensively and don't like leaving loose ends. With the General/NSPropertyListSerialization methods I can be 100% positive that the Info.plist is written back out in the same format in which it was originally read. Currently, there are 3 different Info.plist formats defined.

> *General/[NSString stringWithFormat:@"%@/Contents/Info.plist",General/[[NSBundle mainBundle] bundlePath]*

I use stringByAppendingPathComponent so I don't make an assumption on how the bundle path is formated, and then convert it to an NSURL as the remainder of the methods can use it, and General/LSRegisterURL will use it toll-free as well.

In a nutshell...

    
- General/[NSData initWithContentsOfURL:]
+ General/[NSPropertyListSerialization propertyListFromData:mutabilityOption:format:errorDescription:]

// Party on the General/NSMutableDictionary...
 
+ General/[NSPropertyListSerialization dataFromPropertyList:format:errorDescription:]
- General/[NSData writeToURL:atomically:]

General/LSRegisterURL...
...
*I wouldn't go so far as to say it's "nearly blessed" by Apple -- the docs say those functions are designed for installers, etc. Also why are you using General/NSPropertyListSerialization? when you could just use General/[NSMutableDictionary dictionaryWithContentsOfFile:]? Permissions will be a big problem. If the user doesn't have write privileges on the Application folder, or the app is used in a networked environment, this wouldn't work. Also keep in mind that -General/[NSBundle infoDictionary] is not going to be aware of the changed plist.*

----

Yes, 4 minutes after your post it looks like your contribution was overwritten by someone else also questioning my use of General/NSPropertyListSerialization. It happens sometimes. Unfortunately, the person that overwrote your comments was a bit belligerent about pointing out what they "thought" was obvious.
...
> *Also keep in mind that -General/[NSBundle infoDictionary] is not going to be*
