...
Objective-C is the primary language used for programming with Cocoa.  It is based somewhat on the General/SmallTalk programming language.

(You can also program Cocoa in Java, but there can be speed issues: http://www.bignerdranch.com/Java.html )

Objective-C consists of a set of Object Oriented extensions to the C language, featuring things like General/DynamicBinding and a powerful General/RunTime system. An Objective-C object (General/AnObject) communicates by sending General/ObjCMessages.

General/ObjectiveC is a superset of C. That means that virtually all C-code compiles in an General/ObjC compiler. C++ is another C superset. (Actually, C++ is not a true superset of C which can lead to annoying problems (see below)). The nice thing is that the respective syntaxes of Obj-C and C++ do not intersect. This is where the (arguably unusual) syntax of Obj-C pays off big time because it doesn't conflict with anything in C or C++. That leads the way for General/ObjectiveCPlusPlus, which consolidates all the features of the three languages (albeit with some growing pains evident now and then).

We (users of General/MacOSX) compile Objective-C using the General/GnuCompilerCollection. There are other compilers (albeit not many), including the General/PortableObjectCompiler.

General/ObjectiveC was created by General/BradCox - http://www.virtualschool.edu/cox/ - what a hero ! Esse ? o cara !

----

Apples General/ObjC docs at [http://developer.apple.com/documentation/Cocoa/Conceptual/General/ObjectiveC/index.html]
...
Effective Use of the Objective-C Java Bridge Within Project Builder/Xcode: General/XcodeObjCJavaBridge -- General/AlainODea

----

I've added a tiny page basically referencing the Apple doc's on Messages, because I forgot that [ ] is the syntax for messages, so I thought someone else might have also. It's at General/ObjCMessages.

----

Anyone learning Objective-C may like to check out this archived, freely available General/NeXT manual:
...
It is a good introductory text, from a simpler era, but still surprisingly relevant. - General/AlexClarke

*what the hell is wrong with that page? the linked docs are in like a 48 point font*

----

The manual above from the General/NeXT days is just the older version of the apple doc cited further above !
...
It's good to know we now have the old General/NeXTStep and the old Mac OS in one and the same OS, the ultimate OS X !
...
http://developer.apple.com/documentation/General/DeveloperTools/Conceptual/General/CppRuntimeEnv/Articles/General/CPPROverview.html)
...
Leopard will introduce significant updates to Objective-C, referred to officially as "Objective-C 2.0", and unofficially as C*. The following information is gleaned from public sources: the compiler source code (http://gcc.gnu.org/viewcvs/branches/apple/trunk/gcc/objc/) and compiler test cases (http://gcc.gnu.org/viewcvs/branches/apple/trunk/gcc/testsuite/objc.dg/). I had not have access to privileged information at the time of writing. --General/JensAyton
...
*     @required and     @optional keywords for General/FormalProtocols.
...
* Warnings will be generated for use of     @defs and implicit typecasts of General/SELs, saying these "will not be supported in future."
...
*I notice you're using the property call on m to set the "baz" property to a number.  Does that mean the number is getting General/AutoBoxed?*
...
Do I need to protect my Cocoa app's General/ObjC methods from being called by other apps or processes, and if so how?  My app will surely malfunction badly if that happens!   Thanks --General/DarelRex@gmail.com
...
----
Yes, it's called General/DistributedObjects (accessed via General/NSConnection), and it doesn't happen unless you explicitly set it up.
...
Do synthesized properties need to be looked at at all in -dealloc, or does General/ObjC release them automatically when an object deallocates?
