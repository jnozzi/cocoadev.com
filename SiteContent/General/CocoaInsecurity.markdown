...
* And most importantly: '''STRIP YOUR FINAL EXECUTABLE OF ALL SYMBOLS BEFORE PACKAGING''' This is done using the UNIX program called ''strip'' which comes with OS X. To view the man page, open the Terminal and type <code>man strip</code>. You can also use [[ProjectBuilder]] to view the man page in prettier style. Stripping all non-global and non-external symbols (which include your application's references to the [[AppKit]] and [[FoundationKit]], as well as to any other externally linked frameworks you may use) ''should'' keep prying eyes from class-dumping your program's executable to file to view all of your classes and methods.
