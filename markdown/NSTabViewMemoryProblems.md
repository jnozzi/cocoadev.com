...
I've recently begun work on tabs for the browser that I'm working on. I have an General/NSTabView in my document window, tabless, and a General/TabViewController which is my custom General/NSView which handles the display of the tabs themselves. The initial tab is empty. My problem right now is that I'm getting Signals 10 and 11 on startup relating to my General/WebView being over-released, or so it appears. Right now, my     - (void)windowControllerDidLoadNib:(General/NSWindowController *) aController method is as such:

    
[super windowControllerDidLoadNib:aController];

webView = General/[[WebView alloc] initWithFrame:[tabView frame] frameName:@"Tab Frame" groupName:@"Group Thing"];
[webView setUIDelegate:self];
[webView setFrameLoadDelegate:self];
[webView setResourceLoadDelegate:self];
General/NSString *urlText;
if (queuedUrl == nil)
	urlText = [Preferences objectForKey:General/JBHomePage];
else
	urlText = queuedUrl;
General/webView mainFrame] loadRequest:[[[NSURLRequest requestWithURL:[NSURL General/URLWithString:urlText]]];
General/tabView tabViewItemAtIndex:0] setView:webView];



In addition, I have 4 toolbar items whose target is webView, which they retrieve from the document using an accessor method. No other part of the program directly accesses the webView. Right now, what's wrong is that the toolbar's trying to call the [[WebView<nowiki/>'s validateMenuItem method to determine whether these toolbar's actions (stopLoading:, reloadPage:, goBack:, goForward:) should be enabled or disabled, but the webView has been deallocated. I tried modifying my - (General/WebView *)webView; accessor method to return a retained copy but then the toolbar items don't validate correctly - in fact they never turn on. I don't retain, or release, the webView anywhere else. I also tried doing General/tabView tabViewItemAtIndex:0] setView:[webView retain but that doesn't work either.

I understand the basic fundamentals of memory management and usually don't run into troubles with them - in this case, it appears that somewhere, the functionality of either the General/NSToolbar or General/NSTabView class is performing releases that I don't approve of. Or perhaps the General/WebView's General/WebFrames are doing something as well - when I ran it though the General/OmniObjectMeter program I saw cases of the General/WebFrame class retaining and releasing the General/WebView.

In my test program, where I was designing the tab controller, I had a method which added a tab and inserted a new General/WebView using this same code (except the delegate stuff) as the subview of the new General/NSTabViewItem and it worked fine. I think something in the toolbar code is releasing my General/WebView, but I'm not sure what. Does anyone have any ideas?
...
I have the feeling you're not going to solve your problems until you look at what's going on in a debugger. A few tips though-- you can call General/tabView selectedTabViewItem] view] to get the currently selected view. Also, you'll certainly want to look into the responder chain for validating and enabling your toolbar and menu items. It's the best way to handle something like what you're trying to do.
...
I don't usually retain objects when I return them, I'm just trying to figure out how to fix this problem. I've run it through the debugger, but the problem is that somewhere my General/WebView is being released that I don't have control of. All I want to do is make a General/WebView and stick it in a General/NSTabViewItem and let a toolbar control it. I've used the debugger and looked into it but as I said before, the program crashes on startup unless I retain a new copy of the webview every time it's accessed which is just an excessive way of fixing the zombie problem. All I know is that between the time the program starts and the time it crashes, it allocates one, and some code *somewhere* releases it into oblivion. Also, how would I use the responder chain to validate and enable toolbars and menu items? I don't understand how that would work, maybe I just don't understand how validation works, but I thought that the target of the action validates the toolbar item/menu item. I think I know how to use the responder chain for other actions, just not validation... Do you just set the target of the action to nil, and let it figure out what responds to that particular action, and if it responds to that action, use that responder's validate method? Also, will it always pick the front-most tab's General/NSResponders before other tab's General/NSResponders?

Also, when I was looking into subclassing General/NSTabView, I didn't see anything about drawing the tabs, or the structure of the view at all, so it seemed like if I were to subclass it, I'd need to rewrite the drawing code for the entire thing anyway.

----
A few suggestions:

0) Once you have made your newly allocated General/WebView the view of a tab view item, you presumably don't need it anymore.  The tab view item takes ownership.  Try this
...
1) Override -release in a subclass of [[WebView, use that subclass in your tab view, set a breakpoint in your -release method, and know for sure what the call stack that releases your General/WebView is.  Remember that lots of framework classes may retain and release/autorelease you web view and you shouldn't be concerned.  Just  look for the surprising release/autorelease coming from your code.

2) Correct the evidently non obvious violation of the memory management rules in your code.

3) Go back to using the base class General/WebView.

For tool bar validation:

You seem to have a start on understanding the responder chain.  Look into General/NSApplication's -targetForAction: method and read about the way menu items are automatically validated.  Use a the same or similar technique with your tool bar buttons.

For custom General/NSTabView drawing/behavior:

Look at General/NSTabViewItem http://developer.apple.com/documentation/Cocoa/Reference/General/ApplicationKit/ObjC_classic/Classes/General/NSTabViewItem.html
...
2006-01-10 17:32:10.481 General/InterfaceTester[13725] <General/WebFrameView: 0x3453c0>
2006-01-10 17:32:10.481 General/InterfaceTester[13725] <General/WebView: 0x3430d0>
2006-01-10 17:32:10.481 General/InterfaceTester[13725] <General/NSView: 0x326990>
2006-01-10 17:32:10.481 General/InterfaceTester[13725] <General/NSWindow: 0x365b50>
2006-01-10 17:32:10.482 General/InterfaceTester[13725] <General/NSWindowController: 0x341290>


I then click a link. I wait for it to load, click the button again, and get:

    
2006-01-10 17:32:23.729 General/InterfaceTester[13725] <General/NSWindow: 0x365b50>
2006-01-10 17:32:23.729 General/InterfaceTester[13725] <General/NSWindowController: 0x341290>


I was able to partially solve this in the browser by sending makeFirstResponder to the window whenever the first resource is loaded in a provisional load, but the buttons sometimes turn off momentarily. I was thinking about possibly doing something with General/NSWindowController but I'm not really sure where to go from here. I think the problem is that the actual first responder is the General/WebFrameView, which is being released by General/WebView when the URL changes. Perhaps I should break this off into a new dicussion?

---

This looks like a bug in General/WebView, or at least unexpected behaviour. You should be able to fix it by adding the following code to your controller, and making that controller the General/UIDelegate of the webview (either programmatically or in IB). 
