...
All General/ProducersAndConsumerModel<nowiki/>s have the potential for General/PriorityInversion.

One way to implement this is using General/DistributedObjects, but the overhead of DO is not negligeable and some situations may require a much more responsive, lower-cost solution.

----**The portable solution**----

A simple solution can be written with General/NSConditionLock. We want the consumer thread to block if the queue is empty, waking only when the queue becomes full again; we will use condition 0 when the queue is empty, and 1 when the queue is full.

    @interface General/ThreadSafeQueue : General/NSObject {
  General/NSMutableArray* elements;
  General/NSConditionLock* lock; // 0 = no elements, 1 = elements
}
...
@end
    ...
    @implementation General/ThreadSafeQueue

-(id)init {
  if (self = [super init]) {
    elements = General/[[NSMutableArray alloc] init];
...
-(id)dequeue {
  ...
  id element = General/[elements objectAtIndex:0] retain] autorelease];
...
Help is at hand, and its name is, again, General/NSConditionLock. The *condition* we want the lock to monitor is *the presence or absence of data on the atomic stack* - more specificially, whether data has been inserted on the stack since the last time the consumer thread ran dry. We will denote a dry consumer thread by condition 0, and new data added to an empty queue by condition 1.

The two General/AtomicSQ functions we need are General/AtomicSQPush and General/AtomicSTPQueuePop. We will craft two new functions around these to do the condition lock code.

**General/KTConsumeEntry**
...
General/NSConditionLock *consumerQueueLock;
...
struct message *memory = General/KTConsumeEntry(&consumerQueue,
                                        &localCache,
                                        consumerQueueLock);
...
This code should be perfectly thread-safe now; if the queue is empty, General/KTConsumeEntry will sleep the consumer thread until data turns up.

The implementation:

    
void *General/KTConsumeEntry(void *volatile *consumerQueue,
                     void **lS,
                     General/NSConditionLock *emptyQueueLock)
{
    void *data = General/AtomicSTPQueuePop(consumerQueue, lS);
    while (data == NULL)
        {
        [emptyQueueLock lockWhenCondition:1];
        [emptyQueueLock unlockWithCondition:0];
        data = General/AtomicSTPQueuePop(consumerQueue, lS);
        }
    return data;
}


The code is a simple extension of General/AtomicSTPQueuePop. Given a queue (with an associated thread-local stack pointer) and a condition lock, we first try popping data off the queue. If there is none, we sleep on the condition lock until another thread indicates there is data available. Once done, we reset the condition lock and pop the data off the queue.

Note that this means we must initialize the condition lock in condition 0, but this is reasonable as General/NSConditionLock's init method does this automatically.

**General/KTProduceEntry**
...
// Do something with memory
General/KTProduceEntry(&consumerQueue,
               consumerQueueLock,
               &memory);
memory = NULL; // Make sure we never touch memory again


This code should also be perfectly thread-safe; if the consumer thread is asleep, General/KTConsumeEntry will wake it up to handle the data.

The implementation:

    
void General/KTProduceEntry(void *volatile *consumerQueue,
                    General/NSConditionLock *emptyQueueLock,
                    void *data)
{
    if (General/AtomicSQPush(consumerQueue, data))
        {   // Wake the consumer
        [emptyQueueLock lock];
...
This method is even simpler. Given a consumer queue, a condition lock and an entry to put on the queue, we let General/AtomicSQPush do the hard work of the atomic push. If that function returns YES, the queue was empty when we added the data, so we need to rock the condition lock to condition 1 to wake the consumer thread (or prevent it sleeping, if we are quick enough). Use -lock instead of -lockWithCondition: here is deliberate.

**Provisos**

There are several provisos for this code.

Firstly, since it relies on the General/AtomicSQ code, it is currently tied down to the General/PowerPC platform, though the Intel x86 platform (and indeed most modern processor architectures) is perfectly capable of implementing the General/AtomicSQ functions used.

Secondly, General/KTConsumeEntry is an "aggregate constant time" method - that means that over many runs, the time taken to execute it will average out as constant. However, there may be occasional latency issues as the code runs down a long queue of stored data that has just been retrieved, looking for the end. (An General/NSConditionLock alternative could be coded that is true-constant time, give or take the latency in waking a thread.)

Thirdly, this code has now been tested, but an exact comparison with alternative methods of implementing the General/ProducersAndConsumerModel has not been carried out. Theory suggests this code should be pretty optimal.

Finally, don't forget the many warnings on this site about the inherent problems of General/SynchronizationPoints; both the methods above have them, so if you find you are calling them a lot you may wish to redesign your code to transfer more in each individual item on the queue, or use other optimizations.

-- Refactored by General/KritTer; comments appreciated

----
What about performSelectorOnMainThread ?
Does General/AtomicSQ still works on G5?
...
--*The following discourse was moved from an unrelated page entitled General/CodingAnEfficientProxyObject *

It's an interesting idea, Kritter, and you were pretty enlightening on this case. Did you ever see my posts on the cocoadev list or the JediKnight2 source code? The way he did the push from 1 thread to another was more efficient than DO, but it still involved an archival, unarchival, and the use of an General/NSPort. 

I don't suppose you could whip up a magic trick to efficiently call a selector on an object in another thread with data? :)

-- General/DaveFayram

Not I. Switching threads is beyond me. You can happily just pass the address of the fast proxy, though, if that speeds up the code - no archive/unarchive needed. But then, you could just do an General/NSCopy and swear not to touch the copy and have the same thread safety.
...
To read the next item off the stack, you atomically swap the top pointer for what it points to (the next item) unless it's NULL in which case you sleep the thread then try again. That makes for a fairly trivial asm function, barely more complicated than the General/AtomicLockSwap.
...
Well, the code's up on General/AtomicSQ, though I intend to make a more efficient STP queue. What would you use it for? Suppose you want efficient communication between threads; create a comms queue for each thread and pass structures to them from other threads without the overhead of standard General/ObjC thread communications. Or say you want a shared heap of chunks of memory; just push them all on an atomic stack and pop them off as needed, pushing them back when you're done. It's all about coordinating threads without messing about with expensive overheads - something you need if your threads talk a lot - and unnecessary point of synchronization (when one thread has to stall waiting for another thread to respond) on multi-processor systems.

(As a point of interest, this latter can be minimized by the use of <code>oneway void</code> in protocols; this keyword stops one thread stalling for a response from another when using a standard General/NSProxy to send General/ObjC messages between them.)
...
Firstly, as I mentioned in General/SynchronizationPoints, *any* kind of shared memory is expensive in one way or another, and the more General/SynchronizationPoints you have the worse your performance will be, regardless of how efficiently you craft your code.
...
Essentially, the problem boils down to this: you want a thread that has nothing in its queue to sleep, allowing other threads more processor time. If another thread adds to its queue, you want to wake the thread up again. These are non-trivial operations that require OS-level support, and I do not possess that knowledge. If I do, I will write a complete page about using atomics, etc., to create an efficient producer-consumer implementation. Until then, you will either have to use the General/NSPort-based solutions or put up with threads wasting processor time (spinning) when they have nothing to do :)

-- General/KritTer

*Until then, you will either have to use the General/NSPort-based solutions or put up with threads wasting processor time (spinning) when they have nothing to do :)*

This isn't a correct statement. If you have a thread blocked waiting on something like an General/NSConditionLock it isn't taking any CPU time. Also folks seem to be overly concerned with locking overhead when nine times out of ten it likely wont be the major source a performance problem.

-- Shawn

----

**Example using General/NSCondition**
...
-(id)init {
    if (self = [super init]) {
        elements = General/[[NSMutableArray alloc] init];
        lock = General/[[NSCondition alloc] init];
    }
    return self;
}
...
-(id) dequeue {
    ...
    element = General/[elements objectAtIndex:0] retain] autorelease];
    [elements removeObjectAtIndex:0];    
    [lock unlock];
...
----
Why are you using the private (and therefore unsafe) [[NSCondition instead of the public General/NSConditionLock? **[This no longer applies as of 10.5. General/NSCondition is now a public class.]**

----
Sorry to stir an old thread, but what about using General/CFRunLoop and General/CFRunLoopSource? Using the General/NSThread General/APIs will create a new run loop for you, and then General/CFRunLoopSourceSignal and General/CFRunLoopWakeUp can be used to signal new input from the producing thread. General/CFRunLoop sleeps when it is waiting, so it does a lot of the hard work that General/KritTer mentioned. The same shared-memory queues can then be used as above.
