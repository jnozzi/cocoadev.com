http://developer.apple.com/documentation/Cocoa/Reference/General/ApplicationKit/Classes/NSPopUpButton_Class/

----

Related pages:

*General/DynamicMenuForNSPopUpButton
...
After trying to deal, off-and-on for some period of time (it became my Holy Grail quest) with the problem of General/NSPopUpButton and reflecting the value of the General/NSPopUpButton selecting in a General/NSTextField in real-time, even after working on this with someone with much more Cocoa experience than I have (we are talking over a decade here and has authored books on Cocoa programming), the conclusion is that Cocoa Bindings are not appropriate, at least not at this juncture, for this task. And given that this can be coded using the old-fashioned Controller in about...oh, five minutes, it doesn't seem worth the effort. I have nixed the code that was here since, first it was mine and because it will only confuse people. I am leaving the text of the problem here. If anyone wants the code that was here, just email me at jdhouse4@cocoacoder.org. I'm disappointed that General/CocoaBindings is, compared with General/CoreData, so lacking in so many things that developers need, from decent debugging feedback (there isn't any) to the real inelegance of it all. As a comparison, look at the way we use General/CoreData and General/CocoaBindings. The experience is worlds apart, from nice to horrid. I hope in the future that those on the General/CocoaBindings team will work to make using (and debugging) General/CocoaBindings as elegant as General/CoreData.
...
Original post (moved from General/BindingQuestions):
For those of you that are advanced Cocoa programmers and know how to use Bindings, my apologies, but I am at a loss. Another caveat; it's been a couple of years since I programmed in an General/NSPopUpButton. Anyway, I submitted this to Apple's General/CocoaDev mailing list and got back...silence. By comparison, this place is hopping, so here goes.

To demo how to make General/NSPopUpButtons work in Cocoa using Cocoa Bindings, I made a very simple Cocoa App called aptly enough "General/PopUpButtonDemo" that does nothing more that display someone's age when you select their name from the pop up button (General/NSPopUpButton).

There is only one Class, "General/PopUp". Some of the code, namely the first two Class methods in the implementation file for General/PopUp.h/m I copied from Malcolm's "General/ToDo" app Controller.h/m and then changed it. That was a bad idea because his works--mine doesn't.

Right now, on the UI I just have the General/NSPopUpButton on the Window because I wanted to see the list of names before I went off and made the problem more complicated.

I used an General/NSArrayController, General/PopUpController, because it's my understanding that a General/PopUpButton is handled like an array. The General/PopUpController has as its attribute the Class General/PopUp and a contentArray Binding to General/FileOwner Model Key Path "popUps". The General/NSPopUpButton itself is bound as follows:


* content = arrangedObjects General/[PopUps (General/NSArrayController)]
* contentValues = arrangedObjects.popUpName General/[PopUps (General/NSArrayController)]
...
since there is no other object. General/PopUp is alone.

The General/NSArrayController "General/PopUps" is bound to 

    contentArray = popUps [File's Owner (General/NSApplication)]

When I run the app, I keep getting an error:

[<General/NSApplication 0x3136e0> valueForUndefinedKey:]: this class is not key value coding-compliant for the key popUps.

But I did create the General/NSArray popUps in General/PopUp.h. At least, in my code, I have an General/NSArray that is created and called...popUps!

----

Your contentArray should be bound to the popUps method in your General/PopUp object, not your File's Owner (General/NSApplication).
...
<pre>
File: General/PopUp.h
#import <Cocoa/Cocoa.h>

@interface General/PopUp : General/NSObject 
{
    General/NSArray *popUps; // !
    General/NSString *popUpName;
    double popUpValue;
}
+ (id)popUpWithName:(General/NSString *)aName andPopUpValue:(double)aValue;

- (double)popUpValue;
- (void)setPopUpValue:(double)aValue;
- (General/NSString *)popUpName;
- (void)setPopUpName:(General/NSString *)aName;
@end

File: General/PopUp.m
#import "General/PopUp.h"

@implementation General/PopUp
 + (General/NSArray *)allPopUps
 {
// !     static General/NSArray *popUps;
	 popUps = General/[[NSArray alloc] initWithObjects:
	     General/[PopUp popUpWithName:@"Jim" andPopUpValue:43],
	     General/[PopUp popUpWithName:@"Tony" andPopUpValue:40],
	     General/[PopUp popUpWithName:@"Summer" andPopUpValue:29],
	     nil];
     return popUps;
 }
...
+ (id)popUpWithName:(General/NSString *)aName andPopUpValue:(double)aValue
{
    id newPopUp = General/[self alloc] init] autorelease];
...
*Maybe that'll work? -- General/RyanStevens*
...
----
I am confused what the "otherArrayController" is compared to the popupArrayController. From mmalc's To Do example (where the above code comes from), I can see where the 2nd General/NSArrayController comes from. What I am not sure about yet is if only one General/NSArrayController can be used as well, so that selectedObject is bound to popupArrayController.selection.attribute instead.
...
----
Note: I just found out (on 10.4.3) that if you create a dynamic menu for your pop-up button, create the menu items with a nil action or the actions applied through the General/NSPopUpButton setAction: method will not be called. IE:

    
	General/NSMenu * menu = General/[[[NSMenu alloc] init] autorelease];
	General/NSEnumerator * anEnum = General/self surfaces] objectEnumerator];
	[[ASurface * surf;
	while(surf = [anEnum nextObject])
	{
		id<General/NSMenuItem> menuItem = [menu addItemWithTitle:[surf name] 
	...
	General/NSPopUpButton * button;
	while(button = [anEnum nextObject])
		[button setMenu:General/menu copy] autorelease;
	
	[self redisplayMenus];


While an action was specified in the     -addItemWithTitle:action:keyEquivalent: call, any later calls to the General/NSPopUpButton -setAction: method does not work as expected, but if nil is specified as the original action, then all is well. I do not know if this is documented behavior, but it is confusing, and I spent too much time solving it!
-General/JeremyJurksztowicz
----
Even if it's confusing, it's a feature, not a bug. This way you can create menu items that all have different actions, even from the popup button. For example, this way you can hypothetically have a Back button in a browser that sends a     back: action when clicked, but a     goto: action when a menu item is selected. --General/JediKnil

----
Hi, Does anyone know how to programmatically create an General/NSPopUpButton that's small?  In IB you can choose 3 sizes, along the lines of normal, small, and mini?  I'd like mini but I can't find any function in either General/NSPopUpButton or General/NSButton that involve setting the size style.  Does anyone have any advice?  (Incase it's unclear I don't want the button to be just small by setting its size with an General/NSRect.  There are different kinds of sizes along the lines of large and small scroll bars.)
...
*General/NSCell has -setControlSize:.*
...
//
[menuButton setFont:General/[NSFont systemFontOfSize:General/[NSFont systemFontSizeForControlSize:General/NSMiniControlSize]]];
//

-General/JasonSwain

----
That code alone will not work without calling     setControlSize:. All it does is change the font. If you're claiming that this alone is sufficient to make a small General/NSPopUpButton (which is what I understand from what you're saying), I would love to see a complete example so I can verify that it works.
----
...
----
Note for folks who have General/NSPopUpButton pull-down menus: *never* remove the menu item at index 0 on the menu.  You will get very, very weird results if you do this, and probably a lot of head-scratching to go with it.  (This is mentioned in the documentation, but only in a single sentence.)  -- General/AndrePang

----
Hi, does anyone knows how to populate an General/NSPopupButton with General/NSAttributedStrings using cocoa bindings. It seems like an impossible task using default bindings (contentValues only binds to an array of strings, but not attributed ones). The idea behind this is being able to display pictures along the text entries in the popup menu. I guess I could just use the old-fashioned controller approach but I wanted to implement this using bindings.

My guess is that I need to subclass General/NSPopupButton to add a new binding. Any ideas ? -- General/PierreChatel
...
----
This little tutorial shows you how to use fixed values in General/NSPopUpButtons with bindings. You know, an "m" = "male" and "f" = "female" kind of thing: http://blog.springenwerk.com/2009/09/nspopupbutton-with-fixed-values-and.html
