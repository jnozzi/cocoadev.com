...
This is my data method (the program is using the [[NSDocument architecture). 'model' is a instance of a class called General/MatrixData which I've implemented. It holds 13 General/NSMutableArray instances used by the program. This method comes from the General/MyDocument that General/MatrixData stores arrays for.

    
- (General/NSData *)dataRepresentationOfType:(General/NSString *)aType
{
	General/NSMutableArray * test = General/[[NSMutableArray alloc] init];
	[test addObject:@"AH"];
	printf("Setting:\n");

	/*
		'model' is a instance of a class called General/MatrixData.
		This should set a General/MutableArray in the General/MatrixData class to test. 
	...
	*/
	...
	//This prints out null. It should return the test I just made.
        ...
        printf("P0 in data. %s\n", General/[model returnRow:0] description] UTF8String]);
...
I declared a new      General/MyDocument  called temp and unarchived into that instead. It didn't make a visible difference in execution, however.
...
Here's loadData. I've changed this a bit. setData sets the General/MatrixData object for the document (the thing that was saved before). displayMatrix refreshes the window. Unfortunately, if I print out an array from the General/MatrixData object after loading using printf, it comes out not as null, but as blank.
    
- (BOOL)loadDataRepresentation:(General/NSData *)matr ofType:(General/NSString *)aType
{
	[self setData:General/[NSKeyedUnarchiver unarchiveObjectWithData:matr]];
        [self displayMatrix];
	return YES;
}
...
----
It is a document-based application.  'model' is declared in the General/MyDocument class .h file,      General/MatrixData * model; with no extern tag, and initialized and instantiated in the .m  file, in a method called generateMatrix which gets called when the user asks the program to do something (it's a simple program). At first, I was doing the initializing and instantiating in General/MyDocument's init method, but I read another topic here about saving where the open command was calling the class' init and wiping the loaded variables clean, so I moved it.
----
...
Do all documents share the same model? In that case, a proper place to put it is probably in the General/MainMenu.nib.

*Or in a General/NSDocumentController subclass*
----
I agree, I think some design review is in order to hash this out properly.

> *It is a document-based application. 'model' is declared in the General/MyDocument class .h file with no extern tag.*

Technically, that's not safe, as every file to include/import "General/MyDocument.h" will get a fresh (read uninitialized) General/MatrixData pointer named *model*. The compiler/linker may complain, but that would be a fairly project specific assumption.

If you want to keep it in the General/MyDocument files; I would move it out of the .h file or tag it with extern in the .h file and add it to the .m like so.

*General/MyDocument.h*

    extern General/MatrixData * model;

*General/MyDocument.m*

    General/MatrixData * model = nil;

----
General/MyDocument is the main class. It's the one doing the saving. However, when I try to print the data there, it comes out blank. It also comes out blank in General/MatrixData's encodeWithCoder method, which is doing the archiving.

-KH
----
It seems that in many cases you are accessing the P0-n arrays in General/MyDocument and not in General/MatrixData, but it is the General/MatrixData object which you encode and later decode.

----
I would strongly suggest that rather than spending your time trying to find the problem, you simply start from scratch with a "cleaner" design. I.e. where there is only the model object and not half the code in General/MyDocument and the other half in General/MatrixData.
...
@interface General/MatrixData
{
   General/NSMutableArray* rows[NUMBER_OF_ROWS];
   ...
}
...
@end
...
for(size_t i = 0; i != sizeofA(rows); ++i)  // sizeofA is from /usr/include/c.h
   rows[i] = General/[[NSMutableArray alloc] init];

Which, judging from your code, will remove **a lot** of lines.

I also noticed that your code passes General/NSMutableArray around and retains it when it needs to keep it. This is not very safe, as the caller may later change the array (which I think is actually the case in your code).

----
I created a new project and rebuilt a lot of it. This got rid of a lot of weird errors, but not the save error. I cleaned it up so that General/MatrixData is the model, General/MyDocument the controller, etc just have methods for calculation that General/MyDocument actually prints the screen. Now, saving works just by having General/MyDocument archive the General/MatrixData object with a General/NSKeyedArchiver. Unfortunately, the data still prints as blank. Not even nil, just blank.
...
Save... is writing a file to disk with a 8 KB mass (there's not much to save) and with the proper icon, extension, etc. However, the data that I try to print in the code (see the dataRepresentation... method in General/MyDocument) comes out as blank.

- KH

----

You are instantiating another instance of General/MyDocument in the General/MyDocument.nib. **Don't** do this, because General/MyDocument is already being instantated as the File's Owner. To fix this, remove the General/MyDocument instance in General/MyDocument.nib and link everything up to File's Owner instead. Because there are two documents, there are two sets of data, and the File Owner's (the real document's) data never got set and therefore was always blank when saving. As a side note, you should really consider cleaning up the code and learning some good memory management (release & retain). You should also never call "dealloc" directly on any object except "super". -- General/RyanBates

----
Saving and loading works just fine now. I could've sworn my books created new instances though.

The dealloc stuff was a typo that I fixed, thanks. As for the memory management, I thought I had cleaned most of that up, since the arrays are set and gotten via setter methods in General/MatrixData. The C-arrays would probably help so I don't have to reference stuff 12 times each.

BTW, is there a quick and easy way to generate a custom General/NSView for printing? I tried making a custom View in interface builder that encapsulated the portions of the window I wanted to print and then making it outlet to the General/MyDocument, but something's up with the printing. The print panel comes down correctly, and a job is being sent to the printer, but no page is ever printed, I'm assuming because it's getting no data. I'm using

    -(void)printShowingPrintPanel:(BOOL)showPanels

generating a an General/NSPrintOperation, and then sending it to 
