I am trying to run a very simple multithreaded tool. I have an array of jobs, where a job is simply some calculation I can start with     [job runJob], which takes several minutes on average for each job. To take advantage of multiple processor, I decided to make this very simple     General/MultiJob class. I am not looking for fancy multithreading here. I know there are plenty of super-robust multithreading classes out there, but I want to understand what is going on in my particular class.
...
Thanks for your help! General/CharlesParnot
...
@interface General/MultiJob : General/NSObject
{
	...
	int nextJobIndex;
	int threadCount;
	General/NSLock *myLock1,*myLock2;
}
...
@implementation General/MultiJob
...
- (id)initWithJobs:(General/NSArray *)anArray
{
	...
	if (self!=nil) {
		...
		nextJobIndex=0;
		threadCount=0;
		myLock1=General/[[NSLock alloc] init];
		myLock2=General/[[NSLock alloc] init];
	}
	return self;
}
...
- (void)decreaseThreadCount
{
	...
	threadCount--;
	[myLock2 unlock];
	General/NSLog(@"threadCount = %d",threadCount);
}
...
- (void)runJobs:(id)sender
{
	...
	General/BCKJob *aJob;
	pool1=General/[[NSAutoreleasePool alloc] init];
...
*Oups, my comments in the code are a bit misleading. I meant 'put the main thread to work on those jobs'. This is why I start with i=1 (see below), the main thread being the extra thread and being treated just like any other thread... at least until it is finished.* General/CharlesParnot
...
*Again, I should have commented that better. I start the loop at 1, because I include the main thread as one of the thread in threadCount. It is a bit confusing, but I really want this class to be just working, and not doing anything else like run loops and thread messaging. I guess not including the main thread in threadCount could also be confusing, anyway.* General/CharlesParnot

�General/DustinVoss
----
Actually, the counter and two locks can be replaced with a single General/NSConditionLock (semaphore).
...
*Thanks for understanding me so well!! You are right, this is what I meant.* General/CharlesParnot

One bug I noticed is that General/MultiJob's dealloc method does not call [super dealloc].
*Arrrgh. I keep forgetting this one in half of my classes. Thanks.* General/CharlesParnot

Again, this could be cleaned up greatly with an General/NSConditionLock (hypothetically named m_threadLock here) initialized to the thread count. The loop waiting for the threads to exit would be replaced with      [m_threadLock lockWhenCondition:0], [m_threadLock unlock] . Before the thread method exits, call      [m_threadLock lock], [m_threadLock unlockWithCondition:[m_threadLock condition] - 1] .

*Hah, I *knew* there was a semaphore in there somewhere! I had forgotten that General/NSConditionLock takes integers.*

----
Finally, after more than 24 hours of inactivity, I got what I needed. This site is great. Thanks very much!
I thought I would have to use General/NSConditionLock, but things were still quite unclear to me. I think I get it now, and will include the revised code for review below after testing it. It might make a great example of General/NSConditionLock, too, so I will probably refactor this page to link it in the General/NSConditionLock page on this wiki. General/CharlesParnot
...
*Thanks so much again for your explanations and help. This compiler problem is really all I deserved from not trying to do things right. Anyway, I posted the corrected code (and hopefully correct) as an example of General/NSConditionLock, in General/NSConditionLockMultiJobExample.* --General/CharlesParnot