...
This is an archive copy of the now retired C**'ocoaMostWanted page. See General/CocoaDevelopersMostWanted for a list of the current "most wanted" pages.
...
Your wish is my command. Here is a new page: General/CocoaDevelopersMostWanted � go there and help me make style guidelines. I'll try to start with an outline, sub-pages, and a couple of items. �General/BrentGulanowski

Page size was over 32k - as an interim fix I moved the Fulfilled Wishes section to General/CocoaMostWantedFulfilledWishes and the Denied/Impossible Wishes section to General/CocoaMostWantedDeniedImpossibleWishes
----**Cocoa / General/ObjC**----


* *Regular Expression Suport*. (Votes: AYES: 4 NAYS: 0 MAYBE: 0) 
How about adding built in regex support to Cocoa?  That way people wouldn't have to defer to third party options such as embedding perl code, or using General/MoKit, etc.  This seems like something which is useful enough that it should be built in.  Even languages like General/REALBasic have this included! 

:What about regex.h?  Try man regex.


* *Give us some Apple sanctioned means of running Cocoa apps on other platforms.* (General/YellowBox for Windows, OS X on other platforms, or whatever). (Votes: AYES: 15 NAYS: 6 MAYBE: 1) - This would really make Cocoa more desirable! And could also cannibalize Apple's hardware sales.

Hum, nobody knows the existence of the General/GNUstep project http://www.gnustep.org ?
If you use General/OpenStep and not too much Apple extensions it's not too hard to have a code compiling both on Mac OS X and on General/GNUstep (linux/bsd, windows and beos in (slow) preparation)

gnustep is barely a blip of activity.

I think cross-platform Cocoa can only help them expand mindshare - possibly even help them when they *cough* port Mac OS X to x86 hardware *cough*. If anything, more people will be willing to code with it, thus more apps will run on the Mac OS platform. --General/JLOwens
...
* *Allow General/NSToolbar positioning on top or left of window* (AYES: 2 NAYS: 0)

Wouldnt' work for all things, but if all you're using a toolbar for is file stuff & drawers the option is nice

* *Add automatic General/GarbageCollection* (Votes: AYES: 7 NAYS: 16 FENCE: 2) [see General/GarbageCollection]

* *Objective-C General/EnterpriseObjectsFramework!* (Votes: AYES: 10 NAYS: 0) 
...
A namespace is a formal grouping of object names, such that you can refer to the group as a whole by a single name. A class is a namespace. What's being asked for here is the ability to have arbitrary namespaces, so that you could have a class named, for example, General/MyName::Object instead of General/MNObject. There is also in some languages a way to say "assume that I'm working in the General/MyName namespace," so that you don't have to type "General/MyName::" as a prefix in the implementation of General/MyName::Object. This is far more flexible and scalable than the two-letter prefix which currently disambiguates different names.

*Add the undocumented General/NSMethodSignature method +signatureWithObjCTypes: to the API. More generally, make the creation of arbitrary utility objects easier! (Votes: AYES: 2 NAYS: 0)
...
What are you smoking? Brad Cox himself added closures to Objective-C twelve years ago. It just never propogated up to General/NeXT. See General/HigherOrderMessaging on this site for examples of what it would be used for.

Speaking of General/HigherOrderMessaging, I'd like to see Apple add it to their API's.  jd
...
* * Improve General/NSAffineTransform/General/CGAffineTransform to allow for Projective Transformations.*
...
Easy workaround: Select the sound file, choose "Get Info...", select "General/QuickTime Player" in the "Open with..." area, click the "Change for all" button. Suddenly, all such sound files will be played by General/QuickTime player, which doesn't add anything to any library.
...
At least, give us General/NSAlias class.
Though I do agree with a poster to General/CocoaDev who said that aliases should check paths first, rather than last. 

*note from General/PaulBayley
An alias is an arbitrary description of how to find a file. You can resolve an alias any way you wish. If you want an alias to behave like a symlink you just have to remove (make null) the General/FileID and host General/FolderID.

Paths ought to be replaced with not only Aliases, but General/FSRefs (or equivalent) depending if the file primitive is being stored (or used for IPC), or if the file primitive is being used at runtime respectively. When I use General/FSRefs for my runtime file primitive structure it doesn't loose track of the file, even on UFS volumes. If I tried to do the same with Aliases I don't believe it would work.
...
* *Un-deprecate the deprecated General/NSMenu... classes, replacing the Carbon menu manager with the one from General/NeXTStep, and making General/NSPopUpButtonCell (a subclass of General/NSMenuItemCell) no longer a paradox (being a still-in-use subclass of a deprecated superclass).* (Votes: AYES: 0 NAYS: 1)

The Carbon menu manager is more powerful than the General/NeXTStep one. Apple should include more of its features in General/NSMenuItem.

* *Bring back the other style of General/NSSplitView bar. Only the bar with pin stripes exists in 10.3.* (Votes: AYES: 2 NAYS: 2)
...
----**General/ProjectBuilder** / **General/XCode**----


Maybe a bit contentious this, but * * dump project builder altogether * in favour of something like netbeans (http://www.netbeans.org) or eclispe ( http://www.eclipse.org ) I know OBJ-C purists here will be offended by using an IDE written in Java, but both of these General/IDEs are massively much more powerful than project builder and could be extended for use as Objective-C/ Cocoa / Webobjetcs tools much more easily than writing project builder from scratch. Apple could leverage the enormous amount of work going into these General/IDEs and focus their efforts on better compilers and debuggers. (Votes: AYES: 1 NAYS: 0)

* *General/ProjectBuilder should have some code generation for easily adding class and instance properties and accessors* - and some refactoring capabilities, please! See Smalltalk! (Votes: AYES: 10 NAYS: 0)
...
* Wanna check out my lil forum thread General/XCodeTooMuchLikeATextEditor ? (Don't vote on this one) 02/18/2004*
...
*see Also General/RefactoringInCocoa*

* * Open Source General/ProjectBuilder!* (Votes: AYES: 2 NAYS: 0)
...
* *Support/allow General/OtherLanguages* (Votes AYES: 4 NAYS: 0)

? I'm thinking about General/PythonCocoa, some kind of General/RubyCocoa already seems to exist, and was it that Perl people are at it too? (Yep - we are. See General/CamelBones) How about C#-Cocoa?

*There is in fact a project for Cocoa and Mono (.NET) integration: http://www.cocoasharp.org/ *

TOM Cocoa would be interesting

Apple appears to be working on a language plug-in architecture. They already have keyword recognition for many languages, including General/DyLan. Not granted yet, but they're working on it.
...
* Apparently there's a much better book than "Learning Cocoa" on the way from the guy who runs General/BigNerdRanch. -- General/FinlayDobbie

* Having seen a pre-release version of Hillegas's book, it is very impressive and easy to read, but it may not be the book that everyone is waiting for. You need to know C and you need to have thoroughly read and understood Apple's Inside Cocoa, Objective-C language book from General/FatBrain. Aaron's book is aimed at programmers, not novices --General/WmMoss

* One excellent General/NeXTSTEP programming book and one book on Rhapsody's Yellow Box are being revised for Mac OS X. -- General/ScottSteinman


OK, now just jump on over to the General/CocoaBooks page for some info on published (or soon to be) books about Cocoa Programming.  ---> K. General/McClanahan
...
**[note3]** I vote against user contributions for this reason: In my experience, most programmers don't know how to write in a coherent and understandable way. Want proof, look at the number of circular definitions in the General/CocoaGlossary section of this site.

**[note5]**
Although the major source of General/ProjectBuilder slowness is jam and its orphans lying underneath the GUI, it also adds a great bit of slowness if you set it to index the source files (and that's on by default). Besides, jam's multiple processor feature is broken since 10.0 (according to some Apple engineers), so it's always using one processor.... Fun. --General/SlavaKarpenko
...
**[note6]**
General/MachO is the binary format and ABI used natively in General/MacOS X. Although it works, it's somewhat slower than, for example, CFM. The reason is that it was designed for CISC processors (m68k, x86), and does not follows the de-facto RISC ABI declared by Motorola & IBM. For this reason, it uses wierd outdated PC-relative addressing of data, completely leaves register r2 (RTOC) ignored, and much more oddities that are not present in other General/PowerPC General/ABIs. The usage of General/MachO saved Apple about 6 months of changing the development tool chain to support more modern General/ABIs, but at the price of 30-40% performance loss (according to tests made by some kernel/lowlevel General/AppleComputer engineers I know). --General/SlavaKarpenko

Is it the Mach kernel's fault or the General/MachO format? --General/AdamAtlas
...
That's ridiculous.  Sure, it saved them six months or so, but wouldn't six months be worth a boost of that level?  I thought Apple was devoted to quality rather than just spitting out OS after OS (like some other company I know of)!  Apparently, I have put too much faith in them! --J. General/McArthur

You can lose a lot of ground in six months. -- General/RobRix

For the hard facts (vs secondhand hearsay) on General/MachO vs CFM, take a look at http://www.omnigroup.com/mailman/archive/macosx-talk/2002-October/000309.html - ignoring r2 may not be such a bad move.
