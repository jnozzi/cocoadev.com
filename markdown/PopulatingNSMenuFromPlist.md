...
@interface General/MenuFactory : General/NSObject {}
+ (General/NSMenu*)menuWithPropertyListFile:(General/NSString *)fileName;
+ (General/NSMenu*)menuWithArray:(General/NSArray *)array;
@end

//
//  General/MenuFactory.m
//
//  Created by Colas Schretter on 03/08/05.
//  Copyright 2005. All rights reserved.
//

#import "General/MenuFactory.h"

@implementation General/MenuFactory

+ (General/NSMenu*)menuWithPropertyListFile:(General/NSString *)fileName {
	// read a hierarchy of key/value pairs on file
	General/NSMenu *menu = General/[MenuFactory menuWithArray:General/[NSArray arrayWithContentsOfFile:fileName]];
...
+ (General/NSMenu*)menuWithArray:(General/NSArray *)array {
	General/NSMenu *menu = General/[[NSMenu new] autorelease];

	// the first entry of an array is its title
	int i;
	for (i = 1; i < [array count]; ++i) {
		if(General/array objectAtIndex:i] isKindOfClass:[[[NSArray class]] ) {
			General/NSMenuItem *item = General/[[NSMenuItem new] autorelease];
			General/NSMenu *submenu = General/[MenuFactory menuWithArray:[array objectAtIndex:i]];
			...
			// the first entry of an array is its title
			General/NSString *title = General/array objectAtIndex:i] objectAtIndex:0];
			[[NSAttributedString *attributedTitle = General/[[[NSAttributedString alloc] initWithString:title] autorelease];
			[item setAttributedTitle:attributedTitle];
		} else if (General/array objectAtIndex:i] isEqualToString:@""]) {
			...
			General/NSArray *tokens = General/array objectAtIndex:i] componentsSeparatedByString:@"~"];
			...
			General/NSAttributedString *attributedTitle = General/[[[NSAttributedString alloc] initWithString:key] autorelease];
			[item setAttributedTitle:attributedTitle];
...
Hi, I tried the previous code in my project and it doesn't work ("initwithcoder" is not found ),the program needs to implement the General/NSCoding protocol.So I wrote this in General/MenuFactory.h :
...
@interface General/MenuFactory : General/NSObject <General/NSCoding> {} 
+ (General/NSMenu*)menuWithPropertyListFile:(General/NSString *)fileName;
+ (General/NSMenu*)menuWithArray:(General/NSArray *)array;
...
The problem is that I can't write anything inside "initWithCoder" definition without errors like  "-General/[MenuFactory supermenu]: selector not recognized [self = 0x33b0a0]"; so this is the code I added in Menufactory.m:
...
How can I use "General/MenuFactory"'s code without errors and with little modifications only? 
Angelo

----

I think you're confused. The purpose of the General/MenuFactory CLASS is to make pre-populated General/NSMenu objects from plists on demand. There is no instance of General/MenuFactory ever made or needed so there is no purpose in being able to archive or dearchive one. Making it adopt the General/NSCoding protocol is unnecessary and fruitless. The error you're seeing is because having done this, you've confused menuFactory with General/NSMenu itself - it has no 'supermenu', hence it's not found.

My advice is to undo your changes and understand what's going on. General/MenuFactory's job is to manufacture menus (that's what factories do!). As the original responder suggested, you should not be saving the contents of menus (i.e. doing so is attempting to use the menu itself as a data model, when in fact all it is is a user interface representation (view)). A better idea is to have some internal representation of your data (model) and generate the menu from it, and to save/restore this model. You need that third step - you're trying to cut corners whether you realise it or not, and it's biting you.

One other very minor error to note in passing - if you make a class adopt a protocol like <General/NSCoding>, you don't need to declare the methods of the protocol as part of the @interface - it's already implied. You just need to implement the methods in your @implementation. However, it's harmless to do so, it just adds unnecessary clutter. Hope this helps. --General/GrahamCox
...
So I need an Nsarchive containing General/NSUrls to files (I can do that) that every time I click on the menu,shows a submenu with a list of recent "annotations" files (loaded from a xml file) and when I click on one of them opens the files' url.
...
Think about it in terms of model-view-controller. Your model is a list of files. A good container for a list is an General/NSArray. So one possibility is to maintain an General/NSArray of NSURL objects. You can write methods that add or remove items from this list. You can archive the array to and from a plist (XML), since General/NSArray already supports the General/NSCoding protocol. So far so good. One view of this model is a menu, so you need a method to populate a menu from the list. Some code similar to the above General/MenuFactory would be a good starting point - just iterate over the list, extract part of the URL (you might not want to have the full pathname, just the last component - General/NSString has methods for breaking down paths) and make each menu item. You need to rebuild the menu whenever the list changes, not just at -awakeFromNib time. If you wrap your container in a model object of your own, this is very easy to arrange. Finally, what happens when the user selects an item in the menu? This is where the controller part comes in. It responds to the menu selection by looking up the relevant URL item, and performing the required action - opening the file say. In reality to do all this requires very little code - just a handful of very short methods in one or two new objects. It's a very well-defined problem space with 99% of the solution provided for you by Cocoa as is - you just have to join the right bits together really.
