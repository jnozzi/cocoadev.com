...
-(void)awakeFromNib
{
	...
	[self addObserver:self forKeyPath:@"fullName" options:(General/NSKeyValueObservingOptionNew | General/NSKeyValueObservingOptionOld) context:nil];
}

- (void)observeValueForKeyPath:(General/NSString *)myKeyPath ofObject:(id)myObject change:(General/NSDictionary *)myChange context:(void *)myContext
{
	General/NSLog (@"Observing Value for Key Path: %@; Object: %@; Change: %@....", myKeyPath, myObject, myChange);
	General/NSLog (@"Done observing.\n");
}

-(General/IBAction)myPushMe:(id)sender
{
...
Now, the next move is to have a set of keys that are observed for changes. When a change occurs, the program should send a message to a model object to update a dependent value. (the myStartTime and myEndTime variables are bound, via General/IBPalate bindings, to datePickers. They work fine in the version of the program that triggers total bill calculations through generic notifications. However, no matter what I've tried, they fail to invoke any sort of KVC-mediated bill calculation. For example:
    
@implementation General/MyBillableController

+(void) initialize
{
	[super initialize];
	[self setKeys:General/[NSArray arrayWithObjects:@"selection.myStartTime", @"selection.myEndTime", nil]
...
// IB outlet to this object, but if I insert General/NSLog lines in the awakeFromNib method of this controller
...
-(void)awakeFromNib
{
// myEventTotalOutlet is an General/IBOutlet to an General/NSTextField that should contain the calculated bill float.

	[myEventTotalOutlet bind:@"value"
					toObject:self
				 withKeyPath:@"myEventBill"
					 options:General/[NSDictionary dictionaryWithObjects:General/[NSArray arrayWithObjects:General/[NSNumber numberWithBool:YES], nil]
					 forKeys:General/[NSArray arrayWithObjects:General/NSContinuouslyUpdatesValueBindingOption, nil]]];

	[self addObserver:self forKeyPath:@"myEventBill" options:(General/NSKeyValueObservingOptionNew | General/NSKeyValueObservingOptionOld) context:nil];
...
----
First responder: Yes, it seems that something needs to "care" for the computer to invoke the dependent key. However, in the second example, I thought I had properly bound the General/NSTextField to the myEventBill key. Nevertheless, the program still fails to invoke the myEventBill getter, even when I include the code in the controller object itself.
Second responder: The will change/did change methods aren't part of KVC compliance, acording to page 25 of the KVC coding manual from Apple. I got the impression that these methods were strictly NOT to be used if you rely on automatic Key-Value binding, which is the default setting. Is that wrong? Are they not already inherited from General/NSObject?

(Taken from the manual for General/NSKeyValueObservingProtocol):
...
+ (BOOL)automaticallyNotifiesObserversForKey:(General/NSString *)key
...
In the second example you give, you attempt to set dependent keys using a key path.  This behavior isn't supported for setKeys:triggerChangeNotificationsForDependentKey:- this method only supports single keys.  There is some information that could be useful on the Core Data FAQ page: http://developer.apple.com/documentation/Cocoa/Conceptual/General/CoreData/Articles/cdFAQ.html#//apple_ref/doc/uid/TP40001802-DontLinkElementID_26

Without seeing much of the code, it appears you are subclassing General/NSObjectController.  Is this really necessary?  An alternate approach would be to duplicate the myStartTime and myEndTime in your model object, and have the actions for the General/DatePickers (or whatever control you're using to choose the dates) update the appropriate field in the model.

-sbooth
----
sbooth-
Thanks for responding! I did subclass General/NSArrayController for this application so that I could better regulate the General/NSDatePickers which are bound to the xcdatamodel through IB palate settings. Specifically, I wanted to be able to disallow certain date choices. Nevertheless, the dependentKeys thing still isn't working for me. Basically, when a user changes a date or time, the computer should recalculate the total bill based on the new time interval. Even when I write matching getter and setter methods IN THE CONTROLLER OBJECT, they fail to be invoked. (Along the lines of your suggestion (I think), I had tried to use methods in the controller object to invoke methods in the model object, but I still couldn't get the dependentKeys method to work.) I just want to know how (or even if it's possible) to have the dependent keys listed above lead to the invocation of a particular getter method by using KVC. The code works as it is by using notificaitons (I look at General/TextDidEndEditing for General/NSTextFields (not shown) or at the validation method for the General/NSDatePickers, then post notifications that way), but since this specialized notification system exists in 10.4, I figured I try using it instead. I can post more code if it will make clearer what I've tried. Thanks again for any insight or suggestions!
