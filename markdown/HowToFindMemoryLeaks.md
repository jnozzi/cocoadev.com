...
Some tips and threaded discussion about using such tools as the *leaks* program and links to other General/DeveloperTools for the purpose at hand:



* Expand the disclosure triangle beside "Executables" in General/XCode and highlight your executable (In Xcode 2.5, double-click the executable and select the "Arguments" tab in the Info window. Be sure to add environment variables, not arguments.)
* The right pane shows the arguments you can pass your application at runtime.  Click the + below Environment Variables.  The new Name should be General/MallocStackLogging and the Value should be YES. Add an environment variable for General/MallocStackLoggingNoCompact=YES as well.
...
* Examine that method in General/XCode.
* see General/RetainReleaseTips for information on how to manage memory.  The basic rule is: If you alloc, retain or copy it, you need to release it.
...
*In addition to Leaks, there are General/ObjectAlloc and General/OmniObjectMeter.



NOTE: You should make sure you do not run leaks with General/NSZombieEnabled or it will look like everything is leaking!
...
Unfortunately the system seems to leak, in particular, General/NSSavePanel leaks a lot of General/NSCFURLs and General/NSCFStrings and the "drag manager" leaks General/NSCFDates.  But if I also set the     General/MallocStackLogging environment variable, I get a stack dump, and thus it's easy to spot what's my mess, and what's the systems mess! :)

----
This may be new with Panther, I'm not sure, but you can also just type     leaks General/YourApplicationName, and it will work (assuming there's only one copy running). No need to fart around with pids if you're typing everything manually.
...
I've been playing with leaks and unfortunately there seems to be a lot of things that it will not catch -- leaks in General/ObjectiveC are likely to happen due to circular retain counts, and leaks doesn't detect these. I also tested some other things, where I'm quite sure that I'd thrown away the pointer to the memory in question (like add'ing some new'ed object to a std::map which I then disposed), but leaks still did not detect it.
...
The documentation for leaks says that General/MallocDebug picks up certain things that leaks doesn't, but doesn't pick up some things leaks does. I don't know if General/MallocDebug picks up General/CircularRetains, but it seems to be more General/MacOSX-tuned. So using them to complement each other seems like the best policy.
...
Also, General/NSZombieEnabled is quite a handy little method for diagnosing this kind of thing in-app.  Basically when an object is deallocated, instead of being freed, it's converted to a 'zombie' class, and will log when you try to send a message to it.  It's normally used for debugging extra deallocations, but it can just as easily be used to check if objects were freed as expected.-- Bo

----

I've noticed that changing colors using the color wheel in an General/NSColorWell (from the color picker) will cause RAM usage to go up repeatedly... is this some sort of Cache, is there any limit, can I stop it? Or am I missing something :-/ It seems to leak in all apps?

----

Turn on General/MallocHistoryLogging, use leaks, and see what you can track down.
...
I believe that what I have are General/RetainCycles -- which means that General/MallocDebug isn't picking it up as a memory leak.  The number of objects in the autorelease pool however, is far too numerous to see if any of them will leak even after the pool releases each occurrence of every object.  Are there other tools out there that would catch something in memory if it isn't referenced by anything in the stack?  Does "leaks" do that kind of thing at all?
...
leaks isn't showing my anything, except three leaks in the General/AppKit, which I couldn't care less about (Apple likes to "leak" (reuse) it's save and open panels).  What are general cycle leak hunting techniques?
...
General blocks are blocks that do not refer to either General/ObjC or General/CoreFoundation objects, i.e. that are malloc'ed directly.  The number after the dash is the size of the block in bytes, and they are grouped by size.    -- Bo
...
Have you turned on General/MallocStackLogging? Maybe these really are leaks. Post a sample if you're not sure.