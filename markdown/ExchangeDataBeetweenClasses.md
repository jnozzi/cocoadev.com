...
I was thinking about creating an "General/AppController" class to hold all the globals but is it the right way? - General/StephaneDassieu.
...
However, a better way (more General/ObjC-like and less prone to linker errors) of getting access to that global is to create a getter and a setter as class methods in your class1.
...
That must have been off of a General/GNUStep list.  The GNU people have fights about #import, but people working on Macs don't, generally.  Go ahead and use #import.

*And, in any case, the debate has been resolved. At one point the use of #import was deprecated in gcc, but it no longer is. So the only reason General/GNUStep people or anyone doesn't use it is pure stubbornness / habit.*
...
* Class2 is also a General/NSView subclass but as been made in PB.
...
For a start, you shouldn't be doing *anything* complicated in any View class. If it's not a simple matter of displaying something on the screen, it belongs in a Controller object: see General/ModelViewController. What *should* be happening is this:
...
----
Sure, it helps a lot.
To be more precise, i'm trying to do an Automator like interface (see General/AutomatorLikeUI).
Here what i've done:


* My Class1 is "General/PlanView". It's a custom view made in IB and added to the window.
* My Class2 is "General/BlocView". It's a custom class i can add to "General/PlanView" on the fly.
* When i add a General/BlocView to General/PlanView, i want it to go under the previous one as in a list. - *so i have an yPos variable to hold the Y value of the previous General/BlocView in General/PlanView.m*
* When i add a General/BlocView to General/PlanView, i want it to draw its index (number) in the top left circle (as in Automator). - *so i made an accessor/mutator to the index value in General/BlocView.m*
* When i click the the close box of a General/BlocView, i want it to be remover from General/PlanView. - *so i have a removeFromPlan: method in General/PlanView.m*


As you see, i didn't used a Controller class. I just set the General/PlanView 'as the controller'. (Tell me if i'm wrong to do so, maybe it's better to create a third class to control the Plan and the Blocs?).

I modified my code regarding to tour advices (thank by the way). So now, General/BlocView has an accessor/mutator to change it's index number (i'll need this: if i have 3 blocs numbered 1-2-3 and i delete the #2, i need General/PlanView to update the index of #3 to #2 to have 1-2 and not 1-3, and i also need to redraw the view to remove the blank space but it's another story)
Thanks to this, my code is a lot cleaner, no more class methods nor class variables.

My problem now is when i made General/PlanView in IB, i didn't added it an outlet.
How can General/BlocView sends messages to General/PlanView?? It don't even know General/PlanView exist...
...
I tried General/self superview] removeFromPlan]; inBlocView.m but it don't works (removeFromPlan is a method in [[PlanView.m)
Should i add an outlet to General/PlanView? How to connect this outlet to General/BlocView? - General/StephaneDassieu
...
If you're creating General/BlocView on the fly, you'll need to manually tell it what action to take on being clicked. Ideally, you would derive General/BlocView from General/NSControl, not General/NSView; this lets you set up a standard action with     setAction: and     setTarget:; otherwise, you will need to duplicate this functionality by adding an outlet to General/BlocView that General/PlanView sets to point to itself when it creates a new General/BlocView.

----
*otherwise, you will need to duplicate this functionality by adding an outlet to General/BlocView that General/PlanView sets to point to itself when it creates a new General/BlocView.* --> this is exactly what i'm trying to do. But regarding to General/HowToUseOutlets post, it looks impossible! Can you post a generic sample code please?
I already added **General/IBOutlet General/NSView *plan;** to planView.h but what now? I can load this outlet in IB but i cannot connect it to General/BlocView as there is no representation of it in IB.. - General/StephaneDassieu

*update -> Damn, searching for 2 hours without any results.
General/NSNibOutletConnector *nib = General/[[NSNibOutletConnector alloc] initWithCoder:]; send an error "General/NSNibOutletConnector undeclared�". I really need some help to do this : i haven't found anything on the internet about how to create outlets on the fly.*
...
Damn.. you're last post is kinda rude to me.. I already made a lot of tutorials and i also read 2 books (General/BookCocoaProgMacOSX and General/BookLearningCocoa) and also bought General/BookCocoaInANutshell... and i read the General/ObjC.pdf from Apple too. I think i'm suffering from being raised on General/REALbasic so i've learn OOP the wrong way. And i'm French so it's quite harder to learn in a foreign language.
...
In fact the two books i have don't even talk about the General/NSLib class because they were written before 10.3 - General/StephaneDassieu

----

I don't see the previous post as being rude; your questions are pretty basic, and indicate that whatever you've read, you need to go over it again. Cocoa is a large subject and you're not going to 'get' it in one reading. In addition to reading and doing tutorials, look at lots of sample code. /Developer/Examples/General/AppKit/ on your hard drive is a good place to start.

As for your question, there's nothing magical about General/IBOutlet<nowiki/>s - they're just instance variables prefaced with a General/IBOutlet keyword that General/InterfaceBuilder looks for and adds those names to its list of objects. If you want to assign an object you created programmatically to an General/IBOutlet, just assign it.

<code>
General/IBOutlet General/NSView *myViewOutlet = myViewICreatedProgrammatically;
</code>

----

Oh well, i surely need to learn a lot. I'm going to write the app from 0 again in order to use a Controller class to handle both the plan and blocs, maybe it will makes things more clear both for you and me. *regarding the 'rude' word, i didn't mean you has been rude, but it's hard for me to realise how poor i am <sic>..* - General/StephaneDassieu
...
my experience started with hypercard, moved to a basic "Basic" on mac os 7.5, then to C and powerplant, and finally I am working in Cocoa and some applescript.  Cocoa is the very first OOP language/environment/framework I have used.  General/RealBasic is user friendly, and straight forward, and while realBasic != C, it does not exclude you from being able to use Cocoa.  A C course or two may help, as a lot of the "heavy lifting" in Cocoa defers to C  (Objective C is welded on top of C which means you can use C Inside General/OBjC), but its not necessary.  
...
2.  any control class (subclass of a view that allows mouse interaction) by definition allows you to set the target of its action message. (look in General/NSControl for exact phrasing)
3. General/NSView allows you to add a subview... any class that decends from General/NSView, supports this behavior.  so any object that basically takes up space in the window can have views inserted into it.
...
*I already added     General/IBOutlet General/NSView *plan; to planView.h but what now?*
...
@interface blocView {
    ...
    General/IBOutlet General/NSView *plan;
}

- (void)setPlan:(General/NSView *)plan;
...
-(void)addNewBloc {
    blocView *newBloc = General/blocView alloc] initWithFrame: ... ];
...
Thanks to all of you, all your advices are really helping me. I've rewrote my app again and i'm using a class controller as a bridge between General/BlocView, General/PlanView and other controls.
Will try to complete my objective before crying victory but i think i've done the things the right way this time. - General/StephaneDassieu

---- 

Great, i did the code above and it's working great. As i used a Controller class to handle General/BlocView and General/PlanView, i modified it a bit (    General/IBOutlet General/NSView *plan; is     General/IBOutlet General/NSObject *controller now). I think my code is pretty neat now. Thanks again! - General/StephaneDassieu.

---- 

One thing you might want to consider is removing your custom General/PlanView and General/BlocView classes entirely, relying solely on existing View classes. Unless you're doing something **visually** impossible with existing classes, you should reuse the code already there.

*I'm sorry, i don't get you... Are you saying that i should try not to use custom classes (General/NSView subclasses)?*

Yes, that's exactly what I'm saying. Any behaviour that needs to be customised that's *not visual* should be moved to a Controller class. If you *can* use a standard class (i.e. any General/NSView subclass that comes with Cocoa), *do*.

The only time you should break that is when making a new, visual metaphor (say, a view that displays all its components in a rotatable circle, or maybe an Automator-like interface) that you wish to be reusable. In this case **only**, you should make **one** new General/NSView (that will contain and control the *visual* behaviour of its internal views) and *migrate* the Controller behaviour there. While you are initially designing it, **keep the code in a Controller**.

Chances are that unless you've actually used this metaphor more than once, you won't generalise it properly anyway, especially if you're a Cocoa newbie. Consider making an General/NSView subclass as the final step, only to be taken once the code is rock-solid and proven in multiple applications. Never do it prematurely.

---- 

Really disturbing news : almost all the tutorials i made used General/NSView subclasses.. even the Apple's ones!
Anyway, i already changed General/BlocView to a General/NSControl subclass, only General/PlanView is still a General/NSView subclass but it's not controlling the visual behaviour of it's internal views anymore, i made a Controller class. But i think's that's what you wanted to say is : i can make General/NSView subclasses but i have to handle them from a Controller (General/NSObject subclass) one right? Or should I prefer General/NSControl subclasses?? - General/StephaneDassieu

----

Okay, I confess, "never subclass General/NSView" is a little optimistic about just how customisable General/NSView is. But unless you know for sure what you're trying to do cannot be satisfied by an existing class, don't subclass! For example, why are you making General/PlanView? What methods do you need to put in it? Why are you subclassing General/NSControl? What methods do you need to put in it?

Are you sure you can't achieve what you want to achieve with a combination of existing Cocoa classes (no custom subclasses of anything other than General/NSObject!), actions, notifications, and delegates? If you're not sure what some of those latter meant, it's because you're missing knowledge of some of the best ways Cocoa provides for customising its classes. Check out General/CocoaGlossary for more.
...
----
Ok, i'm totally lost now ahah... here my General/BlocControl.m code (I renamed General/BlocView to General/BlocControl as it is a General/NSControl subclass) :
    
// code removed - it was a really bad one and i don't want to give bad examples.


How could i handle all of this without subclassing General/NSControl and then 'Create Files' in IB?
...
2. 'General/NSObject' may not respond to 'removeTheBloc:'

Is it normal? - General/StephaneDassieu.

*to get rid of the warnings, declare     -removeTheBloc: in your header file. -->     - (void)removeTheBloc:(General/BlocControl *)removeBloc; already declared in the Controller.h Should i declare      -removeTheBloc: also in General/BlocControl.h ?*

----

Oops, i forgot to add     #import General/PlanController.h at the top of General/BlocControl.m
But i still have a warning : 'General/NSObject' may not respond to '-removeTheBloc:'

*Sounds like you've got a Controller object that's pointed to by a     General/NSObject*. Either change     General/NSObject to your controller type wherever its stored, or change it to     id (which will quash the warnings, but is less ideal).*