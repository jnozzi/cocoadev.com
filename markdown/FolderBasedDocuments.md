...
*Create a Cocoa document based application in Xcode named "General/AppForFolderBasedDocument" (File -> New Project -> Cocoa Document-based Application) 
*In the Group&Files outline view open **Targets** and select **General/AppForFolderBasedDocument**
*With **General/AppForFolderBasedDocument** highlighted blue select the **Get Info** menu item (File -> Project -> Get Info)
...
*Set the class that handles the document to "General/MyDocument" in the **Class** field
...
*find the entry for the new document type and modify this entry so that it includes the keyed item **General/LSTypeIsPackage** set to **YES**
    
			<key>General/CFBundleTypeExtensions</key>
			<array>
				<string>folderdoc</string>
			</array>
			<key>General/CFBundleTypeIconFile</key>
			<string>General/FolderDocIcon</string>
...
- (BOOL)loadFileWrapperRepresentation:(General/NSFileWrapper *)wrapper ofType:(General/NSString *)type {
    General/NSLog(@"loadFileWrapperRepresentation: %@ type: %@", [wrapper description], type);
}
...
*This would probably be a good situation in which to just stick with the traditional Cocoa techniques. Once Apple irons out the General/CoreData stuff, we might be able to use it for packaged files.*

----

New folder-based General/CoreData General/NSDocument subclass was moved to its own page... General/CoreDataNSDocumentPackagesSubclass
...
----
My first guess would be that you have a browser which doesn't properly handle form refreshes, keeping an old version of the page text around despite the underlying form changing, but (irritatingly) updating the "olddate" hidden field, thus circumventing collision detection. This seems to happen more readily if you jump around with the back and forwards buttons in your browser while you're editing the page. Another possibility is that, while looking at the old revision of the page, you hit "revert" instead of "edit". Chances are very high that it was accidental :) -- General/KritTer

----

Sorry to bump this, but I wanted to get back on-topic with this page... Go ahead and take a look at the General/MyDocument class code above, and give me a critique if you have the time. If it works well for everyone, it would be a nice solution to the problem of not being able to use General/CoreData easily for Document-based file packages. (moved to its own page... General/CoreDataNSDocumentPackagesSubclass)

----

Apple has updated General/CoreData in Leopard to allow you to write your own file - the Atomic store type. Anyone know if this means we can use folder-based documents with this?

----
A better place to discuss the Core Data issue with packages is: General/NSPersistentDocumentAsPackage (since that's the precise and only topic of that page). No responses since I updated that last week.