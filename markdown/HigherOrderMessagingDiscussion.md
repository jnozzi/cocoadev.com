...
Refactored from General/HigherOrderMessaging; see that page for context.

----

For a conceptually simpler implementation of General/HigherOrderMessaging(more flexible, too, I think) using General/CodeBlocks, see General/OCBlock.  In this implementation, as in Smalltalk, collections respond to -do:, which takes a block as an argument, as well as to -collect:, -inject:into:, -select:, -reject:, -detect:, -detect:ifNone:, and others...   Additionally, using blocks means that more than one instruction can be sent to each object in the array, and that the existing methods need not exist yet, and you can use each item as an argument...   --General/JoeOsborn

I'd disagree about greater conceptual simplicity with blocks; they are indeed more flexible, but there are tradeoffs on either side. -- General/RobRix

I also disagree about greater conceptual simplicity with blocks, strongly.  Blocks are, in a sense, anonymous functions that interact in magical an mysterious ways with their lexical scope.  HOM on the other hand is simply taking a message and distributing it, something that can be easily explained to complete novices.   -- General/MarcelWeiher

----

Getting back to the orignal issue:  It seems to me that to do what you want (implementing -General/[NSArray do]), you'd be better off just implementing something like these in an General/NSArray category:

    
// performs aSelector on each object in the array.  returns an array
// containing all of the return values.
- (General/NSArray *)arrayByMakingObjectsPerformSelector:(SEL)aSelector;
...
newArray = General/myArray do] foo];
...
After talking with General/MarcelWeiher, I'm getting near to moving to Joe's blocks, but not just yet, not just yet. Trampolines are conceptually simpler (IMHO), and although it's taken some ad-hockery to work around a problem with gcc, I think I might have -select about to work (at long last).

-- General/RobRix

Yes.  In fact, Higher Order Messaging in its present form evolved out of an implementation of "General/EnumFilters" that presented a pipeline of message-based filters.  The filters were created using the "standard" forms, but the number of forms just kept increasing, and there were a number of different filters, so it was getting to be a real pain.  It took me a while to realize that all the "withSelector:with:" style messages really contained a second message-send that was dying to come out.  It then took me a little longer to figure out that the easiest way to represent a message-send in code was...wait...a message-send!  But the Aha! was great when it hit, and I also realized that not only does it solve my "exploding number of useless cover-methods" problem, but also was completely general and even looked a lot nicer.  -- General/MarcelWeiher
...
General/mutableString do] appendString:[stringArray each;



-- General/MarcelWeiher


In General/SmallTalk, I'd go with blocks. In General/ObjC, I think bouncing is conceptually simpler because forwarding et al is intrinsic to the language.
...
I was bored, so I decided to implement of General/RobRix's above specification for a General/TrampolineObject. You can find the result at General/BSTrampoline, and feel free to use it for anything you like.
...
I've been thinking of posting General/LSTrampoline as an example but since it's not working for -select/-reject yet, I'll wait on it. I'm gonna be taking a closer look at -select in your implementation, though, cos I'd like to be able to do [array select] rather than General/array collect] selectOn:...] which is what I'm currently looking at.
...
Well, finally, General/LSTrampoline is done, working, and General/UnitTested (Joe will love that). And it doesn't crash, either! :)

-- General/RobRix

General/UnitTested!?!  General/RobRix General/UnitTesting?  General/VeryStrange!

Are they unit tests or functional tests?  Oh, well, at least you're testing externally to the object. (:  Carry on!

--General/JoeOsborn

Both unit and functional tests--i.e. General/LSHOMTest does what's expected of it (although I've not coded it using assertions or anything), and General/LodeStone is also behaving itself.
...
- (General/NSArray *)objectsReturningValue:(long)value
                      fromSelector:@selector(someMethod)

and 

- (General/NSArray *)objectsReturningObject:(id)object
                       fromSelector:@selector(someMethod)


(and counterparts with 'withObject:' on the end) in a category on General/NSArray. Of course, the first one is redundant as it could be done using General/NSValues or General/NSNumbers, but it's more convenient to just pass in the value. They would of course make the objects in the array perform the selector, and if they returned the right value then they'd be put into the returned array. The second one would compare return values from someMethod with object using isEqual, but there should also be an objectsReturningObjectIdenticalTo version.
...
I bet General/RobRix didn't expect me to post anything here.

-- General/AngelaBrett

General/RobRix was indeed surprised, Angela! Anyhow, to give some examples of how I'm using General/LSTrampoline:

    
// -drawRect: on General/LSCanvasView
-(void)drawRect:(General/NSRect)rect
{
...
General/[delegate elements] do] draw];
...
}

// -selectedElements on [[LSCanvas - this one amuses me :)
-(General/NSSet *)selectedElements
{
	return General/elements select] isSelected];
}
...
Actually, I'm starting to feel a bit uneasy about this. Sure, General/monkeys do] eatObject:banana] has nicer syntax than 
...
Hey, look, there's history! Well, to make up for my gaffe, here's what General/RobRix said which I deleted:
...
Seriously, General/monkeys do] eatBanana] makes sense to me even if it's not classic [[object variable] anotherMessage] syntax. And also, it does match +alloc as in [[Class alloc] init]. 
...
And now here's what General/RobRix said which has yet to be deleted :)
...
General/[[array collect] resultsOfSomethingOrOther] do] logThyself];
...
Anyyway, something else occurred to me:  what if this "trampolining" could be done through a separate object, but as a category on General/NSArray?  Then this:

    
General/monkeys do] eatObject:banana]
...
One potential drawback would be that any selectors that General/NSArray normally responds to would be impossible to send to the collection (unless we also put a "do" method in our General/NSArray category that returned a separate trampoline object).  
...
There's a big benefit to having transparent collections, but it might be better to write your own General/NSArray subclass to do it rather than try and hack Apple's one.

I'm curious, General/RobRix. How are you altering the profile of the message you're sending?
    
-(id<protocol>)myMethod;

id<protocol> ret = General/array do] myMethod];
[ret doSomeProtocolMethod]; // N'u-uh


-- [[KritTer

If blurring the syntax between single instances and collections is your goal, you might as well implement the HOM methods on General/NSObject as well and override those implementations on the collection classes. Otherwise, why bother with trampolines at all?

You could write -forwardInvocation: on the collections to forward messages to the contained objects, but straight trampoline HOM is conceptually simpler--and it's already done :) But like General/KritTer, I'd suggest doing this on a subclass rather than on General/NSArray itself.

As for your question, General/KritTer, I dunno what you mean. And -do doesn't return anything anyhow... . Really not sure what you mean.

-- General/RobRix

*plus you list a protocol in <...>, not a category* Oh, yeah, silly of me. It's been a while.

What I mean is, what does General/array do] myMethod] actually return?
...
I can tell you what General/BSTrampoline would do - I can't say about General/LSTrampoline because I haven't looked at it closely enough (*is lazy*).
General/array do] method] would return the result of [[array lastObject] method], with, indeed, the correct return type. That, however, is an implementation detail. But not one that's likely to change.
...
Not a good idea to do General/array do] copy] then :) -- [[KritTer

No matter what -do returns, no; the documented spec. says that -do returns nothing at all, so trying to use the nothing it returns would be quite silly of you, even if it *is* something. -- General/RobRix

But I thought -do returned a trampoline... and you're sending the [no]thing it returns a message so it had better be something. -- General/AngelaBrett

Kritter's snippet would be fine - what you shouldn't do is General/[array do] copy] something]. As [[AngelaBrett says, -do returns a trampoline; [aTrampolineMadeWithDo something] returns nothing at all.

-- General/ThomasCastiglione

General/KritTer's snippet would **not** be fine, since you'd be leaking every copied object. -- General/KritTer, talking in the third person

Thank you, General/KritTer, exactly what I would have said in the second half of my comment if I'd remembered to write it. General/array collect] copy], on the other hand, is perfectly fine, if you assign to something and otherwise don't leak memory like a sieve.
...
Point taken. It wouldn't *crash* though :) Not until it had been run a few thousand times and filled up the hard drive with VM pages, at any rate.-- General/ThomasCastiglione

Yeah, but the brazen bugs are always the easy ones. I'd rather have one of them than a schroedinbug any day. -- General/RobRix

So if I'm understanding this correctly, neither [array do] nor [array collect] actually return the correct type for whatever method you then send them? -- General/KritTer

Neither, indeed, do [array select] or [array reject] - they and [array collect] return General/NSArrays (for whichever method you send them). That's the way it's meant to work, though - you're getting an array of the collected, selected or rejected results. --General/ThomasCastiglione

It's fun discussing what do doesn't. :) As I understand it [array select] [array reject] and [array collect] all return trampolines. But General/array select/reject/collect] someMethod] returns an array of the selected, rejected or collected results. It looks nice at first but when I think about the semantics too much I don't like it (see above.) -- [[AngelaBrett
...
This is all really quite nice.  I have a thought though:  As far as I can tell there is not yet support for iterating over arguments instead of message targets.  For example, in General/FScript, you can do
...
General/@"hi" collectEach] stringByAppendingString:[[[NSArray arrayWithObjects:@"neighbor", @"earthworm",nil]]
...
[Interjection:  actually, as I keep pointing out, there *is* support for iterating over arguments in the original implementation of HOM, General/MPWFoundation.

    General/[@"hi"] collect] stringByAppendingString:[[[[NSArray arrayWithObjects:@"neighbor", @"earthworm",nil] each]]

-- General/MarcelWeiher 

]

EDIT:  Okay, how does this look for a start?  It's based off General/BSTrampoline ONLY because I'm new to this, and I thought the shorter General/BSTrampoline might be easier for me to wrap my head around. 

This is not remotely working code, more trying to think about what's the right thing to do.

I was about to type "put the following in a category on General/NSObject", but I bet that isn't safe, because General/NSObject probably implements forwardInvocation? Or is forwardInvocation a language feature, and thus not General/NSObject's responsibility... hmm.  If it is a problem,  maybe a subclass of General/NSObject posing as General/NSObject, and then forwarding the invocation to the real General/NSObject if it isn't handled?  Is that possible?  Anyway,

    
- (void)forwardInvocation:(General/NSInvocation *)anInvocation
{
    for (i = 2; i < General/anInvocation methodSignature] numberOfArguments]; i++)
    {
...
Then General/BSTrampoline needs another bunch of modes, such as kCollectEach or something.  Our code is triggered like so:
    
General/NSArray *suffixes = General/[NSArray arrayWithObjects:@"neighbor", @"earthworm",nil];
General/NSArray *strings = [@"hi "  stringByAppendingString:[suffixes collectEach]];

This hopefully yielding the array      {@"hi neighbor", @"hi earthworm"} .

PS - Nah, forwardInvocation has to be a language feature, right?  There shouldn't be a problem putting it in a category on General/NSObject.

Further comment:  I see that this sort of thing is implemented in General/MPWFoundation already, with slightly different syntax, so I'll probably use that implementation and stop working on this.  In General/MPWFoundation, the call

    
General/NSArray *string = General/@"hi " collect]  stringByAppendingString:[suffixes each;
...
-forwardInvocation: is NOT a language feature per se. It is implemented by General/NSObject. For one thing, it uses General/NSInvocation, something which is not a part of General/ObjC, but of Cocoa. Remember always that General/ObjC != Cocoa.
...
All of this is to say that if anybody wants to extend General/LSTrampoline with this, I'd be thrilled (: I've just not bothered yet.

-- General/RobRix

----

Okay, thanks, you just helped me focus in on my confusion: message forwarding is part of the runtime, but the actual calls to forwardInvocation are made by cocoa classes.  Regardless, there's no problem, General/NSObject doesn't have a forwardInvocation method that it's unsafe to override; it just fires     -doesNotRecognizeSelector: (now I looked it up. :).
...
I've been thinking about HOM again, especially in the context of using it as an analog for blocks (only internalized), and, being unsatisfied with General/LSTrampoline, I decided to make another HOM system. I wanted to see if I could improve upon it at all, and wanted to revisit the subject of (for example) subclassing General/NSMethodSignature instead of using private message calls (doesn't seem to work).

After a brief brainwave when I realized that all the type-related madness run into with General/NSInvocation when you're trying to do -select, -reject, or other mechanisms that deal with boolean messages-- for example,     General/array select] isKindOfClass: [[[NSArray class]] is actually introduced with General/NSInvocation...

So, using the lower-level     forward:: and     performv:: skips lightly around all that, and thus was born General/MonoHOM.

-- General/RobRix

----

Since I can't comment here (the page has reached the max size), I've replied at General/HigherOrderMessagingDiscussionPartII. --General/OfriWolfus
