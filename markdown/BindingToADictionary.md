I'm trying to bind something (an General/NSDatePicker for now, but it could be anything) to an General/NSDictionary, like binding it to the user defaults, but I haven't been successful. What would be the way to do this?
...
*2. Open the General/MainMenu.nib file in General/InterfaceBuilder.
...
----
Thanks, but I'm trying to bind to an regular General/NSDictionary, not the user defaults. I've tried these bindings:

    
Bind to: theDictionary (General/NSObjectController)
Controller key: Selection
General/ModelKeyPath: date


And theDictionary is bound to a dictionary in the controller. I'm pretty sure these bindings aren't right though. 
What happens is the General/NSDatePicker just turns gray and won't let me change it.

----
Is it an General/NSDictionary, or an General/NSMutableDictionary? You can't change it if it's not mutable. Also, make sure that the dictionary actually has an entry with a 'date' key.-CS

----
Sorry, it's an General/NSMutableDictionary. And I tried setting an entry with a 'date' key in the -init method, to no avail...
