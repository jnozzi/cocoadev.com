...
-(General/NSArray *)doSomethingIDontCareWhatJustGiveMeBackAnArray
{
    General/NSMutableArray *example = General/[[NSMutableArray alloc] init];
    [example addObject:@"blah"];
    return [example autorelease];
}


is it OK to return example or should I do something to it to convert it to a non mutable General/NSArray?
----
In the example as you have written it, I don't see a big problem with it. Although, If the return value was a mutable array that my class was maintaining, i.e. a member variable, I personally would definitely convert it to an immutable array before returning it. To do this, General/[NSArray arrayWithArray:anArray] is one option, General/anArray copy] autorelease] will also return an immutable array from a mutable variant.
