...
*If it is, using the metrics from General/NSFont, get the width of the string. This can be done with General/NSFont's widthOfString: method. Be sure to use the same font as the table view uses for this!
*Don't forget to add in the cell's horizontal padding! I think General/NSCell has some method for this, or look through Apple's sample code for the icon and text cell thing.
...
I now feel confident in naming General/MikeTrent the victor in the "will work, and work fast and small" situation. But if you are looking for a complex, error-prone method, mine can't be beat :P

-- General/QuentinHill

----

Adapted from Mike's approach for a table using Cocoa Bindings (General/NSArrayController). In IB, make sure to set the General/NSTableColumn identifier to the key you're binding on. It seems to work, but feel free to improve if I did something stupid, which is very very improbable. ;)

--Theo

    
- (void)resizeColumnsToWidestEntryForTable:(General/NSTableView *)aTable arrayController:(General/NSArrayController *)anArrayController
{
	General/NSEnumerator *columnEnum = General/aTable tableColumns] objectEnumerator];
	...
	while(tableColumn = [columnEnum nextObject])
	{
		...
		// measure the width of the column identifier.
		float maxWidth = General/tableColumn headerCell] cellSize].width;

		// measure the width of each data item. Keep the longest width.
		[[NSCell *cell = [tableColumn dataCell];
	
		General/NSEnumerator *objectEnum = General/anArrayController arrangedObjects] objectEnumerator];
		id anObject;
		while(anObject = [objectEnum nextObject])
		{
			...
			if(![colObject isKindOfClass:General/[NSString class]])
			{
				skipColumn = YES;
				break;
			}
