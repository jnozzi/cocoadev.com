...
Questions relating to General/CoreData.
Also see: General/CoreDataInheritanceIssues

And now that we've got the initial round out of the way, it is time for some ambitious soul (i.e. not me) to refactor these answers to the appropriate places, ne? http://goo.gl/General/OeSCu
...
{
	return General/self valueForKey:@"cues"] count];
}
...
Does anyone have ideas how one should define new Persistent Store Types? For example, if I want to enable my [[CoreData application to save and fetch the data from General/MySQL database, how should I implement this? I wasn't able to find any "General/NSPersistentStore" class to override, so it seems kinda impossible at this momemt. Any ideas? -nzhuk 

*There's no public API for implementing your own store type in Core Data 1.0. The most you could do is use Core Data for change tracking and do all the reading/writing yourself.* -General/ScottStevenson

----

It looks like the only types General/CoreData can handle natively are numbers, dates, strings, and binary. How are other types to be handled?

*Binary = every type you'll ever need.* :)

I guess you have to write General/KeyValueCoding accessors to convert between your native format and General/NSData, and use setPrimitiveValue:forKey: to let General/CoreData persist it? This strikes me as very inefficient.

I found Apple's recommended solution at http://developer.apple.com/documentation/Cocoa/Conceptual/General/CoreData/Articles/cdNSAttributes.html.

----

If I'm building a Core Data Document-based Application how do I make sure version 2 of my app will be able to open up version 1 documents? - General/MikeZornek

*See this post from cocoa-dev:*
http://lists.apple.com/archives/cocoa-dev/2005/May/msg00640.html

See also: http://developer.apple.com/documentation/Cocoa/Conceptual/General/CoreData/Articles/cdVersioning.html
...
See: http://developer.apple.com/documentation/General/DeveloperTools/Conceptual/XcodeUserGuide21/Contents/Resources/en.lproj/02_04_design_data_model/chapter_15_section_5.html

----

How can I initialize objects newly added to the object graph by a controller? I had thought I could override the add<key>Object: method of the owner, as that is what the controller is hooked up to, but that never gets called, nor does insertObject:in<key>General/AtIndex:. I tried overriding the onAwakeFromInsert method of the General/NSManagedObject sub-class of the newly-added object, but that gets called before the relationships are put into place, and I need those. Any ideas?

*There's an example here: *
http://developer.apple.com/documentation/Cocoa/Conceptual/General/NSPersistentDocumentTutorial/03_CustomClass/chapter_4_section_3.html
...
**
How about overriding init: in your General/NSManagedObject subclass?
...
I did grab the accessor for the collection. "addFieldsObject:". Unfortunately, it is not called. I set up a breakpoint that was not hit. The add is done by a button hooked up to the add: action of the General/NSArrayController handling the collection. The "contentSet" binding of the General/NSArrayController is hooked up to the "selection.fields" binding of the Form 's controller, and "fields" is the KVC/General/CoreData key for the collection. The array controller's entity is set to "Field".

AFAICT, everything looks like it ought to work.

**
IIUC, you could try subclassing General/NSArrayController for your Fields object and ovverride its add: method to populate the order variable with a count of the controller's arrangedObjects.
**
...
What does the "Transient" option mean in General/XCode 2.0's data modeler? I assume this is covered somewhere in the Core Data documentation, but searching for 'transient' yields confusing results for someone clueless about the concept and doesn't know any other keywords to use. :-) Help, please? I find some of the documentation for Tiger a bit ... frustrating.
...
*Yes, and you can use it for values that General/CoreData can't represent. See http://developer.apple.com/documentation/Cocoa/Conceptual/General/CoreData/Articles/cdNSAttributes.html. *

But to use a transient attribute as a calculated value, I'd have to create a subclass of General/NSManagedObject and implement this, setting the entity's class to General/MyManagedObjectSubclass ... or whatever, right? so -calculatedFoo would require General/MyManagedObjectSubclass to contain its own -foo / -setFoo KVC-compliant methods right?

*Correct.*

----

From: http://developer.apple.com/documentation/Cocoa/Conceptual/General/NSPersistentDocumentTutorial/04_Department/chapter_5_section_2.html

*"Because of the way Core Data operates, it is not possible to easily support autosaving in an General/NSPersistentDocument-based application. Core Data cannot save to a store and maintain the same changed state in a managed object context, all while keeping an unsaved stack around as the current document. For similar reasons, General/NSPersistentDocument does not support Save To operations."*

What does this mean? My application implements an auto-save scheme for individual external files managed centrally through a main project file (the General/NSDocument subclass) in a project setting. Creating a new external file automatically adds it to the project, so if the user adds a new external file then doesn't save the General/NSDocument subclass, it puts the whole project in an inconsistent state. So what I'm asking is *why* can't I auto-save? Is there *any way* around that without dropping Core Data? What a miserable limitation in my case! :-(  Reading this more carefully, however, suggests only that it can't make on-the-fly saves, correct? So forcing the context to -save whenever I tell it to should work, right?
...
The way you specify search predicates for a General/NSSearchField is quite straight forward. You can display only the employees whose manager is X.
...
(How) do you have to set the binding of the �filterPredicate� General/NSArrayController (which is very poorly documented)?

*I found a non-binding solution, the pop-up sends an action to a controller which performs a fetch on the General/NSArrayController:*
    
-(General/IBAction)fetchEmployeesForManager:(id)sender {
General/NSString * manager = [sender titleOfSelectedItem];
General/NSPredicate *predicate = General/[NSPredicate predicateWithFormat:@"manager.name like %@", manager];
...
I'm trying to get unit testing (using General/UnitKit) working with my Core Data Model and Classes. I'm having trouble and am looking for an assistance. -- General/MikeZornek

----

Regarding the General/NSManagedObjectContext     -reset method - if I perform an operation that faults in a large number of objects, would I then save the state and 'reset' the context to release/destroy all those objects to save on resource usage? Is that the purpose of     -reset? -- Update: Calling -reset killed all my unsaved changes to the object graph. I still don't know if it's just unsaved changes since last save, or if it completely resets the context to a 'new document' state regardless of the persistent store.

*Using     -reset is effectively similar to deallocing the context and creating a new one. It "clean slates" the context, clearing all of the pending changes out. If you don't     -save, any changes that took place since the last save will be gone. When a General/PersistentStore is added or removed from the General/PersistentStoreCoordinator, you must send     -reset to the General/ManagedObjectContext so that it doesn't get confused. I believe this is the most common use of     -reset.* -- General/ScottStevenson

----

Here's a method to do shell-like globbing from a General/NSSearchField. I'm sure it can be much improved, and hope it will be ;) This is from my first General/CoreData project.
...
{
	General/NSManagedObjectContext * context  = General/[[NSApp delegate] managedObjectContext];
	General/NSManagedObjectModel   * model    = General/[[NSApp delegate] managedObjectModel];
	General/NSDictionary           * entities = [model entitiesByName];
	General/NSEntityDescription    * entity   = [entities valueForKey:@"Post"];
	
	General/NSString *searchString = [sender stringValue];
	...
	General/NSPredicate * predicate = General/[NSPredicate predicateWithFormat:@"text LIKE[c] %@" argumentArray:General/[NSArray arrayWithObject:searchString]];
	
	General/NSSortDescriptor * sort = General/[[NSSortDescriptor alloc] initWithKey:@"text" ascending:YES];
	...
	General/NSArray * results = [context executeFetchRequest:fetch error:nil];
	General/NSLog(@"%@",results);
...
Apple's General/NSPersistentDocument tutorial thing shows how to use -    awakeFromInsert to automatically (and crudely) increase the id of each record added by one; therefore the first would be 1, the second would be 2, and so on. Unfortunately, and here is the crude part, it does this by means of a static int, and so it seems to me that if you relaunch the application it's going to add the next one at 1 again.

So what I'm wondering is, how do you, in your General/NSManagedObject subclass, find the max id--or any property, really--for instances of an entity? Does one use a predicate involving     array[last]? I'm well beyond lost here, so any help is very much appreciated.
...
-(void)awakeFromInsert {
	...
	[self setValue:General/[[NSDate date] descriptionWithCalendarFormat: @"%b %e, %Y" timeZone: nil locale: nil] forKey: @"dateIssued"];
	
	id request = General/[[[NSFetchRequest alloc] init] autorelease];
	
	[request setEntity: General/[NSEntityDescription entityForName: @"Invoice" inManagedObjectContext: [self managedObjectContext]]];
	[request setPredicate: General/[NSPredicate predicateWithFormat: @"id > 0"]];
	[request setSortDescriptors: General/[NSArray arrayWithObject: General/[[[NSSortDescriptor alloc] initWithKey: @"id" ascending: YES] autorelease]]];
	
	id error = nil;
	id array = General/self managedObjectContext] executeFetchRequest: request error: &error];
	
	if(array) [self setValue: [[[NSNumber numberWithInt: General/[array lastObject] valueForKey: @"id"] intValue] + 1] forKey: @"id"];
}
...
**I just use [[[NSNumber numberWithDouble:General/[[NSDate date] timeIntervalSinceReferenceDate]] as the ID. There's probably a better way, but unless you're creating many objects at full speed, you're pretty much guaranteed a unique ID. Even at high speed, it's not very likely to be executed fast enough to get the same ID. For my app, the user is clicking a button (or whatever) to create objects, so this works just fine.**

*That's a good idea, but for invoices I'd prefer the General/IDs to be immediately sequential, rather than just ordered. If I can figure out why the double references in the table are going on, I'll post it here, to be sure.*
...
Oh, and also, I've found out that in a table view, if there is a column whose data cell is a pop up menu, as might be the case if you had IB generate the interface for something with inverse relationships to an Entity-- like an Invoice's connection back to its Project-- and you have a row selected, and then click on that menu on another row... the array controller's selection is not updated at the time that the menu is created. Thus if you are populating that menu based on the array controller's selection, you will tend to see the -    description of an General/NSManagedObject (or your custom class) showing the fault for the value you have selected (if any) and otherwise the values for the selected row... not for the row you're editing. Ouch.
...
or if you only need the update in the GUI, you create a method in the parent called e.g. 'updateKVO', triggered by the change in the price and quantity text fields (the usual target/action mechanism), and in this 'updateKVO', you do
     [self willUpdateValueForKey:@"items'];[self didUpdateValueForKey:@"items"]; --General/CharlesParnot


----

How do you use the filterPredicate in an General/NSArrayController and its bindings?? And what�s the point?

It is simply use to filter the array, and select only those elements that you want to keep in the GUI. For instance, if the objects in the array have a 'isDeprecated' key, you could use a predicate     isDeprecated==0 in the General/NSArrayController to not display the deprecated objects.For this, you type the predicate in the field, and then click 'Set Predicate' and then the predicate is parsed and formatted and ready to use. The predicate is not used in the bindings per se, but only to restrict the number of elements that the GUI will display.
...
*Forgive the "RTFM" response, but: http://developer.apple.com/documentation/Cocoa/Conceptual/Predicates/index.html#//apple_ref/doc/uid/TP40001789  ...this will answer your questions if you actually take time to read it and not skim it. Your assumption that you can only use General/NSPredicate (and fetch requests) when the value is known is incorrect. You can use substitution variables (which is how the search field works). Honestly, "TFM" is exactly what you need to answer these questions.*
...
Hello! I�m saving two dates in my data. When I reopen the app a calculated value is supposed to give the number of days between the two dates using the years:months:days:hours:minutes:seconds:sinceDate: method. I get the error General/[NSCFDate years:months:days:hours:minutes:seconds:sinceDate:]: selector not recognized [self = 0x33f840] and I have to manually reenter each date twice until the calculation will take place normally. What is happening? Do I need to convert General/NSCFDate to something else??

*
The method you're trying to call is a method of General/NSCalendarDate, and your error message indicates that the object is an instance of General/NSDate (actually of General/NSCFDate which is a private concrete subclass of General/NSDate).  Either convert your date object to a calendar date or do something else.
*
...
General/NSArray * keys = General/[NSArray arrayWithObjects:@"mySecondObjects.General/AttributeToSumUP", nil];
[self setKeys:keys triggerChangeNotificationsForDependentKey:@"mySumUpAttribute"];
 --General/FloFl

*Check out     + (void)setKeys:(General/NSArray *)keys triggerChangeNotificationsForDependentKey:(General/NSString *)dependentKey.*

Oh, thank you! Did you read the code above?? A key doesn�t seem to be able to be dependent of keys from a relationship... --General/FloFl
...
-(void)setValue:(id)value forKey:(General/NSString *)aKey
{	
	General/MQUsage *u = [self valueForKey:@"usage"];
	BOOL notifyUsageOfTimeChange = (u && [aKey isEqual:@"timeDuration"]);
	/* if somebody calls setValue:General/ForKey: to change "timeDuration", 
		the General/MQUsage object, which has keys for the sum of timeDuration,
		will be notified with willChange/didChange below.*/	
	if (notifyUsageOfTimeChange)
		General/self valueForKey:@"usage"] willChangeValueForKey:@"totalTime"];			
...
I have an General/NSManagedObjectContext bound to a table via an General/NSArrayController. When I programmatically insert my managed objects into the context, the tableview does not reflect the changes. However when I wire up a button to the arraycontroller add: action, when the button is clicked, the result always shows up.
...
See http://developer.apple.com/documentation/Cocoa/Conceptual/General/CoreData/Articles/cdFAQ.html#//apple_ref/doc/uid/TP40001802-DontLinkElementID_14 ...

----

General/NSArrayControllers bound to General/CoreData entities don't seem to allow multiple selections in table views.  Is there anything I can do so that multiple objects can be selected?

I use many table views bound to array controllers using Entities, and multiple selection is allowed when I check the box in the General/TableView attributes.

----

How do I create a General/SearchPredicate binding for an General/NSSearchField when I want to search within objects linked by a to-many relationship? If I want to return only objects for whom the object linked by a to-one relationship called *fubar* has the searched *name* property, I can just do make the predicate:
...
General/NSArray* uniquedFirstNames = [thisIsAnArrayOfPersons valueForKeyPath:@"@distinctUnionOfObjects.firstName"]; 
...
How do you make the file in a General/NSDocument General/CoreData app as a folder (with extension; see General/FolderBasedDocuments), and save other files, whatever they are, in that package with the document's saved file, when General/CoreData saves the document data in that XML or whatever stored file inside of said folder?

----
You referenced the "General/FolderBasedDocuments" page but you don't seem to have read it. Read the very last section on that page - you **cannot do this with Core Data**.

----

The 32k limit was hit and the last question and a reply has been moved to General/CoreDataQuestionsPageTwo.