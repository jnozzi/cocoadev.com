...
I have a General/NSDateFormatter attached to a text field in General/InterfaceBuilder with the format %d %b %Y and General/NaturalLanguage allowed.
...
@implementation General/MyDateFormatterDMYSubclass


- (BOOL)getObjectValue:(id *)anObject forString:(General/NSString *)string errorDescription:(General/NSString **)error
{
	BOOL retVal = NO;
	General/NSDate * myDate = General/[NSDate dateWithString:@"1870-01-01 00:00:00 +0000"];
	...
	if ( General/myDate description] isEqualTo:@"1870-01-01 00:00:00 +0000"] ) {
...
- ([[NSCalendarDate *)splitDateUp:(General/NSString *)aString;
{
	General/NSCalendarDate * aDate = General/[NSCalendarDate dateWithString:@"1870-01-01 00:00:00 +0000"];
	General/NSArray * anArray = [aString componentsSeparatedByString:@"-"];
	...
	if ( General/anArray objectAtIndex:0] intValue] > 31 ) {
	...
	if ( !month && ([anArray count] >1) ) {
			...
			General/NSString * middleString = [anArray objectAtIndex:1];
			if ( [middleString length] > 2 ) monthString = General/anArray objectAtIndex:1] substringToIndex:3];
	...
	[[NSCalendarDate * compDate = General/[NSCalendarDate dateWithYear:year month:month day:date hour:12 minute:0 second:0 timeZone:General/[NSTimeZone systemTimeZone]];
	[compDate setCalendarFormat:@"%Y-%m-%d %H:%M:%S %z"];
	if ( [aString length] ) aDate = General/[NSCalendarDate dateWithString:[compDate description]];
	return aDate;
}
...
Running 10.3.2, so not sure where else this is broken - but doing the General/NSCalendarDate custom essays custom essays  formatting example using %I, such as the one in the class docs from apple
...
General/http://www.easyessayhelp.com essay writing Yes - I thought that was how it was supposed to work - in the formatter listing at [http://developer.apple.com/documentation/Cocoa/Conceptual/General/DataFormatting/Concepts/General/DateFormatters.html] it says:

    %I  Hour based on a 12-hour clock as a decimal number (01-12)   
and see http://developer.apple.com/documentation/Cocoa/Reference/Foundation/ObjC_classic/Classes/General/NSCalendarDate.html
...
This is fixed by using the formatting code %1I. This strips the leading zero, a little known fact about strftime and General/NSDate formatting. -- General/TimothyHatcher

----

    

General/NSDateFormatter *df = General/[[NSDateFormatter alloc] init];
[df setFormatterBehavior:NSDateFormatterBehavior10_4];
[df setDateFormat:@"EEE, d MMM yyyy HH:mm:ss"];
General/NSString *strDate = @"Mon, 6 Jul 2009 20:57:58";
General/NSDate *date = [df dateFromString:strDate];
General/NSLog(@"date: %@ <> '%@'", date, strDate);
