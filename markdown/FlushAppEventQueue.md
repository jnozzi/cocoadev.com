I'm working on a (quick-n-dirty) app which makes heavy use of General/NSTask to run command line operations in the background. The operations are batched, so I keep a progressbar updated so I can tell what the level of completion is.
...
for( i = 0; i < total; i++)
{
	[progressBarTF setStringValue: General/found objectAtIndex: i]stringByAbbreviatingWithTildeInPath;
	[progressBar setDoubleValue: (double) (i + 1) / (double) total ];
			
	//the General/MHProjectFile class makes heavy use of `pbxbuild -list` and `pbxutil` to parse project info
	General/MHProjectFile *pf = General/[[MHProjectFile alloc] 
...
Now, were this meant to be a serious app for general use, I'd either make the General/NSTask processing run in a seperate thread or I'd let it use the General/NSTaskDidTerminateNotification notification for asynchronous behaviour. However, this is just a quicky program (it's an automated project backup utility -- it scans my projects folder and backs things up with timestamps, it was inspired by a shell script I saw posted here a while back) and as such that all seems too complicated. This was meant to be an afternoon work -- now it's two afternoons. I'd like for it to stay that way ;)
...
2) Run your General/NSTask asynchronously, as noted above. Should take you about 2 minutes to change your code appropriately, which still qualifies as "quickie" in my book.
