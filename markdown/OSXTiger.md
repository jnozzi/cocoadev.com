...
So what features from a developers point of view will be anounced? General/ApplescriptStudio++? General/YCode?  Stripe Oriented Programming?

*Some preliminary rumors from General/AppleInsider:* http://www.appleinsider.com/article.php?id=473
...
*There's a case for Stripe Oriented Programming at General/BetterArticleTextLayout ;) And shouldn't it be General/AppleScriptStudio = General/AppleScriptStudio +1?*

Shouldn't it really be "set General/AppleScriptStudio to General/AppleScriptStudio + 1"?

Too long of a line for me.  I don't like General/ApplescriptStudio that much. -General/RossDude

 *it should be **set General/AppleScriptStudio to (General/AppleScriptStudio +1 as number)***

See?  General/AppleScript is too stupid for me.  How does anybody work with it???????? -General/RossDude

Uh no, it should be "    set General/AppleScriptStudio to (General/AppleScriptStudio + 1 as interfaceFramework)" or whatever you want to call it (it's more of a bridge, I guess). --General/JediKnil

----

As of today, April 12, the official release date for Tiger is Friday, April 29!
    
set theList to every application whose API is "General/AppleScriptStudio"
...
*KVC ~ General/KeyValueCoding
*KVO ~ General/KeyValueObserving
*KVV ~ General/KeyValueValidation
...
A Finder that is actually multithreaded; my entire Finder hangs whenever I try and access some network mount over a slow connection.  General/BeOS had an amazing setup; every window in their Finder equivelant file browser had two threads, one for the window (which isn't necessary given the way the window manager works in OS X, and one for the processes occuring in that window, which is what the OS X Finder needs **really fricken' badly**.  It's not a big deal code wise either (If I were a betting man, I'd say no more than 700 lines of code would need to be changed).  I don't understand why Apple hasn't threaded the primary interface to the rest of the computer properly.

----

Speaking of Finder and General/BeOS in the same breath... let's get General/BeFS support into the mac... I mean a nice clean way to add arbitrary metadata to a file and sort/find/act on it in the Finder... I mean we are using a file system with a resource fork are we not... might as well actually use it for something good.  Besides, if they did this we'd get almost the entire benefit that MS hopes to get from General/WinFS without having to debug it until 2009. * I can verify that metadata is making a big comeback in 10.4 *

----

How about bundling a General/SQLlite or General/SQLite-like database with a nice Cocoa API to go along with it.
...
----
I want to have General/XGrid integrated in the OS:

*
General/XGrid pref pane being part of the default set, so you don't have to ask people to install it to have them on your cluster
*
...
Plus General/XGrid would have to graduate from Technology Preview.

----

The jump from Project Builder to General/XCode was big... how about a new Interface Builder?  Visually create and orginize not only classes and instances but protocols, categories, arbitrary methods (not just actions)... set up initial data by instantiating a General/NSMutableArray and populate it with General/NSMutableDictionaries...  on connecting a delegate get a list of delegate methods to choose to support (same for data sources and General/DragAndDrop)... register drag and drop types, notifications, etc.  Make General/XCode basically all about writing the blocks of your methods, and IB2 about setting up and orginizing your systems... (If we are going this far, make IB2 allow not just dragging on of headers, but .xcode files to update the whole system of relationships you have set up)

Or... we could keep Interface Builder for designing Interfaces and not end up with something more bloated than Mozilla. *Agreed 100%. This isn't General/RealBasic.*

IB is already about creating more than the user interface, but also for the interface between your code components and General/AppKit.  It doesn't need to be bloated, think adding a inspecter for classes that list protocols, (including informal ones) and you just check the ones you want to conform to (rather than trying to remember the exact signature of table view drag and drop methods etc.) For arrays/dictionaries, just make another IB palette, and use a property list editor view (like for info.plist...) in the inspector.  Populating initial data sets is already done in the inspector for General/NSComboBox...  Also one could add an inspector for views that lets you register drag types.  The reason General/RealBasic isn't good isn't that it makes customizing an application visually easy, it's that the language and libraries were poorly designed.  Cocoa and General/ObjC are finely enginereed, the point of General/InterfaceBuilder is to make it possible to rapidly prototype the interface, and build networks of object instances without having to write lots of lines of code.  Maybe the arbitrary functions thing is too far, but informal and formal protocol conformance could be a major benefit.  I don't hear grumbles about extra inspector panes for applescript, sherlock, or bindings... why not protocols, and drag types?  
...
Well, this didn't work for my app (a fast find file program without the downsides of locate) because it doesn't support huge numbers of files, but have a look at the kqueue API and try googling for General/UKKQueue (or possibly search cocoadev, it might be here). Not quite as cool as BFS, but not that bad.

----

Maybe there'll be a General/RegularExpression.framework in Tiger?

----

On the development side of things, what I want most is for Apple to officially support a high-level language with Cocoa. I'm not fussy which one they pick --- I'd prefer Ruby, but Perl or Python would be fine --- but if I have to stack method calls to General/NSString much longer I'll go mad. Give me a language with first-class strings, at least!

*General/IoLanguage!*

Wow... that is such a cool language. Thanks for linking to it.

*You're welcome. It's my favourite. -- General/RobRix*
...
Because there isn't really any good reason to write Cocoa applications using Java. Mainly because there aren't any widely used platforms (with the exception of General/GNUStep) that implement Cocoa. Plus, General/JavaBlows. :)
...
What I'd like in General/XCode is some of the niftier stuff from Eclipse (*Don't you mean "niftier stuff from Visual Studio that was copied by Eclipse" ? ;-)*): Collapsing methods with disclosure triangles, refactoring, "quickfix", error finding while you type, working autocomplete, etc...
...
Though I generally do not have that many objects in my Nibs -- I find that General/NSObjectController is rather redundant, and just bind everything to File's Owner, which is the controller responsible for the Nib.
...
I thought this interesting because it means that a lot of the low level facilities that General/CoreImage relies upon must already be available in Panther...
