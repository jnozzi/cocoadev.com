Hi, I am getting to grips with Cocoa/General/ObjC and have the following conundrum...
...
The third class is where all my more serious gubbins is, it basically uses some array's and some other simple things, and stores the data I am using (just for some General/NSTableViews) in my app.
...
and have instantiated it on awakeFromNib thus -     thirdClassInstance = General/thirdClass alloc]init];
...
Don't want to seem stupid, but I'm not sure I understand everything in the General/SingletonDesignPattern. It seems real complex to me, is it all necessary to allow access to my instance from another class? 
...
It's not *that* complex, and it *is* just a slightly different way of instantiating the class. Instead of calling General/thirdClass alloc] init] multiple times, you just call it once from thirdClass* sharedInstance method (after first checking firstClass == nil), and then call [thirdClass sharedInstance] the rest of the time.
...
Note that this design pattern (essentially) mirrors that in [[NSUserDefaults, General/NSFileManager, General/NSNotificationCenter, General/NSApplication, General/NSHelpManager ... It's everywhere.

----
You could just link up all three controllers in IB...
Use the nib to instantiate them.. give each General/IBOutlets to the others, and then link them up in IB.
You can add classes to IB by dragging the .h file from PB/General/XCode.. and then you instatiate your controllers by choosing the class, and the choosing instantiate from the Classes menu.
...
All I'd like to do is change a couple of things (in an General/NSMutableArray) in the programmatically instantiated class. (Thanks for all the advice, but some of it conflicts, and I'm still confused!)
...
Since you say 'programatically instantiated class', do you have instances of the other 2 classes in your nib? If so, the easiest thing to do would be to create an outlet (I'll call it owningClass - you can call it whatever you like) in the class that does NOT instantiate ThirdClass and connect it to the class that does. Then add an accessor method to the class that instantiates thirdClass as described above. Then you can say 
    General/owningClass thirdClass] doWhateverWithThirdClass] in the class that doesn't create thirdClass. You can call any method in thirdClass that way.
...
See also General/HowToTransmitDataBetweenClasses
