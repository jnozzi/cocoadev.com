...
I'm working on an app that contains within its Resources folder some very sensitive information.  I've been looking into ways to prevent people from poke around with it by exploring the package with little or no success.  I am not aware of anyway to block access to the file itself, so I have been looking for methods of encrypting it.  Anyone got any suggestions on what to  use? Cocoa/General/ObjC things are best, but I can use plain C if necessary. --General/OwenAnderson

----

How large are your resources?

Unlikely to ever become more than a megabyte.  Also, I suppose I exaggerated when I said "very sensitive".  It's not like people's credit card numbers or something.  But it is people's intellectual property and I don't want just anyone to be able to easily extract it from the package. --General/OwenAnderson
...
If the information is user data, such as documents or the like (what would they be doing in Resources?) you could work with General/OpenSSL to encrypt and decrypt the data using a user-supplied public/private key pair. The user would need to supply the key, of course. 
...
I looked through the Security framework but couldn't decipher the API, and the documentation was little help.  Does it implement such symmetric ciphers?  If so, where's the documentation on them?  I looked into crypt(), but I could never find any good documentation beyond the man page, which is just a tad spartan for someone without a background in cryptography.  --General/OwenAnderson
...
I agree the Security framework API is a mess, but it's not as bad as General/IOBluetooth. I'll take a look at it. Maybe someone else here has some advice on its use.
...
See General/CheesyXOREncryption for an example of what I'm talking about.
...
I think the zlib trick is cool, especially when combined with XOR in some way for a poor-man's scrambler, but again, it's just obscuring the source. I agree, for real applications use General/OpenSSL or General/SecurityFramework. But Owen's original problem was he didn't "want just anyone to be able to easily extract" his data, not encrypting sensitive data. I call this level of security General/SeatLeashSecurity, to borrow a metaphor from the bicycle world.

-- General/MikeTrent

----

Also, I think that for protecting intellectual property like Owen wants to the encryption doesn't really have to be insanely uncrackable.  It really only matters that it would take some work to get in. To the best of my knowledge, the General/DigitalMillenniumCopyrightAct (DMCA) explicitly prohibits "circumvention" of any copyright protection schemes, no matter how flimsy they are.  Just look at the big deal made over DVD scrambling and programs that can rip General/DVDs to easily sharable mpegs.  General/DVDs are hardly unbreakable, but they hold a work that took millions of dollars and a lot of people to produce.  Just the fact that there is a barrier there that must be broken is why the DMCA protects General/DVDs. Owen's issue seems like a similar problem.  General/SeatLeashSecurity would probably be good enough.  Just my 2 cents.
...
*Yes, see above, I call this level of security General/SeatLeashSecurity. -- General/MikeTrent*
...
Sorry for going a little off-topic (and perhaps write what some would consider flame bait) but as for the **AA and the resources having to be in memory, this is what the new General/TrustedComputing tries to remedy, and according to news, the x86 developer machines from Apple uses Intels Infinion chip.
...
So while DRM is theoretically impossible today, it may require hardware hacking in the future. So **AA are probably not wasting their time/money, if they have helped the industry (including Apple) to switch to General/TrustedComputing.