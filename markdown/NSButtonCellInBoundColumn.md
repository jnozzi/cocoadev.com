...
I have an General/NSArrayController that contains Foo objects. Each Foo object contains (among other things) an array of Bar objects called "bars". I have an General/NSTableView for my Foos, whose only column is bound to the Foo array controller. With me so far? ;-)

I have another General/NSArrayController for the Bar objects whose content is bound to Foo's "selection.bars".  The selected Bars table has two columns: "Bar Title" and "X" (for delete bar from selected foo). In the "X" column, I have set the data cell type to General/NSButtonCell to get a pretty little round 'x' button to remove the Bar from the selected Foo when clicked. To accomplish this, the "X" column's Target is bound to Foo's "selection" with "removeBar:" as the selector. Argument is bound to Bar's "selection".
...
- (void)tableView:(General/NSTableView *)aTableView willDisplayCell:(id)aCell forTableColumn:(General/NSTableColumn *)aTableColumn row:(int)rowIndex
{
	...
	if (aTableView == tableTasks)
	{
		...
		{
			id task = General/acTasks arrangedObjects] objectAtIndex:rowIndex];
