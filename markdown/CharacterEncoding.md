...
General/NSString is *conceptually* a unicode string.  Internally it does everything in unicode.  It only gets messy when you read and write the string to some device (like a file or network).  Suddenly, exactly what format the bytes are in matters.  Thats why you have to specify an encoding when you go to General/NSData.  Most of these encodings are now considered legacy.  The world is going to unicode (nevermind that unicode sucks for asians - its the best thing going for now).  The preferred persistent format is UTF8.
...
{originally posted on Apple's General/CocoaDev list by Todd Blanchard}

posted here by General/DiggoryLaycock - because it explains so well - where Apple's docs just throw jargon at you

----
I would like to add just small addition:
There are some predefined constants in General/NSString.h like General/NSASCIIStringEncoding and such but they of course doesn't contain all available encodings on the OS X. I'm finding extremely usefull following code (for everybody who need to handle with text encodings)...
     
@implementation General/NSString(_encodings_)
- (General/NSArray*)allAvailableEncodings
{
    General/NSMutableArray*     array = General/[[NSMutableArray array] retain];
    const General/NSStringEncoding*     encoding = General/[NSString availableStringEncodings];

    while (*encoding) {
	General/NSMutableArray*	row = General/[NSMutableArray arrayWithCapacity:2];

        [row addObject:General/[NSString localizedNameOfStringEncoding:*encoding]];
        [row addObject:General/[NSNumber numberWithInt:*encoding]];
        encoding++;
...
- (int)numberFromLocalizedStringEncodingName:(General/NSString*)aName
{
    General/NSArray *encodings = General/self allAvailableEncodings] retain];
    [[NSEnumerator *en = [encodings objectEnumerator];
    General/NSArray *encPair = General/[NSArray array];
    int searchedNumber = 0;
    
    while (encPair = [en nextObject])
    {
	if (General/encPair objectAtIndex:0] isEqualTo:aName])
...
----
Just found a link that also belongs here on the General/TextFileParsing page of General/CocoaDev... read it...loved it:
http://www.joelonsoftware.com/articles/Unicode.html --General/CharlesParnot
...
http://www.popjisyo.com/General/WebHint/Portal_e.aspx  (basic translation help for Japanese <-> English )

----
And don't forget to create UTF8 strings with precomposedStringWithCanonicalMapping (cf. General/NSString; Unicode Normalization Form C, http://diveintomark.org/archives/2004/07/06/nfc )

----

Leopard (10.5) supports specifying the encoding of a text file via the "com.apple.General/TextEncoding" file attribute. This attribute has a format like "UTF-8;134217984", where the first part is the IANA name and the second is the General/CFStringEncoding value. General/NSString supports this to automatically read/write the encoding of a file.