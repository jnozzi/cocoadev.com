...
**Can my General/NSTask inherit a tty from the launching environment?**

I have a simple cocoa app that calls terminal apps using General/NSTask.
...
You can't inherit a tty because there is no tty to inherit; your app's parent is the window server, or loginwindow, or one of those things, and they don't have any attached tty. If you need one, you'll have to create your own. General/NSTask doesn't really need deep examples, because there isn't much deepness you can do with it. All of the farting around with ttys happens before you start to mess with General/NSTask.

----

I was under the impression the General/NSTask could inherit through a setenvironment call which I thought changes its environment. I guess there isn't really an environment key that can flag the task in this manner.  How does the os know the difference between a cli program and a gui ?
...
General/NSTask is just a wrapper around fork() and exec(). It has nothing to do with ttys or anything like that, it's just an OO way to execute other programs.

----

**Unsolved problem in which app crashes - appears to include problems with setting up General/NSFileHandle**
...
which I guess will not work on any computer except my own. Is there quite a simple way to link the General/LaunchPath to a place within the bundle (for example if I was to place it inside 'magnetic/content/Resources/magnetic').
...
- (General/IBAction)createBanner:(id)sender
{
General/NSTask *magnetic;
General/NSPipe *output=General/[[NSPipe alloc] init];
General/NSString *theResult;

magnetic=General/[[NSTask alloc] init];
[magnetic setLaunchPath:@"/sw/bin/magnetic"];
[magnetic setArguments:General/[NSArray arrayWithObject:[bannerField stringValue]]];
[magnetic setStandardOutput:output];
[magnetic launch];
theResult=General/[[NSString alloc]
initWithData:General/output fileHandleForReading]
availableData]
...
*
- (General/NSData *)availableData

Returns the data available through the receiver. If the receiver is a file, returns the data obtained by reading the file from the file pointer to the end of the file. If the receiver is a communications channel, reads up to a buffer of data and returns it; if no data is available, the method blocks. Returns an empty General/NSData if the end of file is reached. Raises General/NSFileHandleOperationException if attempts to determine file-handle type fail or if attempts to read from the file or channel fail.
*
...
General/NSString *thePath=General/[[NSBundle mainBundle] pathForResource:@"magnetic" ofType:@""];
magnetic=General/[[NSTask alloc] init];
[magnetic setLaunchPath:thePath];
//etc...

Then you can make your script a resource and add it to your project and to the application target. It will be automatically added inside the resources when you build. You should even be able to work on it inside General/XCode. You may even get the syntax coloring and everything (is it a shell script?). If you modify the script, it will be updated in the app bundle at the next build (so do not work on the version inside the built product).
