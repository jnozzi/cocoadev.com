...
This page is a resource for techniques used in sorting the contents of an General/NSTableView by row based on the sort order of one of its columns.

A table view displays the contents of its data source passively, using the informal protocol General/NSTableDataSource. Typically, the data are in something like an General/NSMutableArray, declared as an instance variable of the data source object. The data source of the General/NSTableView is an outlet of General/NSTableView, similar to  a delegate object.

In sorting the data source array, which typically is a container for other objects, you need to compare those objects. The venerable NSC**'omparisonResult is how you compare objects with each other in a general way, by returning it from a method that compares two objects in the array. To learn about the basic nature of object comparison, consult the documentation for General/NSObject and in partcular, the     equals method.

A simple example of sorting a table using this technique is found at General/SortingExampleUsingNSComparisonResult

Further information on sorting an General/NSArray using sort descriptors can be found at

http://developer.apple.com/documentation/Cocoa/Conceptual/General/SortDescriptors/Concepts/Creating.html
...
With General/CocoaBindings and such objects as General/NSArray controller, you have powerful new ways of implementing this functionality (see below).
...
For further help on General/NSArray methods, see the numerous array-related topics to be found at this site.
...
General/SortingTableViewByClickingColumnHeaderExample

Since Jaguar, the icons that indicate ascending or descending sort order are now available as named images:

You can get these images with General/[NSImage imageNamed:] using the identifiers
...
With the introduction of General/CocoaBindings, a powerful technology for handling this responsibility is incorporated in General/NSArrayController
