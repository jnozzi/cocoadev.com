...
So, as discussed in General/CocoaDevRegistrations, General/TWiki is a collaboration tool similar in many ways to this one.  Except where this is based on the "Original Wiki" code, General/TWiki has a number of enhancements, such as page version histories & diffs, email notifications of changes, and user accounting / registration.

So far, sounds good.  But after reinstalling General/TWiki, I still don't feel like I like it very much.  It seems like everything that was good and simple about Wiki has been complicated by a factor of 10.

But why don't you try it and tell me what you think.

http://www.cocoadev.com/twiki/bin/view.pl/Main/General/WebHome [note: this link is now dead]
...
What about General/UseModWiki instead? It does CVS diffs, email notifications, and allows simple HTML and arbitrary wiki links using double square brackets instead of General/WordsJammedTogether. -pmb
...
I agree General/[TWiki is too complex]- David Kopec
...
Well I agree General/TWiki looks too complex at its current setup, isn't there an option to simplify it or something? If not, let's write a Wiki in Cocoa! :-) -- General/FinlayDobbie

----

Don't you mean in General/WebObjects, Finlay? :) My basic feeling is that all General/CocoaDev *really* needs is journalling...just like Mac OS X needs it :) -- General/RobRix

----

The General/TWiki you see at the above link is pretty much out of the box.  Yes, it could probably benefit from some customization.  But it's not clear where to start -- what can be done from within the program itself, and what requires source level editing.  Say for example, changing the font, which would be pretty high on my list of priorities, were we to switch.  Does anyone on here have General/TWiki experience that could help me make sure I'm not being unjustly harsh?

And the other question is whether making the customizations to General/TWiki would exceed the amount of time it would take to add the small set of extra features that seems to be desired to THIS Wiki.
...
I agree that this demo page looks horrid. But I'd still like features. If I had the time (see General/MikeTrent) I'd volunteer to look at your wiki script and take a stab at some of these things. But there you go ... 
...
- Integrated mailing list searching.  So, if you're at the General/NSDocument page, you can launch a search for mailing list items with "General/NSDocument" in the subject, let's say.  I have an almost complete archive of cocoa-dev, and most of General/MacOsxDev
...
Steven have you ever thought of doing it in General/WebObjects? 
...
and http://lightingwiki.com/General/WikiSandBox?action=edit for complicated formating (too complicated) But I like the multiple titles.

General/FrancoisFrisch

----

I didn't really consider General/WebObjects, because I'm much more familiar with PHP, and it's free.  :)
...
However it doesn't work for java classes (General/NSAlertSheet) and a lot of classes are obj-C AND java. They are only obj-C in the beta site. 

Also in General/NSActionCell if you click on  a     - setAction:   in one of the method's "see also" links you get to wiki page where as all the others(as far as I can see) work fine. Strange!
...
Hmm, seeing as I think I was one of the people who first let loose the bee into bonnets about General/TWiki...

General/StevenFrank: *My personal opinion: too complicated... Way too many options, too much text everywhere, and just basically ugly*

I really wish someone would revamp that default install for those guys.  I would, but I don't have time.  They manage to cram every single feature into the first page, and it just looks nasty.  First thing I did when I installed a General/TWiki was to strip it down to look not unlike the front page of this site.
...
General/TWiki has a lot of the features you want:

General/StevenFrank: *My ideas, in a nutshell, are: 
...
*- Simplified markup. *  It definitely has this.  It's a central feature of General/TWiki.  If anything, you might want to consider ripping this bit of regex out of the General/TWiki source and pasting it into your own customized Wiki

*- Reciprocal linkage. *  Doesn't have this, but I seem to remember someone working on a "plugin" for this.

So, it looks like General/TWiki has 3 out of 6 of your wishlist.  How hard would all six be to implement on your existing Wiki, and how hard would it be to grok General/TWiki to implement the remaining 3?  
...
Maybe I'll go download General/TWiki again and dig in to see if there's anything else I could say about it.

-- General/LesOrchard

----

We've just rewritten General/WODev in General/WebObjects (It used to be exactly the same perl script as cocoadev) and we are thinking of implementing the idea I had about the Apple Docs. One of the many advantages of General/WebObjects is that it is very easy to add features and such. It also has a database back end which is an advantage and a disadvantage (making diffs available is a lot harder)
...
As for General/WebObjects, I too favor the simplicity and elegance of Perl. Ultimately if WO will fit the bill faster than Perl I'm all for it. But I suspect Perl is the right choice for our illustrious site maintainer.
...
Yeah... this isn't the first time we've run into this problem. The General/CocoaDevToDoList went through about five incarnations or so...
...
As long as we're experimenting (are you still experimenting?), how about adding a feature that makes an rss.xml file with the contents of the Recent Changes page? That way users of programs like Radio General/UserLand (http://radio.userland.com/ ) could *Subscribe* to General/CocoaDev, and that would be very cool.
...
Hey, the RSS subscription thing is a great idea.  Maybe we could hack something up with Stapler or General/RssDistiller (is the second one free)?  I've been meaning to do so.

And, indeed, I've done so with General/RssDistiller: the RSS feed for General/CocoaDev is at http://web.sabi.net/rss/cocoaDevRecentChanges.xml.  Enjoy!
...
The feed is back after a multi-month outage. I've been getting it myself; the General/WebDAV upstreaming to publish it was just broken; sorry.
...
We're not switching engines. Standard Wiki syntax was **not** designed to make adding code easy. On a personal note, I didn't find General/CocoaDev intimidating, and I'd never used a Wiki before. Coming from a coding/markup background, I'd probably have found the ideosyncratic markup of standard Wikis more perplexing than %%s. But some better middle ground, that I'd plump for. -- General/KritTer

----

Standards wiki syntax resemble traditional plain text. For exaple, General/MoinMoin syntax: = Title =, == Title 2 ==, **Bold**, *Italic*, ---- (horizontal line), `inline code`,      Block of code  etc. I think this syntax is much easier to read and write. 


* General/MoinMoin is here: http://moin.sourceforge.net
* General/MoinMoin developers wiki is here: http://twistedmatrix.com/users/jh.twistd/moin/moin.cgi/
* General/MacMac, General/MoinMoin Hebrew wiki site is here: http://mac.plonter.co.il/plonwiki
...
It's the syntax for italic I was worried about, but I've just realised * isn't valid in General/ObjC anyway. Sweet. On this note, I'm going to get in touch with General/StevenFrank and see whether he's amenable to a switch.

In the meantime, let's have a community vote. -- General/KritTer

**Should we add standard Wiki markup to General/CocoaDev's parser?**
...
----
It's been a while since I last took a look at various Wiki technologies. To start with, this wiki is fine, it works quite OK, and there's a lot of good programming gone into this implementation. Anyway, if I would do a similar site, my favourite technology out there is General/MediaWiki (the engine powering General/WIkiPedia and similar web sites). The reason is that it has a General/MySQL backend and uses PHP, two things that will minimize performance issues, not that the General/MediaWiki is much bigger than usemod and similar lightweight wiki systems. The other thing I like about General/MediaWiki is that it has a clean editing environment for histories, comments, and forces people to login and this way keep the content editable without guessing who did what, and when. The discussion tag is especially helpful as a lot of side discussions could be moved out from the main page. Also, General/MediaWiki avoids using General/CamelCase so it looks more like a normal web site, and makes it possible to build links of any name structure (that then shows up in google searches, too).

Anyway, I'm sure this site could evolve into something similar by adding similar features along the way. Also, it's quite a pain to move content from one structure to a new wiki system, it's like starting all over in many cases (General/MediaWiki actually allows to migrate usemod databases over). 

http://www.onlamp.com/pub/a/onlamp/2004/11/04/which_wiki.html has a review of the common Wikis. As an update to this review, the initial configuration of General/MediaWiki could now be done via a browser session.
