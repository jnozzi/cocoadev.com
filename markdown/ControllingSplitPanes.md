...
I have a split pane in a drawer, and I have set up a delegate so that "- (float)splitView:(General/NSSplitView *)sender constrainMinCoordinate:(float)proposedMin ofSubviewAt:(int)offset" is called, and I can constrain the maximum size of the lower pane.
...
How about implementing the **drawerWillResizeContents:toSize:** or **splitView:resizeSubviewsWithOldSize:** delegate method and explicity re-constraining the split view? I know it's a pain. I, for one, am not happy with Cocoa's auto-resizing mechanisms. �General/DustinVoss
...
It should be noted that this problem seems to be pretty easily overlooked, as General/XCode editor windows' drawers can have the exact same problem when the file layout in said drawer is split, and then the window, or the split view is resized.
