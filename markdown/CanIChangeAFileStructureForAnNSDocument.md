...
- (BOOL)loadDataRepresentation:(General/NSData *)data ofType:(General/NSString *)aType
{
    ...
    if ([aType isEqualToString:@"General/DocumentType"])
	{
		iCallDocument *theNewDocument;
		General/NSRange myRange;		
		
		// Check the General/NSData data to see what type of archive this is... Keyed or Unkeyed.
		...
		myRange.location = 1;
		myRange.length = 8;				// The search parameters for the General/NSData object.
		General/NSLog (@"Here's the string I got: %@", General/data description] substringWithRange:myRange]);


Anyhow, the upstart is that if the data passed to this function start with the first hex signature, it seems that we're decoding a keyed archive, so a conditional instead of the [[NSLog line will order an General/NSKeyedUnarchiver. You get the picture. Now, can anyone else verify that all keyed archives reliably start with <62706c69 (I've set the range to ignore the leading less than sign), whereas unkeyed archives start <040b7479? Thanks again for your interest and support!
...
- (General/NSData *)dataRepresentationOfType:(General/NSString *)aType
{
    if ([aType isEqualToString:@"General/DocumentType"])
	{
		...
		return General/[NSKeyedArchiver archivedDataWithRootObject:self];  
	}
	General/NSLog (@"That was an invalid type to attempt to archive. (iCallDocument dataRepresentationOfType:aType false)");
	return nil;
}
...
[data appendData:General/[NSKeyedArchiver archivedDataWithRootObject:self]];
return data;
...
- (General/NSData *)dataRepresentationOfType:(General/NSString *)aType
{
    if ([aType isEqualToString:@"General/DocumentType"])
	{
		General/NSMutableData *data = General/[[NSMutableData alloc] initWithBytes:signature length:sigLen];
		[data appendData:General/[NSKeyedArchiver archivedDataWithRootObject:self]]; // Add the archived data
...
- (BOOL)loadDataRepresentation:(General/NSData *)docData ofType:(General/NSString *)docType
{
	...
	if (docSignature[sigLen - 1] != '\0') { // If the last byte of the document signature is not 0
		...
		General/NSRange dataRange = General/NSMakeRange(sigLen, [docData length] - sigLen);
		General/NSData *data = [docData subdataWithRange:realDataRange];

		self = General/[NSKeyedUnarchiver unarchiveObjectWithData:data]; // Not sure if this is how you do it.
		return (self != nil);
	}
}

I think this will work...but note that I'm not the person who suggested the idea, so I could easily be wrong. Note that since the old archives don't have any signature (unless you are going with Apple's private signature), you have to make sure not to cut anything off (using     subdataWithRange:) that needs to be there for General/NSKeyedUnarchiver to work. --General/JediKnil
----
Again, thanks to everyone for their suggestions. I basically did what you guys said, but I converted the data back to an General/NSString so I could use     isEqualToString: for the comparison between the first n bytes and the expected signature General/CString. Seems to work.... Only time will tell for sure. 'Course, it would've been nice if there was a convenient way to identify keyed v. unkeyed archives without these machinations. Even nicer if the General/NSKeyedUnarchiver would simply notify us that it's dealing with an unkeyed archive. At least this solves the problem in an apparently reliable way. If anyone wants the final code, drop me an e-mail through www.ascendiac.com > feedback. -dan

----

I'd try to use an General/NSKeyedUnarchiver and, if it fails (by raising an exception), switch to an General/NSUnarchiver.

    
id obj = nil;

NS_DURING
    obj = General/[NSKeyedUnarchiver unarchiveObjectWithData:data];
NS_HANDLER
    obj = General/[NSUnarchiver unarchiveObjectWithData:data];
NS_ENDHANDLER

// Do whatever you want with obj now


The contents of your archive should not *replace* your General/NSDocument-subclass object. You should copy data from the archive into instance variables in the General/NSDocument-subclass object. In the code above, the next few lines would do something like:
...
- (BOOL)loadDataRepresentation:(General/NSData *)data ofType:(General/NSString *)aType
{
	...
	{
		iCallDocument *theNewDocument;
		General/NSRange myLocalRange;
		myLocalRange.location = 0;
		myLocalRange.length = 12;				// The search parameters for the General/NSData object.
		General/NSString *myDataSignature = General/[[[NSString alloc] initWithData:[data subdataWithRange:myLocalRange] encoding:NSUTF8StringEncoding] autorelease];
		...
		if ([myDataSignature isEqualToString:@"keyedArchive"])
		{
			...
			// Now, send the data without the signature bytes to the General/NSKeyedUnarchiver:
			theNewDocument = General/[[NSKeyedUnarchiver unarchiveObjectWithData:[data subdataWithRange:myLocalRange]] retain];
			if (theNewDocument)
			{
			...
			{
				General/NSLog (@"Failed to unarchive the Keyed archive. iCallDocument > loadDataRepresentation");
				return NO;
			}
		}
		else
		{
			...
			theNewDocument = General/[[NSUnarchiver unarchiveObjectWithData:data] retain];
			if (theNewDocument)
			{
				[self setTheMasterSchedule:[theNewDocument returnTheMasterSchedule]];
				[self setTheOtherInfoDictionary:[theNewDocument returnTheOtherInfoDictionary]];
			...
			{
				General/NSLog (@"Failed to unarchive the Unkeyed archive. iCallDocument > loadDataRepresentation");
				return NO;
			}
		}
		// Both types of unarchivers failed. Post a notice!
		General/NSLog (@"Could not determine if the archive was keyed or not. Failed at: loadDataRepresentation:(General/NSData *)data ofType:(General/NSString *)aType.");
		return NO;
	}
	General/NSLog (@"Could not unarchive the file. Failed at: loadDataRepresentation:(General/NSData *)data ofType:(General/NSString *)aType, from if ([aType isEqual...]).");
	return NO;
...
----
A note on trying the keyed archiver and catching the exception, even when caught, General/CFLog(0) dumps several lines to stderr: 

**
General/CFLog (0): 
	General/CFPropertyListCreateFromXMLData(): plist parse failed; the data is not proper UTF-8. The file name for this data could be:
	-filename-
