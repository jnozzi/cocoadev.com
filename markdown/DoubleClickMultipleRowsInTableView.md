...
The second attempt, which was ultimately successful, was to subclass General/NSTableView and override <code>mouseDown:</code> to discriminate between single and double clicks. The idea is to forward genuine single clicks to <code>super</code>, but not double clicks. Of course, at the beginning of a double-click the table view has no way of knowing whether the click will be single or double until it waits for the second click to arrive (or not). 

We can implement this waiting period by calling the General/NSObject method <code>performSelector:withObject:afterDelay:</code> in the subclass's <code>mouseDown:</code> method. If a double-click event arrives in the meantime, we can call the General/NSObject class method <code>cancelPreviousPerformRequestsWithTarget:</code> to prevent the single-click.
...
- (void)mouseDown:(General/NSEvent *)theEvent
{
	...
	else
	{
		...
		General/[NSObject cancelPreviousPerformRequestsWithTarget:super];

		General/self target] performSelector:[self doubleAction;
	}
}
...
I finally solved the problem by overriding General/NSTableView's     selectRowIndexes:byExtendingSelection: method to disable the selection change when a single row of a multi-row selection is clicked on. I also implement a new method,     selectOnlyRowIndexes:, that accomplishes the deselection behavior and is called after the time delay. I also override the General/NSView method     dragImage:at:offset:event:pasteboard:source:slideBack: so that a multiple-row selection is preserved after dragging. As a final nicety, I check the user defaults to get the value of the     doubleClickThreshold.

Here's the code:

    
static unsigned int General/TAModifierKeyMask = General/NSShiftKeyMask | General/NSAlternateKeyMask | General/NSCommandKeyMask | General/NSControlKeyMask;

@implementation General/TAMultirowDoubleClickTableView

- (int)mouseRowForEvent:(General/NSEvent *)theEvent
{
...
- (void)selectOnlyRowIndexes:(General/NSIndexSet *)rowIndexes
{
    [super selectRowIndexes:rowIndexes byExtendingSelection:NO];
}

- (void)selectRowIndexes:(General/NSIndexSet *)rowIndexes byExtendingSelection:(BOOL)flag
{
    General/NSEvent *theEvent     = General/[NSApp currentEvent];
    int      mouseRow     = [self mouseRowForEvent:theEvent];
    BOOL     modifierDown = ([theEvent modifierFlags] & General/TAModifierKeyMask) != 0;
    
    if ( General/self selectedRowIndexes] containsIndex:mouseRow] && (modifierDown == NO))
    {
...
- (void)mouseDown:([[NSEvent *)theEvent
{
    ...
    if ( General/self selectedRowIndexes] containsIndex:mouseRow] && (modifierDown == NO) && (doubleClick == NO) )
    {
    ...
    else if ( doubleClick == YES )
    {
        ...
        General/[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(selectOnlyRowIndexes:) object:General/[NSIndexSet indexSetWithIndex:mouseRow]];
...
Try this - my General/TableView works on single click rather thanm double click - which is what I want.
...

{
   ...
   General/NSEvent *newEvent = General/[NSEvent mouseEventWithType:[theEvent type]
...
The example code General/ThomasAnderson posted has one small bug -- it doesn't fill in the sender parameter of the doubleAction selector, so it will segfault if you try to access that from within the doubleAction method.  The fix is to change the line:
