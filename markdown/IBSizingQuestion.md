...
if I have a General/NSBox with a General/NSBrowser as a subView (as a concrete example)
...
select the box and the arbitrary view and make them subviews of a General/SplitView.
...
the browser, who should maintain its distance from the top and bottom of the General/NSBox, has now gone off the top of the General/VisibleRect of the General/NSBox.
...
btw: I put my General/NSBrowser into a General/NSBox bc I need to also put Other controls in the same space and its the only semi-reliable 
way to get the sizing/spacing behaviour I want in a split View

----

This is a well known problem with General/NSSplitView - wether it is a true bug or not, I don't know. To fix it, you will need to set Min/Max limits on the split view. To accomplish this, you need to add code to the Split View's delegate. For information on how to do this, see the General/SplitViewBasics. There is also an additional "bug" where the min/max limits do not take effect when the user resizes the window. This issue is also addressed on that page. -- General/RyanBates