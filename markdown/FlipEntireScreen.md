...
----
Mac OS X's negative screen function is performed by applying a General/ColorSync/ICC profile to the output device; other ICC based transformations also allow the contrast capabilities and momentary flash effects.
...
----
That�s a hardware function; it�s only available with certain graphics chips. However, things like General/FastUserSwitching, General/DashBoard and General/FrontRow show that it�s possible, at some level, to use the �screen� content as a texture. The interface used for this is private and subject to change. -General/JensAyton