...
----
**This page is for General/NSTableView related solutions only**.
----
...
*entry5 - Change any row's color in an General/NSTableView
...
The trick is to realize that General/NSTableView uses one combo box only, it won't record the user selection since it reuses a single UI object... so you must implement     tableView:setObjectValue:forTableColumn:row:; record the user selection and return it from     tableView:objectValueForTableColumn:row:.

See General/NSComboBoxCellExample for a more comprehensive example.

----

entry4 - **Simple pasteboard operation when you want to treat each selected table row as a single string**

I have an General/NSTableView subclass that handles copying selected rows to the pasteboard as a string. The code runs from the     copy: method, and works fine. However, I call a method via the delegate using     performSelector:withObject:withObject:, and since you can only pass objects, I have to pass an     General/NSNumber instead of an     int to indicate the row:
...
if ([self numberOfSelectedRows]>0)
{
	...
	{
		General/NSMutableString *s = General/[NSMutableString string];
		int i;
		for (i=0; i<[self numberOfRows]; i++)
		{
			...
			{
				[s appendFormat:@"%@\r",
					[theDelegate performSelector:selector withObject:self withObject:General/[NSNumber numberWithInt:i]]];
			}
		}
		...
		if ([s length]>0)
		{
			...
			General/[NSPasteboard setString:s];
		}
	}
}


How I would  change this code to use     General/NSInvocation so that I could pass an     int instead of an     General/NSNumber. Would it be more efficient to create an     General/NSInvocation for each iteration of the loop and pass an     int, or just use     performSelector:withObject:withObject: and pass an     General/NSNumber?
...
In this case, there's no need to use either     performSelector: or General/NSInvocation. Just send the message thusly:
...
The simplest way to change row colors in an General/NSTableView is to subclass your General/NSTableView and use the same technique as in : http://developer.apple.com/samplecode/MP3_Player/index.html . Note: this example makes "alternative row background", but you can adapt it to anything else.
