...
General/NSValueTransformer is described nicely here:
http://developer.apple.com/documentation/Cocoa/Conceptual/General/ValueTransformers/index.html

Note, that you have to register the transformer, as explained in the above link, in order for it to work with bindings. The name you use for registering is what you'll use inside Interface Builder when doing binding.  *This does not appear to be true in IB3; you can just give it the name of a class and it will instantiate it.  Is this behavior documented anywhere, or did I get lucky?* http://goo.gl/General/OeSCu
...
the Model Key Path you can create General/ValueTransformers based on the Container Object e.g. the General/NSArrayController.
...
Bind To: General/GenomeArrayController
Controller Key: self

Model Key Path was not set

The General/ValueTransformer transformedValue: method was... 

    
- (id)transformedValue:(id)value {

	if ([value selectionIndex] > 0 && General/value arrangedObjects] count] > 3) {
		return [[[NSNumber numberWithBool:YES];
	} 
	
	returnGeneral/[NSNumber numberWithBool:NO];

}


----
Regarding above post, If you are using General/NSObjectControllers, you can use "selection" instead of "self". The downside to this is that this causes invocations for every change to the selection.
...
Has anyone got a list of public domain General/NSValueTransformer classes?


* http://homepage.mac.com/oscarmv/General/OMVFPValueTransformers.sitx
 
 * Math and transform sequences
 
*http://www.cocoalab.com/cocoalab/developer/classes/General/CLValueTransformers.zip
 *General/CoreData faults <-> bool
 * points <-> other units
 * General/NSString <-> General/NSNumber
 * Prefix numbers with 0
 * radians <-> degrees
 * General/NSCalendarDate <-> General/NSString 
 * SMTE time <-> decimal seconds
 
* Here on General/CocoaDev
